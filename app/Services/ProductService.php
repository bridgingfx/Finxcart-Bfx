...
class ProductService
{
    ...
    public static function getCategoriesArray(array $legacyCategoryColumns): array
    {
        $category = [];
        if (!empty($legacyCategoryColumns['category_id'])) {
            $category[] = [
                'id' => $legacyCategoryColumns['category_id'],
                'position' => 1,
            ];
        }
        if (!empty($legacyCategoryColumns['sub_category_id'])) {
            $category[] = [
                'id' => $legacyCategoryColumns['sub_category_id'],
                'position' => 2,
            ];
        }
        if (!empty($legacyCategoryColumns['sub_sub_category_id'])) {
            $category[] = [
                'id' => $legacyCategoryColumns['sub_sub_category_id'],
                'position' => 3,
            ];
        }
        return $category;
    ...
    /**
     * Derives the legacy single-branch category_id/sub_category_id/sub_sub_category_id
     * columns from an arbitrary set of assigned categories, for code that hasn't been
     * migrated to the many-to-many `categories` relation yet.
     */
    public static function resolveLegacyCategoryColumns(\Illuminate\Support\Collection $categories): array
    {
        $categoryId = null;
        $subCategoryId = null;
        $subSubCategoryId = null;

        $root = $categories->first(fn ($category) => (int)$category->position === 0);
        if ($root) {
            $categoryId = $root->id;
        } else {
            $ancestor = $categories->sortBy('position')->first();
            while ($ancestor && (int)$ancestor->position !== 0) {
                $ancestor = \App\Models\Category::find($ancestor->parent_id);
            }
            $categoryId = $ancestor?->id;
        }

        $subCategory = $categories->first(fn ($category) => (int)$category->position === 1 && (int)$category->parent_id === (int)$categoryId);
        if ($subCategory) {
            $subCategoryId = $subCategory->id;

            $subSubCategory = $categories->first(fn ($category) => (int)$category->position === 2 && (int)$category->parent_id === (int)$subCategoryId);
            if ($subSubCategory) {
    ...
    {
        $variations = [];
        $hasColors = $request->has('colors_active') && $request->has('colors') && count($request['colors']) > 0;
        $colorMap = $hasColors
            ? $this->color->whereIn('code', $request['colors'])->get()->keyBy('code')
            : collect();
            ...
            foreach ($combinations as $combination) {
                ...
                foreach ($combination as $combinationKey => $item) {
                    if ($combinationKey > 0) {
                        ...
                        if ($hasColors) {
                            $str .= optional($colorMap->get($item))->name ?? $item;
                        } else {
                            $str .= str_replace(' ', '', $item);
                        }
                    }
    ...
    {
        foreach ((array) json_decode($product['images'], true) as $image) {
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        $selectedCategories = \App\Models\Category::whereIn('id', $request['categories'] ?? [])->get();
        $legacyCategoryColumns = self::resolveLegacyCategoryColumns($selectedCategories);
        $isEventOrBroker = in_array($request['product_type'], ['event', 'broker']);
        $notSellable = $isEventOrBroker;

        $brochure = null;
        $brochureStorageType = null;
        if ($isEventOrBroker && $request->hasFile('brochure')) {
            $brochure = $this->fileUpload(dir: 'product/brochure/', format: $request['brochure']->getClientOriginalExtension(), file: $request['brochure']);
            $brochureStorageType = $storage;
        }
        ...
        return [
            ...
            'category_ids' => json_encode(self::getCategoriesArray(legacyCategoryColumns: $legacyCategoryColumns)),
            'category_id' => $legacyCategoryColumns['category_id'],
            'sub_category_id' => $legacyCategoryColumns['sub_category_id'],
            'sub_sub_category_id' => $legacyCategoryColumns['sub_sub_category_id'],
            ...
            'delivery_mode' => $request['product_type'] == 'digital' ? ($request['delivery_mode'] ?? 'manual') : null,
            ...
            'unit_price' => $isEventOrBroker ? 0 : currencyConverter(amount: $request['unit_price']),
            'purchase_price' => 0,
            'tax' => $isEventOrBroker ? 0 : ($request['tax_type'] == 'flat' ? currencyConverter(amount: $request['tax']) : $request['tax']),
            'tax_type' => $isEventOrBroker ? 'percent' : $request->get('tax_type', 'percent'),
            'tax_model' => $isEventOrBroker ? 'exclude' : $request['tax_model'],
            'discount' => $isEventOrBroker ? 0 : ($request['discount_type'] == 'flat' ? currencyConverter(amount: $request['discount']) : $request['discount']),
            'discount_type' => $isEventOrBroker ? 'flat' : $request['discount_type'],
            ...
            'minimum_order_qty' => $isEventOrBroker ? 1 : ($request['product_type'] == 'digital' ? 1 : max(1, (int)$request['minimum_order_qty'])),
            'video_provider' => 'youtube',
            'video_url' => $request['video_url'],
            'preview_url' => $request['preview_url'],
            'not_sellable' => $notSellable,
            'external_url' => $notSellable ? $request['external_url'] : null,
            'book_tickets_url' => $request['product_type'] == 'event' ? ($request['book_tickets_url'] ?: null) : null,
            'view_floorplan_url' => $request['product_type'] == 'event' ? ($request['view_floorplan_url'] ?: null) : null,
            'sponsor_exhibit_url' => $request['product_type'] == 'event' ? ($request['sponsor_exhibit_url'] ?: null) : null,
            'open_account_url' => $request['product_type'] == 'broker' ? $request['open_account_url'] : null,
            'contact_broker_url' => $request['product_type'] == 'broker' ? $request['contact_broker_url'] : null,
            'event_days' => $request['product_type'] == 'event' ? ($request['event_days'] ?: null) : null,
            'event_industry_brands' => $request['product_type'] == 'event' ? ($request['event_industry_brands'] ?: null) : null,
            'event_speakers_count' => $request['product_type'] == 'event' ? ($request['event_speakers_count'] ?: null) : null,
            'event_audience' => $request['product_type'] == 'event' ? ($request['event_audience'] ?: null) : null,
            'brochure' => $brochure,
            'brochure_storage_type' => $brochureStorageType,
            'status' => $addedBy == 'admin' ? 1 : (getWebConfig(name: 'new_product_approval') == 1 ? 0 : 1),
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        $legacyCategoryColumns = self::resolveLegacyCategoryColumns($selectedCategories);
        $isEventOrBroker = in_array($request['product_type'], ['event', 'broker']);
        // Preserve a legacy not_sellable flag when the type isn't event/broker, rather than silently un-flagging it.
        $notSellable = $isEventOrBroker ? true : (bool) $product->not_sellable;

        $brochure = $product['brochure'];
        $brochureStorageType = $product['brochure_storage_type'];
        if ($isEventOrBroker && $request->hasFile('brochure')) {
            $brochure = $this->update(dir: 'product/brochure/', oldImage: $product['brochure'], format: $request['brochure']->getClientOriginalExtension(), image: $request['brochure'], fileType: 'file');
            $brochureStorageType = $storage;
        } elseif (!$isEventOrBroker) {
            $brochure = null;
            $brochureStorageType = null;
        }
        ...
        $dataArray = [
            ...
            'purchase_price' => 0,
            'tax' => $isEventOrBroker ? 0 : ($request['tax_type'] == 'flat' ? currencyConverter(amount: $request['tax']) : $request['tax']),
            'tax_type' => $isEventOrBroker ? 'percent' : $request['tax_type'],
            'tax_model' => $isEventOrBroker ? 'exclude' : $request['tax_model'],
            'discount' => $isEventOrBroker ? 0 : ($request['discount_type'] == 'flat' ? currencyConverter(amount: $request['discount']) : $request['discount']),
            'discount_type' => $isEventOrBroker ? 'flat' : $request['discount_type'],
            'attributes' => $request['product_type'] == 'physical' ? json_encode($request['choice_attributes']) : json_encode([]),
        ...
        foreach ($combinations as $combination) {
            ...
            foreach ($combination as $combinationKey => $item) {
                if ($combinationKey > 0) {
                    ...
                    if ($hasColors) {
                        $type .= optional($colorMap->get($item))->name ?? $item;
                    } else {
                        $type .= str_replace(' ', '', $item);
                    }
                }
    ...
    public function getProductSEOData(object $request, object|null $product = null, string $action = null): array
    {
        if ($product) {
            if ($request->file('meta_image')) {
                ...
                // The thumbnail was already uploaded (and encoded) for this same
                // request in getAddProductData() — copy that stored file instead of
                // decoding/re-encoding the original upload a second time.
                $metaImage = $product['thumbnail']
                    ? $this->duplicateStoredImage(sourceDir: 'product/thumbnail/', sourceFileName: $product['thumbnail'], destDir: 'product/meta/')
                    : $this->upload(dir: 'product/meta/', format: 'webp', image: $request['image']);
            } else {
