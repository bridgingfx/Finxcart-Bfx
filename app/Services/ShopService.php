...
class ShopService
{
    ...
    {
        $storage = config('filesystems.disks.default') ?? 'public';
        $shopName = $request['company_name'] ?? $request['shop_name'] ?? $request['vendor_name'] ?? $request['f_name'];
        $shopAddress = $request['company_address'] ?? $request['shop_address'] ?? $request['address'] ?? '';
        $shopContact = $request['company_phone'] ?? $request['phone'] ?? '';
        $shopEmail = $request['company_email'] ?? null;
        return [
            'seller_id' => $vendorId,
            'name' => $shopName,
            ...
            'contact' => $shopContact,
            'email' => $shopEmail,
            'image' => $request->hasFile('logo') ? $this->upload(dir: 'shop/', format: 'webp', image: $request->file('logo')) : 'def.png',
            'image_storage_type' => $request->hasFile('logo') ? $storage : null,
            'banner' => $request->hasFile('banner') ? $this->upload(dir: 'shop/banner/', format: 'webp', image: $request->file('banner')) : 'def.png',
            'banner_storage_type' => $request->hasFile('banner') ? $storage : null,
            'bottom_banner' => $request->hasFile('bottom_banner') ? $this->upload(dir: 'shop/banner/', format: 'webp', image: $request->file('bottom_banner')) : 'def.png',
            'bottom_banner_storage_type' => $request->hasFile('bottom_banner') ? $storage : null,
        ];
    }
