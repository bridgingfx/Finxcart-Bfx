...
class EmailTemplateService
{
    ...
    public function getHiddenField(string $userType, string $templateName):array|null
    {
        ...
        $hiddenData =  [
            ...
            EmailTemplateKey::VENDOR_VERIFICATION_SUBMITTED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::VENDOR_VERIFICATION_APPROVED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::VENDOR_VERIFICATION_DENIED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::VENDOR_REGISTRATION_OTP=>array('product_information','order_information','button_content','banner_image'),
            ...
            EmailTemplateKey::NEW_CHAT_MESSAGE=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::NEW_CHAT_MESSAGE_CUSTOMER=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::NEW_QUOTE_REQUEST=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::QUOTE_REQUEST_REPLIED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::QUOTE_REQUEST_ACCEPTED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::QUOTE_REQUEST_DECLINED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::ORDER_STATUS_UPDATE=>array('product_information','button_content','banner_image'),
    ...
    {
        $image = $request['image'] ? $this->update(dir:'email-template/', oldImage: $template['image'], format: $this->getOriginalImageFormat($request->file('image')),image:  $request->file('image')) : $template['image'];
        $logo = $request['logo'] ? $this->update(dir:'email-template/', oldImage: $template['logo'], format: $this->getOriginalImageFormat($request->file('logo')),image:  $request->file('logo')) : $template['logo'];
        $icon = $request['icon'] ? $this->update(dir:'email-template/', oldImage: $template['image'], format: $this->getOriginalImageFormat($request->file('icon')),image:  $request->file('icon')) : $template['image'];
        $bannerImage = $request['banner_image'] ? $this->update(dir: 'email-template/',oldImage:  $template['banner_image'], format: $this->getOriginalImageFormat($request->file('banner_image')),image:  $request->file('banner_image')): $template['banner_image'];
        return [
    ...
    /**
     * Preserve the uploaded file's own extension instead of converting to webp.
     */
    private function getOriginalImageFormat($file): string
    {
        $supported = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'];
        $extension = strtolower($file?->getClientOriginalExtension() ?? '');

        return in_array($extension, $supported, true) ? $extension : 'webp';
    }
    ...
    public function getTitleData(string $userType,$templateName):string
    {
        $titleData =  [
            ...
            EmailTemplateKey::VENDOR_VERIFICATION_SUBMITTED=>'Verification Submitted',
            EmailTemplateKey::VENDOR_VERIFICATION_APPROVED=>'Verification Approved',
            EmailTemplateKey::VENDOR_VERIFICATION_DENIED=>'Verification Rejected',
            EmailTemplateKey::VENDOR_REGISTRATION_OTP=>'Vendor Registration OTP',
            ...
            EmailTemplateKey::NEW_CHAT_MESSAGE=>'New Message from {userName}',
            EmailTemplateKey::NEW_CHAT_MESSAGE_CUSTOMER=>'New Message from {vendorName}',
            EmailTemplateKey::NEW_QUOTE_REQUEST=>'New Quote Request from {userName}',
            EmailTemplateKey::QUOTE_REQUEST_REPLIED=>'{vendorName} Replied to Your Quote Request',
            EmailTemplateKey::QUOTE_REQUEST_ACCEPTED=>'{userName} Accepted Your Quote',
            EmailTemplateKey::QUOTE_REQUEST_DECLINED=>'{userName} Declined Your Quote',
            EmailTemplateKey::ORDER_STATUS_UPDATE=>'Order # {orderId} Status Update',
        ];
    ...
    public function getBodyData(string $userType,$templateName):string
    {
        ...
        $bodyData =  [
            EmailTemplateKey::REGISTRATION=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            ...
            EmailTemplateKey::REGISTRATION_APPROVED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::REGISTRATION_DENIED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><div><b>Reason:</b> {rejectionReason}</div></div><div><br></div><div>Please contact with admin or support center if you have any queries.</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            ...
            EmailTemplateKey::VENDOR_VERIFICATION_SUBMITTED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::VENDOR_VERIFICATION_APPROVED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::VENDOR_VERIFICATION_DENIED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>{message}</div><div><div><b>Reason:</b> {rejectionReason}</div></div><div><br></div><div>Please contact with admin or support center if you have any queries.</div><div><br></div><div>Meanwhile, click here to visit the '.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::VENDOR_REGISTRATION_OTP=>'<p><b>Hi '.$userType.',</b></p><p>Your OTP code is <strong>{message}</strong></p><p>This code is valid for 5 minutes.</p>',
            EmailTemplateKey::NEW_CHAT_MESSAGE=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>You have received a new message from <b>{userName}</b>:</div><div><br></div><div><i>"{message}"</i></div><div><br></div><div>Login to your freelancer dashboard to reply.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::NEW_CHAT_MESSAGE_CUSTOMER=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>You have received a new message from <b>{vendorName}</b>:</div><div><br></div><div><i>"{message}"</i></div><div><br></div><div>Login to your account to reply.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::NEW_QUOTE_REQUEST=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>You have received a new quote request from <b>{userName}</b>:</div><div><br></div><div><i>"{message}"</i></div><div><br></div><div>Login to your freelancer dashboard to review the request and send your quote.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::QUOTE_REQUEST_REPLIED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div><b>{vendorName}</b> has replied to your quote request:</div><div><br></div><div><i>"{message}"</i></div><div><br></div><div>Login to your account to view the full reply.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::QUOTE_REQUEST_ACCEPTED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div><b>{userName}</b> has accepted your quote for "{message}".</div><div><br></div><div>Login to your freelancer dashboard to view the details.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::QUOTE_REQUEST_DECLINED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div><b>{userName}</b> has declined your quote for "{message}".</div><div><br></div><div>Login to your freelancer dashboard to view the details.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::ORDER_STATUS_UPDATE=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>Your order # {orderId} status has been updated to: <b>{message}</b></div><div><br></div><div>Login to your account to view the order details.</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
        ];
