...
class NotificationRepository implements NotificationRepositoryInterface
{
    ...
    public function getListWhere(array $orderBy = [], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query =  $this->notification->with($relations)
            ->when(isset($filters['sent_to']),function ($query)use($filters){
                return $query->where('sent_to',$filters['sent_to']);
            })
    ...
    {
        return $this->notification->whereBetween('created_at',$params)->where($filters)->whereDoesntHave($relations)->count();
    }
