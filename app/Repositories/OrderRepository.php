...
class OrderRepository implements OrderRepositoryInterface
{
        ...
        if ($status == 'returned' || $status == 'failed' || $status == 'canceled') {
            foreach ($order['details'] as $detail) {
                if ($detail['is_stock_decreased'] == 1) {
                    ...
                    // product() is scoped to status=1, so a since-deactivated/deleted
                    // product resolves to null here — nothing to restock in that case.
                    if (!$product) {
    ...
    public function manageWalletOnOrderStatusChange(object $order, string $receivedBy): bool
    {
        ...
        if ($order['payment_method'] == 'cash_on_delivery' || $order['payment_method'] == 'offline_payment') {
            ...
            if (!$transaction) {
                $transaction = $this->orderTransaction->create([
                    'transaction_id' => getUniqueId(),
                    'customer_id' => $order['customer_id'],
                    'seller_id' => $order['seller_id'],
                    'seller_is' => $order['seller_is'],
    ...
    public function getTopCustomerList(array $filters = [] ,array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        ...
        return $query->paginate($dataLimit)->appends($filters);
    }

    /**
     * Top N by order count — a lean alternative to getTopCustomerList('all') + ->take(N)
     * in PHP, which fetched every customer's order-count row just to trim it client-side.
     */
    public function getTopCustomerListLimited(array $relations = [], int $limit = DEFAULT_DATA_LIMIT): Collection
    {
        return $this->order
            ->with($relations)
            ->select('customer_id', DB::raw('COUNT(customer_id) as count'))
    ...
    /**
     * Top N vendors by order count — a lean alternative to getTopVendorListByOrderReceived('all')
     * + ->take(N) in PHP, which fetched every seller's order-count row just to trim it client-side.
     */
    public function getTopVendorListByOrderReceivedLimited(array $relations = [], int $limit = DEFAULT_DATA_LIMIT): Collection
    {
        return $this->order
            ->with($relations)
            ->whereHas('seller', function ($query) {
