...
class WithdrawRequestRepository implements WithdrawRequestRepositoryInterface
{
    ...
    {
        $query = $this->withdrawRequest->with($relations)
            ->when(!empty($nullFilters), function ($query) use ($nullFilters) {
                foreach ($nullFilters as $field) {
            ...
            ->when(($filters['approved'] ?? null) == 'approved', function ($query) {
                return $query->where('approved', 1);
            })
            ->when(($filters['approved'] ?? null) == 'denied', function ($query) {
                return $query->where('approved', 2);
            })
            ->when(($filters['approved'] ?? null) == 'pending', function ($query) {
                return $query->where('approved', 0);
            })->when(!empty($orderBy), function ($query) use ($orderBy) {
