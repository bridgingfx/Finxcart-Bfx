...
class CustomerRepository implements CustomerRepositoryInterface
{
    ...
    {
        $identity = trim($filters['identity'] ?? '');
        $normalizedIdentity = preg_replace('/[^0-9]/', '', $identity); // keep digits only

        return $this->user->where(function ($query) use ($filters, $identity, $normalizedIdentity) {
            if (!empty($filters['email'])) {
                $query->orWhere('email', $filters['email']);
            }

            if (!empty($filters['phone'])) {
                $query->orWhere('phone', $filters['phone']);
            }
            ...
            if (!empty($normalizedIdentity)) {
                $query->orWhereRaw("REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(phone, '+', ''), ' ', ''), '-', ''), '(', ''), ')', '') = ?", [$normalizedIdentity]);
            }
        })->first();
    }
    ...
    {
        // Callers only ever render {id, f_name, l_name, phone} into a selection
        // dropdown (coupon "assign to customer" picker, POS customer picker) —
        // selecting just those columns instead of the full users row avoids
        // pulling every customer's address/token/image data on every page load.
        return $this->user->whereNotIn('id', $ids)->select(['id', 'f_name', 'l_name', 'phone'])->get();
    }
    ...
    public function getCustomerNameList(object $request, int|string $dataLimit = DEFAULT_DATA_LIMIT): object
    {
        ...
        return $this->user->where('id', '!=', 0)
            ...
            // This result only selects id/text via raw SQL, but User::$appends still triggers the
            // image_full_url accessor per row on ->toArray(), which lazy-loads the `storage` relation
            // once per row (N+1). Hide the append here since this list never renders an image.
