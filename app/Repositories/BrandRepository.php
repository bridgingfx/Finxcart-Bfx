...
class BrandRepository implements BrandRepositoryInterface
{
    ...
    public function getListWhere(array $orderBy=[], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null, bool $lightweight = false): Collection|LengthAwarePaginator
    {
        $query = $this->brand
            // $lightweight skips the brandAllProducts eager-load (+ its per-product
            // orderDetails count) for callers that only need id/name — e.g. populating
            // a <select> on the product add/edit form, where this was dead weight.
            ->when(!$lightweight, function ($query) {
                $query->withCount('brandAllProducts')->with(['brandAllProducts'=> function($query){
