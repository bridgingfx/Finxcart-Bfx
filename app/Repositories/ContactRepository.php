...
class ContactRepository implements ContactRepositoryInterface
{
    ...
    public function getListWhere(array $orderBy=[], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        ...
        // Preserve reply parameter in pagination links even when it's 'all'
        // The filter is already in $filters array from the controller
        $paginationParams = $filters;
        $paginationParams['searchValue'] = $searchValue;
        
        return $dataLimit == 'all' ? $query->get() : $query->paginate($dataLimit)->appends($paginationParams);
    }
