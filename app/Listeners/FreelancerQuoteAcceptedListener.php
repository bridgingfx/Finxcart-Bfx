...
class FreelancerQuoteAcceptedListener
{
    use EmailTemplateTrait, PushNotificationTrait;

    public function handle(FreelancerQuoteAcceptedEvent $event): void
    {
        ...
        try {
            ...
            $message = 'Your quote for "' . $quote->service?->title . '" was accepted.';
            ...
            Log::error('[FreelancerQuoteAcceptedListener] Notification failed: ' . $e->getMessage());
        }

        try {
            AdminNotification::create([
                'type' => 'freelancer_quote_accepted',
                'title' => 'Quote Request Accepted',
                'message' => "{$customerName} accepted a quote for \"{$quote->service?->title}\".",
                'link' => route('admin.freelancer.accounts.index'),
                'reference_id' => $quote->id,
            ]);
        } catch (\Throwable $e) {
            Log::error('[FreelancerQuoteAcceptedListener] Admin notification failed: ' . $e->getMessage());
        }
        ...
        dispatch(function () use ($freelancer, $customerName, $quote) {
            try {
                ...
                $this->sendingMail(
                    ...
                    userType: 'vendor',
                    templateName: EmailTemplateKey::QUOTE_REQUEST_ACCEPTED,
                    data: [
                ...
                Log::error('[FreelancerQuoteAcceptedListener] Email failed: ' . $e->getMessage());
            }
        })->afterResponse();
    }
}
