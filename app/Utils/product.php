...
if (!function_exists('getVendorProductsCount')) {
    /**
     * Backs 4 sidebar badges rendered on every admin page. Previously each call ran its own
     * `DB::table('products')->where('added_by','seller')->get()` — a full-table pull into PHP
     * memory, 4 times per page load. Replaced with one cached, single-pass aggregate query
     * covering all 4 counts, so only the first call per request (per cache TTL) hits the DB.
     */
    function getVendorProductsCount(string $type): int
    {
        $counts = \Illuminate\Support\Facades\Cache::remember('admin_sidebar_vendor_product_counts', 300, function () {
            $row = \Illuminate\Support\Facades\DB::table('products')
                ->where('added_by', 'seller')
                ->selectRaw("
                    SUM(CASE WHEN request_status = 0 THEN 1 ELSE 0 END) as new_product,
                    SUM(CASE WHEN is_shipping_cost_updated IS NOT NULL AND is_shipping_cost_updated = 0 THEN 1 ELSE 0 END) as product_updated_request,
                    SUM(CASE WHEN request_status = 1 THEN 1 ELSE 0 END) as approved,
                    SUM(CASE WHEN request_status = 2 AND status = 0 THEN 1 ELSE 0 END) as denied
                ")
                ->first();

            return [
                'new-product' => (int) ($row->new_product ?? 0),
                'product-updated-request' => (int) ($row->product_updated_request ?? 0),
...
if (!function_exists('getAdminProductsCount')) {
    /**
     * Same fix as getVendorProductsCount() above, for admin-added products (5th sidebar call,
     * including 'all' which previously loaded the whole table just to call ->count() on it).
     */
    function getAdminProductsCount(string $type): int
    {
        $counts = \Illuminate\Support\Facades\Cache::remember('admin_sidebar_admin_product_counts', 300, function () {
            $row = \Illuminate\Support\Facades\DB::table('products')
