...
class OrderManager
{
    ...
    {
        // INV-01 FIX: stock changes run inside a transaction with a row lock
        // (lockForUpdate) so concurrent orders cannot oversell or corrupt variation
        // quantities. Stock is also clamped so it can never go negative.
        // Batched into a single transaction + single lockForUpdate query across all
        // line items (was one transaction and one locked SELECT per line item).
        $restock = in_array($status, ['returned', 'failed', 'canceled']);

        $detailsToProcess = collect($order->details)->filter(function ($detail) use ($restock) {
            return $restock ? $detail['is_stock_decreased'] == 1 : $detail['is_stock_decreased'] == 0;
        });
        ...
        DB::transaction(function () use ($detailsToProcess, $status, $restock) {
            $productIds = $detailsToProcess->pluck('product_id')->unique()->values();
            ...
            foreach ($detailsToProcess as $detail) {
                ...
                foreach (json_decode($product['variation'], true) ?? [] as $var) {
                    if ($type == $var['type']) {
                        $var['qty'] = $restock ? $var['qty'] + $qty : max(0, $var['qty'] - $qty);
                    }
                    $var_store[] = $var;
                }
                ...
                $new_stock = $restock
                    ...
                    : max(0, $product->current_stock - $qty);
                ...
                OrderDetail::where('id', $detail['id'])->update([
                    'is_stock_decreased' => $restock ? 0 : 1,
                    'delivery_status' => $status,
                ]);

                // Keep the in-memory model in sync so a second line item
                // referencing the same product applies on top of this update
                // rather than the stale pre-loop value.
                $product->variation = json_encode($var_store);
    ...
    {
        // FIN-01 FIX: all admin/seller wallet mutations for a status change now run
        // inside a single transaction with a row lock on the order, so balances can
        // never be left half-updated and concurrent status changes are serialized.
        DB::transaction(function () use ($order, $received_by) {
        ...
        self::recordCommissionLedgerForOrder($order, (float) $order_amount, (float) $commission);
        });
    }

    private static function recordCommissionLedgerForOrder(Order $order, float $grossAmount, float $commission): void
    {
        if ($order->seller_is !== 'seller' || !$order->seller_id || $commission <= 0) {
            return;
        }
        ...
        try {
            ...
            $commissionService->recordCommissionLedger(
                ...
                serviceCharge: $commissionService->getConfiguredServiceCharge(),
                orderId: (int) $order->id,
                currency: 'USD',
                referenceType: 'order',
                referenceId: $orderTransaction?->id,
                companyShareAmount: $commission,
                updateWallets: false,
            );
        } catch (\Throwable $e) {
            \Log::error('[OrderManager] Commission ledger record failed: ' . $e->getMessage(), [
                'order_id' => $order->id,
                'seller_id' => $order->seller_id,
            ]);
        }
    ...
    /**
     * Reserves the next order id atomically and returns it.
     *
     * Just computing MAX(id)+1 (the old approach, still visible in git
     * history) isn't enough on its own, even behind a lock: the id it hands
     * back isn't actually claimed in the orders table until the caller's own
     * insert runs, which — in generate_order()/create_parent_order() — is
     * many lines later, after coupon/shipping/address lookups etc. Two
     * requests can each "reserve" the same number in that gap (lock
     * acquired, number read, lock released, second request repeats before
     * either has inserted), so the *reservation* has to be the insert: this
     * inserts a minimal placeholder Order row (id = MAX+1, everything else
     * default/null) while still holding the lock, so MySQL's own primary-key
     * uniqueness is what actually prevents the collision, not just the lock.
     * The caller's later `orders` write (insertGetId with the same id, or an
     * UPDATE) fills in the real columns on this same row.
     */
    private static function reserveNextOrderId(): int
    {
        DB::select('SELECT GET_LOCK(?, 10)', ['finxcart_next_order_id']);
        try {
            $nextId = max(100000, (int) Order::max('id'), (int) OrderDetail::max('order_id')) + 1;
            DB::table('orders')->insert(['id' => $nextId, 'created_at' => now(), 'updated_at' => now()]);
            return $nextId;
        } finally {
            DB::select('SELECT RELEASE_LOCK(?)', ['finxcart_next_order_id']);
        }
    }
...
public static function generate_order($data)
{
    ...
    $order_id = self::reserveNextOrderId();
    ...
    // confirmed — reserveNextOrderId() already inserted a placeholder row
    // for this id, so fill it in rather than inserting a second row.
    DB::table('orders')->where('id', $order_id)->update(array_diff_key($or, ['id' => null]));
    ...
    foreach (CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked', customerId: $getCustomerID) as $cartSingleItem) {
        ...
        $orderDetails = [
            ...
            'product_type' => $cartSingleItem['product_type'] ?? null,
            'digital_delivery_type' => ($cartSingleItem['product_type'] ?? null) === 'digital' ? (session('digital_delivery_type') ?? ($req['digital_delivery_type'] ?? null)) : null,
            'digital_delivery_value' => ($cartSingleItem['product_type'] ?? null) === 'digital' ? (session('digital_delivery_value') ?? ($req['digital_delivery_value'] ?? null)) : null,
            'created_at' => now(),
            'updated_at' => now()
        ];

        // Both branches write to the same product row — collapsed into a single
        // UPDATE instead of two separate round-trips per line item.
        $productUpdateData = [
        ...
        if ($cartSingleItem['variant'] != null) {
            ...
            $productUpdateData['variation'] = json_encode($var_store);
        }
    ...
    app(OrderService::class)->createShipmentsForOrder($order, $order->details);
    ...
    // ======================================================
    // NEW: DYNAMIC ADMIN COMMISSION CALCULATION + LOGS
    // (calculates per-product commission and updates orders & transactions)
    // ======================================================
    try {
        // reload order to ensure fresh relations
        $order = Order::with('details')->find($order_id);
        $order_summary = OrderManager::order_summary($order);

        $order_amount_for_calc = $order_summary['subtotal']
                                 - $order_summary['total_discount_on_product']
                                 - $order['discount_amount'];

        \Log::info('Commission Calculation Started (generate_order)', [
            'order_id' => $order_id,
            'seller_id' => $order->seller_id,
            'order_amount_for_calc' => $order_amount_for_calc,
        ]);

        $total_admin_commission = 0;

        // fetch details for this order and seller
        ...
        foreach ($orderDetails as $detail) {
            $commissionRate = getVendorCommissionRate(
                $detail->order_id,
                $detail->seller_id,
                $detail->product_id
            );

            $product_price = ($detail->price * $detail->qty) - $detail->discount;
            $product_commission = $product_price * $commissionRate;

            $total_admin_commission += $product_commission;
        ...
        \Log::info('Commission Breakdown (generate_order)', [
            'order_id' => $order_id,
            'total_admin_commission' => $total_admin_commission,
            'breakdown' => $breakdown,
        ]);

        $commission = $total_admin_commission;

        // update orders table
        \Log::info('Updating orders.admin_commission (generate_order)', [
            'order_id' => $order_id,
            'admin_commission' => $commission,
        ]);
        ...
        \Log::info('orders table updated (generate_order)', [
            'order_id' => $order_id,
            'admin_commission' => $commission
        ]);

        // update corresponding order_transactions row (if exists)
        \Log::info('Updating order_transactions (generate_order)', [
            'order_id' => $order_id,
            'admin_commission' => $commission,
            'seller_amount_calculated' => ($order_amount_for_calc - $commission)
        ]);
        ...
        DB::table('order_transactions')
            ...
            ->update([
                'admin_commission' => $commission,
                'seller_amount' => ($order_amount_for_calc - $commission)
            ]);

        \Log::info('order_transactions updated (generate_order)', [
            'order_id' => $order_id
        ]);
    } catch (\Exception $e) {
        \Log::error('Commission calculation/update failed (generate_order)', [
            'order_id' => $order_id,
            'error' => $e->getMessage()
        ]);
    }
    ...
    try {
        ...
        if ($emailServices_smtp['status'] == 0) {
            $emailServices_smtp = getWebConfig(name: 'mail_config_brevo');
        }
        if ($emailServices_smtp['status'] == 1) {
            ...
            $childNum = $data['child_number'] ?? null;
            $isChildOrder = $parentId !== null;
            if (!$isChildOrder) {
                $customerData = [
                    ...
                    'order' => $order,
                    'orderId' => $parentId ?? $order_id,
                    'shopName' => $seller?->shop?->name ?? getWebConfig('company_name'),
                    'shopId' => $seller?->shop?->id ?? 0,
                    // The mPDF invoice render used to happen here, synchronously, before
                    // the response — negating the point of deferring the mail send below.
                    // It's generated instead inside OrderPlacedListener's already-deferred
                    // closure, right before the mail actually goes out.
                    'invoiceOrderId' => $order_id,
                ];
                event(new OrderPlacedEvent(email: $email, data: $customerData));
            }
            $dataForVendor = [
...
public static function create_parent_order(array $data, $req = null): int
{
    $user = Helpers::getCustomerInformation($req);
    $getCustomerID = $user ? $user->id : (session('guest_id') ?? null);
    $isGuestUserInOrder = $user == null ? 1 : 0;
    $order_id = self::reserveNextOrderId();
    $address_id = session('address_id') ?? null;
    $billing_address_id = session('billing_address_id') ?? null;

    // reserveNextOrderId() already inserted a placeholder row for this id,
    // so fill it in rather than inserting a second row.
...
public static function send_parent_order_customer_email(int $parentOrderId): void
{
    try {
        $parentOrder = Order::with([
            ...
            'shippingAddress',
            'billingAddress',
            'children.details.productAllStatus',
        ])->find($parentOrderId);
        ...
        $combinedDetails = $parentOrder->children->flatMap(fn($c) => $c->details)->values();
        $parentOrder->shipping_cost = $parentOrder->children->sum('shipping_cost');
        $parentOrder->discount_amount = $parentOrder->children->sum('discount_amount');
        $parentOrder->is_shipping_free = $parentOrder->children->every(fn($c) => (bool) $c->is_shipping_free) ? 1 : 0;

        $firstChild = $parentOrder->children->sortBy('child_number')->first();

        $customerData = [
            'subject'        => translate('order_placed'),
            'title'          => translate('order_placed'),
            'userName'       => $parentOrder->customer->f_name,
            'userType'       => 'customer',
            'templateName'   => 'order-place',
            'order'          => $parentOrder,
            'orderDetails'   => $combinedDetails,
            'orderId'        => $parentOrderId,
            'shopName'       => getWebConfig('company_name'),
            'shopId'         => 0,
            // Generated inside OrderPlacedListener's deferred closure instead of here,
            // so checkout doesn't block on an mPDF render before the response.
            'invoiceOrderId' => $firstChild?->id,
        ];
        event(new OrderPlacedEvent(email: $parentOrder->customer->email, data: $customerData));
    } catch (\Exception $e) {
        \Log::error('send_parent_order_customer_email failed', ['parent_order_id' => $parentOrderId, 'error' => $e->getMessage()]);
    }
}
    ...
    public static function updated_generate_order($data): int
    {
        ...
        // ORD-01 FIX: never load every order into memory with Order::all()->count().
        // Use a DB-side MAX(id) (fast, index-backed) with a floor of 100000 —
        // reserved atomically via a named lock so two concurrent checkouts can
        // never be handed the same id (see reserveNextOrderId()).
        $order_id = self::reserveNextOrderId();
        $billing_address_id = session('billing_address_id') ?? null;
        $coupon_code = $couponProcess['coupon_code'];
        $coupon_bearer = $couponProcess['coupon_bearer'];
        ...
        try {
            ...
            if ($emailServices_smtp['status'] == 1) {
                $data = [
                    ...
                    'shopName' => $seller?->shop?->name ?? getWebConfig('company_name'),
                    'shopId' => $seller?->shop?->id ?? 0,
                    // Generated inside OrderPlacedListener's deferred closure instead of
                    // here, so this order-placement path doesn't block on an mPDF render.
                    'invoiceOrderId' => $order_id,
                ];
  ...
  public static function order_insert($order_data): int
{
    ...
    $or = [
        ...
        'admin_commission' => $order_data['admin_commission'], // temporary will be replaced later
        ...
        'parent_order_id' => $order_data['data']['parent_order_id'] ?? null,
        'child_number' => $order_data['data']['child_number'] ?? null,
        'created_at' => now(),
        'updated_at' => now(),
    ...
    // reserveNextOrderId() already inserted a placeholder row for this id,
    // so fill it in rather than inserting a second row.
    DB::table('orders')->where('id', $order_data['order_id'])->update(array_diff_key($or, ['id' => null]));
    ...
    self::add_order_status_history(
        $order_data['order_id'],
        auth('customer')->id(),
        $order_data['data']['payment_status'] == 'paid' ? 'confirmed' : 'pending',
        'customer'
    );
    ...
    // ===========================
    OrderDetail::where('order_id', $order_data['order_id'])->delete();
    foreach (CartManager::getCartListQuery(groupId: $order_data['data']['cart_group_id'], type: 'checked', customerId: $order_data['user_id']) as $c) {
        ...
        if ($c['variant'] != null) {
            ...
            Product::where(['id' => $product['id']])
                ->update(['variation' => json_encode($var_store)]);
        }
    ...
    // ======================================================
    // CREATE TRANSACTION (only if NOT COD or offline)
    // ======================================================
    if ($or['payment_method'] != 'cash_on_delivery' && $or['payment_method'] != 'offline_payment') {
        ...
        $order_amount = $order_summary['subtotal']
                        - $order_summary['total_discount_on_product']
                        - $order['discount'];
        ...
        // LOG
        \Log::info('Commission Calculation Started', [
            'order_id' => $order->id,
            'seller_id' => $order->seller_id,
            'order_amount' => $order_amount,
        ]);
        ...
        // UPDATE ORDER admin_commission
        \Log::info('Updating Order with Commission', [
            'order_id' => $order->id,
            'admin_commission' => $commission
        ]);
        ...
        \Log::info('Order Table Updated Successfully', [
            'order_id' => $order->id,
            'new_admin_commission' => $commission
        ]);

        // UPDATE ORDER TRANSACTION
        \Log::info('Updating Order Transactions', [
            'order_id' => $order->id,
            'admin_commission' => $commission,
            'seller_amount' => $order_amount - $commission
        ]);
