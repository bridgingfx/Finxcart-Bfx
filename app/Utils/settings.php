...
if (!function_exists('clearWebConfigCacheKeys')) {
    function clearWebConfigCacheKeys(bool $force = false): bool
    {
        // A single settings-form submission can call this dozens of times (once per
        // field via BusinessSettingRepository::updateOrInsert/update/updateWhere).
        // Each call was re-running the full forget/query/re-populate cascade, so a
        // 15-field form save meant 15x the cache churn. Collapse to once per request.
        static $alreadyCleared = false;
        if ($alreadyCleared && !$force) {
            return true;
        }
        $alreadyCleared = true;
...
if (!function_exists('getCustomerFromQuery')) {
    ...
    {
        return auth('customer')->check() ? auth('customer')->user() : null;
    }
}
...
if (!function_exists('cacheRemoveByType')) {
    function cacheRemoveByType(string $type): void
    {
        if ($type == 'business_settings') {
            ...
            Cache::forget(CACHE_FOR_ACTIVE_CURRENCY_LIST);
            ...
            Cache::forget(CACHE_HOME_FULL_CATEGORY_STRUCTURE);
            Cache::forget(CACHE_HOME_CATEGORY_SECTION_PRODUCTS);
            Cache::forget(CACHE_HOME_MARKETPLACE_CATEGORY_PRODUCTS);
            Cache::forget(CACHE_CATEGORY_WITH_PRODUCTS . '_default');
            Cache::forget(CACHE_HOME_SOLUTIONS_CATEGORY_NAMES);
            Cache::forget(CACHE_HOME_SOLUTIONS_SECTIONS);
            Cache::forget(CACHE_HOME_DEAL_OF_THE_DAY);
            Cache::forget(CACHE_HOME_TOP_FEATURED_PRODUCTS);
            ...
            Cache::forget(CACHE_FOR_HOME_PAGE_LATEST_PRODUCT_COUNT);
            Cache::forget(CACHE_FOR_HOME_PAGE_LATEST_PRODUCT_IDS);
            ...
            // Admin sidebar product-count badges (getVendorProductsCount()/getAdminProductsCount()
            // in app/Utils/product.php) — keep them fresh on every add/update instead of waiting
            // out the 5-minute TTL.
            Cache::forget('admin_sidebar_vendor_product_counts');
            Cache::forget('admin_sidebar_admin_product_counts');
