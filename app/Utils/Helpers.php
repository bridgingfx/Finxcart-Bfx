...
class Helpers
{
    ...
    public static function set_data_format($data, $colorMap = null)
    {
        $colors = is_array($data['colors']) ? $data['colors'] : json_decode($data['colors']);
        $query_data = $colorMap !== null ? array_intersect_key($colorMap, array_flip($colors ?: [])) : Color::whereIn('code', $colors)->pluck('name', 'code')->toArray();
        $color_process = [];
...
//        $data['color_image'] = $colorImage;
    ...
    public static function set_data_format_for_json_data($data, $colorMap = null)
    {
        $colors = is_array($data['colors']) ? $data['colors'] : json_decode($data['colors']);
        $query_data = $colorMap !== null ? array_intersect_key($colorMap, array_flip($colors ?: [])) : Color::whereIn('code', $colors)->pluck('name', 'code')->toArray();
        $color_process = [];
        ...
        if ($data) {
            ...
            if ($multi_data == true) {
                ...
                foreach ($data as $item) {
                    if ($item) {
                        $itemColors = is_array($item['colors']) ? $item['colors'] : json_decode($item['colors']);
                        if ($itemColors) {
                            $allColorCodes = array_merge($allColorCodes, $itemColors);
                        }
                    }
                }
                $colorMap = $allColorCodes ? Color::whereIn('code', array_unique($allColorCodes))->pluck('name', 'code')->toArray() : [];
                foreach ($data as $item) {
                    if ($item) {
                        $storage[] = Helpers::set_data_format($item, $colorMap);
                    }
                }
                $data = $storage;
            } else {
    ...
    public static function product_data_formatting_for_json_data($data, $multi_data = false)
    {
        if ($data) {
            ...
            if ($multi_data == true) {
                ...
                foreach ($data as $item) {
                    if ($item) {
                        $storage[] = Helpers::set_data_format_for_json_data($item, $colorMap);
                    }
                }
                $data = $storage;
            } else {
    ...
    public static function getDefaultPaymentGateways(): array
    {
        return [
            ...
            'flutterwave', 'paytm', 'paytabs', 'liqpay', 'mercadopago', 'bkash','now_payments'
        ];
    }
...
//        return (($price-$discount) / 100) * $tax; //after discount decrease
    ...
    public static function sales_commission($order)
    {
        ...
        // FIN-02 FIX: commission must be calculated on the line-item money value,
        // not on the bare rate. getVendorCommissionRate() returns a DECIMAL rate
        // (e.g. 0.10 for 10%), so commission = (unit_price * qty - line_discount) * rate.
        foreach ($order->details as $detail) {

            $seller_id = $order['seller_id'];
            if (isset($detail['seller_id']) && $detail['seller_id'] !== $seller_id) {
                $seller_id = $detail['seller_id'];
            }

            $line_amount = ((float)$detail['price'] * (int)$detail['qty']) - (float)$detail['discount'];
            if ($line_amount < 0) {
                $line_amount = 0;
            }

            $commission_rate = getVendorCommissionRate($order->id, $seller_id, $detail['product_id']);

            if ($commission_rate > 0) {
                // Tier-based commission rate found for this product.
                $item_commission = $line_amount * $commission_rate;
            } else {
                // FIN-03 FIX: fall back to the seller/global percentage commission
                // so orders are never silently charged zero commission.
                $item_commission = (float)str_replace(',', '', self::seller_sales_commission(
        ...
        try {
        // Assuming $order is a mutable Eloquent model instance
        $order->admin_commission = round($total_commission, 2);
        $order->save();
        
        } catch (\Exception $e) {
            Log::error("Failed to update admin_commission for Order ID {$order->id}. Error: {$e->getMessage()}");
            // Depending on requirements, you might re-throw the exception or return 0.
        }
...
if (!function_exists('getVendorCommissionRate')) {
    /**
     * Calculates the commission rate for a specific product based on the vendor's active tier status.
     *
     * @param int $orderId
     * @param int $sellerId
     * @param int $productId
     * @return float The final commission rate (as a percentage).
     */
function getVendorCommissionRate(int $orderId, int $sellerId, int $productId): float
{
    // 1. Load product
    $product = Product::find($productId);

    if (!$product) {
        Log::warning("Commission failed: Product {$productId} not found.");
        return 0;
    }

    // If no vendor tier assigned → commission is 0
    if (!$product->vendor_tier_id) {
        Log::info("Commission: Product {$productId} has no vendor_tier_id → commission = 0");
        return 0;
    }

    // 2. Load tier using vendor_tier_id
    $vendorTier = VendorTier::find($product->vendor_tier_id);

    if (!$vendorTier) {
        Log::warning("Commission: VendorTier {$product->vendor_tier_id} not found → commission = 0");
        return 0;
    }

    $today = now();
    $trialEnd = $vendorTier->trial_end_date ? Carbon::parse($vendorTier->trial_end_date) : null;

    // 3. If inside trial → use trial rate
    if ($trialEnd && $today->lte($trialEnd) && $vendorTier->trial_commission_rate > 0) {

        $rateDecimal = ((float)$vendorTier->trial_commission_rate) / 100;

        Log::info("Commission: Using TRIAL rate for tier {$vendorTier->id} → {$rateDecimal}");

        return $rateDecimal;
    }

    // 4. Otherwise use the normal sales commission
    $rateDecimal = ((float)$vendorTier->sales_commission_rate) / 100;

    Log::info("Commission: Using SALES rate for tier {$vendorTier->id} → {$rateDecimal}");
