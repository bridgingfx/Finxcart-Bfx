...
class CartManager
{
    ...
    public static function getCartListQuery($groupId = null, $type = null, $customerId = null)
    {
        ...
        return Cart::with(['product' => function ($query) {
            ...
            ->when($groupId, function ($query) use ($groupId, $customerId) {
                $query->where('cart_group_id', $groupId);
                if ($customerId !== null) {
                    ...
                    } elseif (session()->has('guest_id') || request()->has('guest_id')) {
                        $query->where(['customer_id' => session('guest_id') ?? request('guest_id'), 'is_guest' => 1]);
                    }
                }
                return $query;
            })
    ...
    {
        return self::getCartListQuery()?->count() ?? 0;
    }

    public static function getCartListGroupQuery($groupId = null, $type = null, $customerId = null)
    {
        return Cart::with([
            ...
            // The cart-details view separately reads $cartItem->allProducts->seller->shop
            // (seller-approval/shop-closure/vacation checks) for every row, unrelated to
            // the filtered "product" relation above — without this it's a 3-relation-deep
            // N+1 per cart item, doubled since the view renders both a desktop and a
            // mobile pass over the same collection.
            'allProducts.seller.shop',
        ])
            ->whereHas('product', function ($query) {
                return $query->active();
            })
    ...
    public static function get_cart_for_api($request, $groupId = null, $type = null)
    {
        return Cart::when(($groupId == null && $type != 'checked'), function ($query) use ($request) {
            ...
            ->when($groupId, function ($query) use ($groupId, $request) {
                $query->where('cart_group_id', $groupId);
                $user = Helpers::getCustomerInformation($request);
                if ($user != 'offline') {
                    $query->where(['customer_id' => $user->id, 'is_guest' => 0]);
                } elseif (!empty($request['guest_id'])) {
    ...
    public static function get_cart_group_ids($request = null, $type = null)
    {
        ...
        self::consolidateCartGroupsBySeller($user);
    ...
    /**
     * get_cart_group_ids() is called many times per request (checkout validation alone
     * calls it twice back-to-back, and it's invoked from most CartManager methods), and
     * each call previously re-ran a full Cart::where(...)->get() consolidation pass.
     * The consolidation is idempotent for a given customer identity within a single
     * request (nothing merges carts mid-request outside of add-to-cart, which already
     * resolves the correct cart_group_id itself), so memoize it per (customerId,isGuest)
     * the same way ProductManager::$hasRepricedCartThisRequest memoizes cart repricing.
     * Keyed rather than a single blind flag so it still runs once for a new identity if
     * the request's auth context changes (e.g. around login).
     */
    private static array $consolidatedCartGroupsForIdentity = [];

    /**
     * Guarantee "one shipment per vendor" regardless of how the cart got fragmented
     * (e.g. guest_id/session drift between add-to-cart calls). If the same seller's
     * items ended up spread across more than one cart_group_id, merge them all onto
     * the first group id before checkout groups the cart into orders.
     */
    private static function consolidateCartGroupsBySeller($user): void
    {
        $customerId = $user == 'offline' ? (session('guest_id') ?? (request('guest_id') ?? 0)) : $user->id;
        $isGuest = $user == 'offline' ? 1 : 0;

        $identityKey = $customerId . '|' . $isGuest;
        if (isset(self::$consolidatedCartGroupsForIdentity[$identityKey])) {
            return;
        }
        self::$consolidatedCartGroupsForIdentity[$identityKey] = true;

        $carts = Cart::where(['customer_id' => $customerId, 'is_guest' => $isGuest])->get();

        foreach ($carts->groupBy(fn ($cart) => $cart->seller_id . '|' . $cart->seller_is) as $sellerCarts) {
            $canonicalGroupId = $sellerCarts->first()->cart_group_id;
            $rowsToMerge = $sellerCarts->where('cart_group_id', '!=', $canonicalGroupId);

            if ($rowsToMerge->isEmpty()) {
                continue;
            }

            $staleGroupIds = $rowsToMerge->pluck('cart_group_id')->unique();
            Cart::whereIn('id', $rowsToMerge->pluck('id'))->update(['cart_group_id' => $canonicalGroupId]);
            CartShipping::whereIn('cart_group_id', $staleGroupIds)->delete();
        }
    }
    ...
    {
        $user = Helpers::getCustomerInformation($request);
        $customerId = $user != 'offline' ? $user->id : (session('guest_id') ?? request('guest_id') ?? 0);
        $cartGroupIDs = CartManager::get_cart_group_ids(request: $request, type: 'checked');
        self::cartCleanByCartGroupIds(cartGroupIDs: $cartGroupIDs, customerId: $customerId);
    }

    public static function cartCleanByCartGroupIds($cartGroupIDs, $customerId = null): void
    {
        CartShipping::whereIn('cart_group_id', $cartGroupIDs)->delete();
        Cart::whereIn('cart_group_id', $cartGroupIDs)
    ...
    public static function addToCartPhysicalProduct($request, $product, $shippingType, $sellerShippingList): array
    {
        ...
        if ($minimumOrderQty > $request['quantity']) {
            return ['status' => 0, 'message' => translate('Minimum_order_quantity') . ' ' . $minimumOrderQty];
        }
        ...
        $cartCheck = Cart::where(['customer_id' => $customerId, 'is_guest' => $isGuest, 'seller_id' => ($product->added_by == 'admin') ? 1 : $product->user_id, 'seller_is' => $product->added_by, 'product_type' => $product['product_type']])->first();
        if ($cartCheck) {
        ...
        if ($request['buy_now'] == 1) {
            ...
            if ($product['product_type'] == 'physical' && $shippingType == 'order_wise') {
                ...
                if ($sellerShippingList && count($sellerShippingList) > 0) {
                    return [
                        'status' => 2,
                        'message' => translate('Please_select_shipping_method'),
                        'shipping_method_list' => $sellerShippingList,
                    ];
                }
                // No shipping methods configured for this shop — proceed to checkout with free shipping
                $cart->update(['is_checked' => 1, 'shipping_cost' => 0]);
                $cart['free_delivery_order_amount'] = OrderManager::getFreeDeliveryOrderAmountArray($cart['cart_group_id']);
                return [
                    'status' => 1,
                    'redirect_to' => 'checkout',
...

    {
        ...
        if ($shippingMethod == 'inhouse_shipping') {
            ...
            if ($product->added_by == 'admin') {
                ...
                // Seller has no shipping methods configured — fall back to admin shipping
                if ($sellerShippingList->isEmpty()) {
                    $shippingType = isset($adminShipping) ? $adminShipping->shipping_type : 'order_wise';
                    $sellerShippingList = $shippingType == 'order_wise'
                        ? ShippingMethod::where(['status' => 1, 'creator_type' => 'admin'])->get()
                        : null;
                }
            }
        }
