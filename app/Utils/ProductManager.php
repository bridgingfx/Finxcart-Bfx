...
{
    // updateProductPriceInCartList() re-queries and potentially rewrites the
    // customer's entire cart; CartManager::getCartListQuery() calls it on every
    // invocation, and a single cart/checkout page render calls that helper
    // several times (desktop + mobile totals, free-delivery check, order
    // summary partial, etc.), so the same repricing pass was re-running up to
    // ~6x per page load. It only needs to run once per request — every call
    // within one request reads the same current-user cart from the same
    // current request, so there is nothing to recompute after the first pass.
    private static bool $hasRepricedCartThisRequest = false;
    ...
    public static function getTopRatedProducts($request, $limit = 10, $offset = 1)
    {
        ...
        $productListData = Product::active()->inStockOrDigital()->withSum('orderDetails', 'qty', function ($query) {
            $query->where('delivery_status', 'delivered');
        })
            ->with(['seller.shop', 'category', 'rating', 'flashDealProducts.flashDeal',
    ...
    public static function get_related_products($product_id, $request = null)
    {
        ...
        $products = Product::active()->inStockOrDigital()->with(['rating', 'flashDealProducts.flashDeal', 'tags', 'seller.shop', 'clearanceSale' => function ($query) {
            return $query->active();
        }])
            ->withCount(['reviews', 'wishList' => function ($query) use ($user) {
                $query->where('customer_id', $user != 'offline' ? $user->id : '0');
    ...
    public static function search_products($request, $name, $category = 'all', $limit = 10, $offset = 1): array
    {
        ...
        $productListData = Product::active()->inStockOrDigital()->with(['rating', 'tags', 'clearanceSale' => function ($query) {
            return $query->active();
        }])
        ...
        if (isset($category) && $category != 'all') {
            $categoryWiseProduct = $productListData->whereHas('categories', fn ($query) => $query->where('categories.id', $category));
    ...
    public static function getSearchProductsForWeb($name, $category = 'all', $limit = 10, $offset = 1): array
    {
        ...
        $productListData = Product::active()->inStockOrDigital()->with(['rating', 'tags'])->where(function ($q) use ($name) {
    ...
    public static function getTranslatedProductSearchForWeb($name, $category = 'all', $limit = 10, $offset = 1): array
    {
        ...
        if ($category !== 'all') {
            $productListData->whereHas('categories', fn ($query) => $query->where('categories.id', $category));
        }
    ...
    public static function getPriorityWiseFlashDealsProductsQuery($id = null, $userId = null): array
    {
        ...
        if ($flashDeal) {
            $flashDealProducts = ProductManager::getPriorityWiseFlashDealsProductsQuerySorting(
                query: Product::active()->inStockOrDigital()->with(['compareList', 'clearanceSale' => function ($query) {
                    return $query->active();
                }]),
                flashDeal: $flashDeal,
                userId: $userId,
    ...
    public static function getProductListData($request, $productUserID = null, $productAddedBy = null, $type = null): mixed
    {
        ...
        $productListData = Product::active()
            ...
            ->withCount(['reviews', 'wishList' => function ($query) use ($user) {
                $query->where('customer_id', $user != 'offline' && $user ? $user->id : '0');
            }])
            ...
            ->when($request->has('search_category_value') && !empty($request['search_category_value']) && $request['search_category_value'] != 'all', function ($query) use ($request) {
                return $query->whereHas('categories', fn ($query) => $query->where('categories.id', $request['search_category_value']));
            })
            ->when($request['offer_type'] == 'clearance_sale', function ($query) {
                $stockClearanceProductIds = StockClearanceProduct::active()->pluck('product_id')->toArray();
                return $query->whereIn('id', $stockClearanceProductIds);
        ...
        // Vendors sometimes create separate product rows with the same name
        // (one per category) instead of assigning one product to multiple
        // categories. Dedupe those by name here, keeping the newest (highest
        // id) row per name, before sorting/pagination is applied.
        $productListData = $productListData->groupBy('name')->map(function ($group) {
        ...
        if ($productSortBy) {
            ...
            if ($productSortBy == 'latest') {
                ...
                $productListData = $productListData->sortBy(function ($product) {
                    return getProductPriceByType(product: $product, type: 'discounted_unit_price', result: 'value', price: $product['unit_price']);
                });
            } elseif ($productSortBy == 'high-low') {
                $productListData = $productListData->sortByDesc(function ($product) {
                    return getProductPriceByType(product: $product, type: 'discounted_unit_price', result: 'value', price: $product['unit_price']);
                });
    ...
    {
        return Product::active()->inStockOrDigital()->with('rating')->withCount('reviews')
            ->when($productAddedBy == 'admin', function ($query) use ($productAddedBy) {
                return $query->where(['added_by' => $productAddedBy]);
            })
    ...
    {
        if (self::$hasRepricedCartThisRequest) {
            return;
        }
        self::$hasRepricedCartThisRequest = true;
        ...
        foreach ($cartItemsList as $cartItem) {
            ...
            $correctSellerId = ($cartItemProduct->added_by == 'admin') ? 1 : $cartItemProduct->user_id;
            if ($cartItem->seller_id != $correctSellerId || $cartItem->seller_is != $cartItemProduct->added_by) {
                Cart::where(['id' => $cartItem['id']])->update([
                    'seller_id' => $correctSellerId,
                    'seller_is' => $cartItemProduct->added_by,
                ]);
            }
    ...
    public static function addAdditionalQueryForCategoryWithSubCategories(
        ...
        if ($request->has('category_ids') && !empty($request['category_ids']) && is_array($request['category_ids'])) {
            $categoryIds = array_merge(
                ...
                $request['sub_sub_category_ids'] ?? []
            );
        } else {
            $categoryIds = array_filter([
                $request['category_id'] ?? null,
                $request['sub_category_id'] ?? null,
                $request['sub_sub_category_id'] ?? null,
            ]);
        }

        $categoryWiseProductIds = self::addAdditionalQueryForCategoryProduct(
            request: $request,
        ...
        return $query->whereIn('id', $categoryWiseProductIds);
    }
