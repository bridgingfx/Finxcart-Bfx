...
if (!function_exists('digital_payment_success')) {
    function digital_payment_success($paymentData)
    {
        if (isset($paymentData) && $paymentData['is_paid'] == 1) {
            ...
            // The order-complete page (PaymentController::web_payment_success) has no
            // other way to know which order(s) this gateway callback just created —
            // stash them in session so it can show the real order number instead of
            // a generic "your order is processing" message.
            session()->put('last_paid_order_ids', $orderIds);
...
// Freelancer Hire - Success (contract is only created once payment actually clears)
if (!function_exists('freelancer_hire_payment_success')) {
    function freelancer_hire_payment_success($paymentData)
    {
        if (!isset($paymentData) || $paymentData['is_paid'] != 1) {
            return;
        }
        ...
        $additionalData = json_decode($paymentData['additional_data'], true);
        $serviceId = $additionalData['freelancer_service_id'] ?? null;
        $customerId = $additionalData['customer_id'] ?? null;
        $scope = $additionalData['scope'] ?? null;

        if (!$serviceId || !$customerId || !$scope) {
            \Illuminate\Support\Facades\Log::error('[freelancer_hire_payment_success] Missing service/customer/scope in additional_data for payment_request ' . $paymentData['id']);
            return;
        }

        $service = \App\Models\FreelancerService::where('id', $serviceId)->where('is_active', true)->first();

        // Reuse the existing contract if this exact payment already produced one
        // (idempotent replay of the gateway's success URL) — see hire()'s own guard.
        $existingContract = \App\Models\FreelancerContract::where('payment_request_id', $paymentData['id'])->first();
        if ($existingContract) {
            session(['freelancer_hire_last_contract_id' => $existingContract->id]);
            return;
        }

        if (!$service) {
            \Illuminate\Support\Facades\Log::error('[freelancer_hire_payment_success] Could not create contract for payment_request ' . $paymentData['id'] . ' — service unavailable. Needs manual review/refund.');
            return;
        }

        $quoteRequestId = $additionalData['freelancer_quote_request_id'] ?? null;
        $packageTier = $additionalData['package_tier'] ?? null;
        $orderService = app(\App\Services\FreelancerOrderService::class);

        try {
            $contract = $quoteRequestId
                ? $orderService->purchaseFromQuote(
                    quoteRequestId: $quoteRequestId,
                    customerId: $customerId,
                    scope: $scope,
                    paymentRequestId: $paymentData['id'],
                )
                : $orderService->purchaseDirect(
                    service: $service,
                    customerId: $customerId,
                    scope: $scope,
                    paymentRequestId: $paymentData['id'],
            ...
            \Illuminate\Support\Facades\Log::error('[freelancer_hire_payment_success] Contract creation failed for payment_request ' . $paymentData['id'] . ': ' . $e->getMessage());
            return;
        }

        session(['freelancer_hire_last_contract_id' => $contract->id]);
    }
}

// Freelancer Hire - Fail (no contract was ever created, nothing to roll back)
if (!function_exists('freelancer_hire_payment_fail')) {
    function freelancer_hire_payment_fail($payment_data)
    {
