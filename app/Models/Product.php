...
class Product extends Model
{
    ...
    protected $casts = [
        ...
        'preview_url' => 'string', // Added here
        'external_url' => 'string',
        'not_sellable' => 'boolean',
        'book_tickets_url' => 'string',
        'view_floorplan_url' => 'string',
        'sponsor_exhibit_url' => 'string',
        'open_account_url' => 'string',
        'contact_broker_url' => 'string',
        'brochure' => 'string',
        'brochure_storage_type' => 'string',
        'event_days' => 'string',
        'event_industry_brands' => 'string',
    ...
    public function scopeActive($query)
    {
        ...
        $productType = $digitalProductSetting
            ? ['digital', 'physical', 'subscription', 'recurring', 'event', 'broker']
            : ['physical', 'event', 'broker'];
        ...
        return $query->when($businessMode == 'single', function ($query) {
            ...
            ->when($brandSetting, function ($query) use ($brandSetting, $productType) {
                if (!in_array('digital', $productType)) {
                    $query->where(function ($query) {
                        $query->whereIn('product_type', ['event', 'broker'])
            ...
            ->when(!$brandSetting, function ($query) {
                // brand feature off — no brand restriction, show all products regardless of brand_id
            })
            ->where(['status' => 1])
            ...
            ->where(function ($q) {
                $q->where('added_by', 'admin')
                    ->orWhereHas('seller.shop', fn($q) => $q->where('temporary_close', 0));
            })
            ->whereIn('product_type', $productType);
    }

    public function scopeInStockOrDigital($query)
    {
        return $query->where(function ($q) {
            $q->whereIn('product_type', ['digital', 'event', 'broker'])->orWhere('current_stock', '>', 0);
        });
    }
    ...
    public function scopeSellerApproved($query): void
    {
        $query->where(function ($query) {
            $query->whereHas('seller', function ($query) {
                $query->where(['status' => 'approved', 'account_status' => 'active']);
    ...
    public function getBrochureFullUrlAttribute(): array
    {
        return $this->storageLink('product/brochure', $this->brochure, $this->brochure_storage_type ?? 'public');
    }
    ...
    public function vendorTier(): BelongsTo
    {
