...
interface ChattingRepositoryInterface extends RepositoryInterface
{
    ...
    /**
     * Unseen-by-customer message counts for a customer, grouped by the given
     * thread-identifying column (e.g. 'seller_id', 'admin_id', 'delivery_man_id').
     * Batches what would otherwise be one COUNT query per chat thread.
     *
     * @param int $customerId
     * @param string $column
     * @return array<int, int> keyed by the column value
     */
    public function countUnseenForCustomerGroupedBy(int $customerId, string $column): array;
}
