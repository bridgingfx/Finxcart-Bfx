...
class ChatController extends Controller
{
    ...
    public function list(Request $request, $type)
    {
        ...
        $chats = [];
        if ($uniqueChatIds) {
            $partnerIds = array_column($uniqueChatIds, $idParam);

            $latestIdsByPartner = Chatting::where(['seller_id' => $seller['id']])
                ->whereIn($idParam, $partnerIds)
                ->select(DB::raw('MAX(id) as id'), $idParam)
                ->groupBy($idParam)
                ->pluck('id', $idParam);

            $latestChatsByPartner = Chatting::with([$withParam])
                ->whereIn('id', $latestIdsByPartner->values())
                ->get()
                ->keyBy($idParam);

            $unseenCountsByPartner = Chatting::where(['seller_id' => $seller['id']])
                ->whereIn($idParam, $partnerIds)
                ->where('seen_by_seller', '0')
                ->select($idParam, DB::raw('count(*) as cnt'))
                ->groupBy($idParam)
                ->pluck('cnt', $idParam);

            foreach ($uniqueChatIds as $uniqueChatId) {
                $partnerId = $uniqueChatId[$idParam];
                $userChatting = $latestChatsByPartner->get($partnerId);
                if ($userChatting) {
                    $userChatting->unseen_message_count = $unseenCountsByPartner->get($partnerId, 0);
                    $chats[] = $userChatting;
                }
            }
        }
