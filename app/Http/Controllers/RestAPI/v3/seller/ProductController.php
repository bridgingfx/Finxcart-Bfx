...
class ProductController extends Controller
{
    ...
    public function add_new(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'minimum_order_qty' => 'required|numeric|min:1|regex:/^\d+(\.\d+)?$/',
        ], [
    ...
    {
        $seller = $request->seller;
        // Scope to the authenticated seller's own product — without this, any
        // seller could update any other seller's stock by guessing product_id.
    ...
    {
        $seller = $request->seller;
        // Only delete a restock request whose product actually belongs to this
        // seller — the id alone is guessable and previously allowed deleting
        // any other seller's restock request.
        $restockProduct = $this->restockProductRepo->getFirstWhere(params: ['id' => $request['id']], relations: ['product']);
        if (!$restockProduct || !$restockProduct->product || $restockProduct->product->added_by !== 'seller' || (int) $restockProduct->product->user_id !== (int) $seller->id) {
            return response()->json(['message' => translate('invalid_prodcut')], 403);
        }
    ...
    {
        $seller = $request->seller;
        // Scope to the authenticated seller's own product — see
        // updateProductQuantity() above for the same fix and rationale.
        $product = $this->productRepo->getFirstWhere(params: ['id' => $request['product_id'], 'added_by' => 'seller', 'user_id' => $seller->id]);
