...
class OrderController extends Controller
{
    ...
    public function track_by_order_id(Request $request)
    {
        ...
        $data = OrderManager::track_order($request['order_id']);
    ...
    public function order_cancel(Request $request)
    {
        ...
        if ($order['payment_method'] == 'offline_payment') {
            return response()->json(['message' => translate('The_order_status_cannot_be_updated_as_it_is_an_offline_payment')], 403);
        }

        if ($order['order_status'] == 'pending') {
            OrderManager::stock_update_on_order_status_change($order, 'canceled');
            Order::where(['id' => $request->order_id])->update(['order_status' => 'canceled']);
    ...
    public function place_order(Request $request): JsonResponse
    {
        ...
        if (count($cartGroupIds) == 0) {
            return response()->json(['message' => translate('no_item_selected_for_order') ?? 'No item selected for order'], 403);
        }
        ...
        $orderIds = [];
        DB::transaction(function () use ($request, $cartGroupIds, $getUniqueId, $newCustomerRegister, $currencyCode, &$orderIds) {
    ...
    public function placeOrderByOfflinePayment(Request $request): JsonResponse
    {
        ...
        $order_ids = [];
        DB::transaction(function () use ($request, $cartGroupIds, $unique_id, $newCustomerRegister, $offline_payment_info, &$order_ids) {
    ...
    public function placeOrderByWallet(Request $request): JsonResponse
    {
        ...
        if ($paymentAmount > $user->wallet_balance) {
            ...
            $order_ids = [];
            DB::transaction(function () use ($request, $cart_group_ids, $unique_id, $user, $paymentAmount, &$order_ids) {
                foreach ($cart_group_ids as $group_id) {
                    $data = [
    ...
    {
        $orderData = OrderManager::order_again($request);
