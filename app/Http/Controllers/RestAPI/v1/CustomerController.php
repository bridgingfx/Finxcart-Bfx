...
class CustomerController extends Controller
{
    ...
    public function get_order_details(Request $request): JsonResponse
    {
        ...
        $reviewsByProduct = Review::where('customer_id', $user->id)
            ->whereIn('product_id', $detailsList->pluck('product_id')->unique())
            ->whereNull('delivery_man_id')
            ->get()
            ->groupBy('product_id');

        $detailsList->map(function ($query) use ($user, $reviewsByProduct) {
            ...
            $reviews = $reviewsByProduct[$query['product_id']] ?? collect();
            $reviewData = null;
    ...
    public function getOrderInvoice(Request $request)
    {
        ...
        $order = Order::with('seller')->with('shipping')->with('customer')
        ...
        $data["email"] = $order?->customer['email'] ?? null;
        $data["order"] = $order;
