    ...
    public function customerSocialLogin(Request $request): JsonResponse
    {
        ...
        try {
            if ($request['medium'] == 'google') {
                ...
                $apple_login = is_array($apple_login) && isset($apple_login[0]) ? $apple_login[0] : $apple_login;
                $teamId = $apple_login['team_id'];
                $keyId = $apple_login['key_id'];
                $sub = $apple_login['client_id'];
                $aud = 'https://appleid.apple.com';
        ...
        if (!isset($claims) && !isset($data['email'])) {
            return response()->json(['errors' => [['code' => 'auth-001', 'message' => translate('email_not_found_from_social_provider')]]], 403);
        }
