...
class UserLoyaltyController extends Controller
{
    ...
    public function loyalty_exchange_currency(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            'point' => 'required|integer|min:1|regex:/^\d+$/'
        ]);
        ...
        // so the mobile app isn't stuck waiting on an SMTP/Brevo round-trip.
        $userEmail = $user['email'];
        dispatch(function () use ($userEmail, $walletTransaction) {
            try {
                Mail::to($userEmail)->send(new \App\Mail\AddFundToWallet($walletTransaction));
            } catch (\Exception $ex) {
            }
        })->afterResponse();
