...
class ProductController extends Controller
{
    ...
    public function get_searched_products(Request $request): JsonResponse
    {
        ...
        $products = ProductManager::search_products($request, $request['name'], 'all', $limit, $offset);

        if ($products['products'] == null) {
            $products = ProductManager::translated_product_search(base64_encode($request['name']), 'all', $limit, $offset);
        }
        $products['products'] = Helpers::product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
    ...
    public function getShopAgainProduct(Request $request): JsonResponse
    {
        $user = Helpers::getCustomerInformation($request);
        if ($user != 'offline') {
            $products = Product::active()->with(['seller.shop', 'clearanceSale' => function ($query) {
                return $query->active();
            }])
