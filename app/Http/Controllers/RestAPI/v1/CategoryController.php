...
class CategoryController extends Controller
{
    public function get_categories(Request $request): JsonResponse
    {
        ...
        // Only "most_order" custom sorting actually reads the eager-loaded `product` relation
        // (see CategoryManager::getPriorityWiseCategorySortQuery) — every other sort mode never
        // touches it, so avoid loading every category's full product collection unconditionally.
        $categoryProductSortBy = getWebConfig(name: 'category_list_priority');
        $needsProductOrderCount = $categoryProductSortBy
            && ($categoryProductSortBy['custom_sorting_status'] == 1)
            && ($categoryProductSortBy['sort_by'] == 'most_order');
        ...
        $categories = Category::when($request->has('seller_id') && $request['seller_id'] != null, function ($query) use ($categoriesID) {
            ...
            ->when($needsProductOrderCount, function ($query) {
