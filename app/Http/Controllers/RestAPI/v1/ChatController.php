...
class ChatController extends Controller
{
    public function list(Request $request, $type)
    {
        ...
        if ($unique_chat_ids) {
            $partnerIds = array_column($unique_chat_ids, $id_param);

            $latestIdsByPartner = Chatting::where(['user_id' => $request->user()->id])
                ->whereIn($id_param, $partnerIds)
                ->select(DB::raw('MAX(id) as id'), $id_param)
                ->groupBy($id_param)
                ->pluck('id', $id_param);

            $latestChatsByPartner = Chatting::with([$with])
                ->whereIn('id', $latestIdsByPartner->values())
                ->get()
                ->keyBy($id_param);

            $unseenCountsByPartner = Chatting::where(['user_id' => $request->user()->id])
                ->whereIn($id_param, $partnerIds)
                ->where('seen_by_customer', '0')
                ->select($id_param, DB::raw('count(*) as cnt'))
                ->groupBy($id_param)
                ->pluck('cnt', $id_param);

            foreach ($unique_chat_ids as $unique_chat_id) {
                $partnerId = $unique_chat_id[$id_param];
                $user_chatting = $latestChatsByPartner->get($partnerId);
                if ($user_chatting) {
                    $user_chatting->unseen_message_count = $unseenCountsByPartner->get($partnerId, 0);
                    $chats[] = $user_chatting;
                }
            }
        }
    ...
    public function search(Request $request, $type)
    {
        ...
        if ($unique_chat_ids->isNotEmpty()) {
            ...
            $latestChatsByPartner = Chatting::with([$with_param])
                ->whereIn('id', $latestIdsByPartner->values())
                ->get()
                ->keyBy($id_param);
            ...
            foreach ($unique_chat_ids as $unique_chat_id) {
                $user_chatting = $latestChatsByPartner->get($unique_chat_id);
                if ($user_chatting) {
                    $user_chatting->unseen_message_count = $unseenCountsByPartner->get($unique_chat_id, 0);
                    $chats[] = $user_chatting;
                }
            }
        }
