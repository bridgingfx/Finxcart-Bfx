...
class ProductController extends Controller
{
    public function list(Request $request)
    {
        ...
        return response()->json(Product::where(['added_by' => 'seller', 'user_id' => $seller['id']])->orderBy('id', 'DESC')->get(), 200);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$product) {
            return response()->json([
                'errors' => [
                    ['code' => 'product', 'message' => translate('not found!')]
                ]
            ], 404);
        }
        ...
        $validator = Validator::make($request->all(), [
            ...
            'shipping_cost'         => 'required_if:product_type,==,physical|gt:-1',
            'minimum_order_qty'     => 'required|numeric|min:1|regex:/^\d+(\.\d+)?$/',
            'code'                  => 'required|numeric|min:1|regex:/^\d+$/|digits_between:6,20|unique:products,code,'.$product->id,
        ], [
