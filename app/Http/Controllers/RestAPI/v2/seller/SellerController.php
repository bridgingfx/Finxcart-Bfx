...
class SellerController extends Controller
{
    ...
    public function shop_product_reviews_status(Request $request)
    {
        ...
        if ($data['success'] == 1) {
            ...
            if (!$reviews) {
                return response()->json([
                    'errors' => [
                        ['code' => 'review', 'message' => translate('not found!')]
                    ]
                ], 404);
            }
    ...
    public function shop_info_update(Request $request)
    {
        ...
        $old_image = Shop::where(['seller_id' => $seller['id']])->first()->image ?? null;
    ...
    public function withdraw_request(Request $request)
    {
        ...
        // PAYOUT-01: balance check + deduction must be atomic and row-locked so concurrent
        // requests cannot both pass the check and over-withdraw the wallet.
        $success = DB::transaction(function () use ($seller, $amount, $request) {
            $wallet = SellerWallet::where('seller_id', $seller['id'])->lockForUpdate()->first();
            if (!$wallet || $wallet->total_earning < $amount || $request['amount'] <= 1) {
                return false;
            }
            ...
            $wallet->pending_withdraw += $amount;
            $wallet->save();
            return true;
        });
        if ($success) {
