...
class PaypalPaymentController extends Controller
{
    ...
    public function success(Request $request): Application|JsonResponse|Redirector|\Illuminate\Contracts\Foundation\Application|RedirectResponse
    {
        ...
        $accessToken = json_decode($this->token(), true);
        if (!isset($accessToken['access_token'])) {
            \Log::error('[Paypal] success() could not obtain an access token.');
            return redirect()->route('payment-fail');
        }
        $accessToken = $accessToken['access_token'];
        ...
        $result = curl_exec($ch);
        if (curl_errno($ch)) {
            \Log::error('[Paypal] success() capture request failed: ' . curl_error($ch));
        }
        curl_close($ch);
        ...
        if (!isset($response->status)) {
            \Log::error('[Paypal] success() got an unreadable response from the capture API.');
            return redirect()->route('payment-fail');
        }
