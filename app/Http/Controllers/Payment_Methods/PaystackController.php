...
class PaystackController extends Controller
{
    ...
    public function handleGatewayCallback(Request $request)
    {
        try {
            ...
            \Log::error('[Paystack] handleGatewayCallback() getPaymentData() failed: ' . $e->getMessage());
            return redirect()->route('payment-fail');
        }

        $orderId = $paymentDetails['data']['metadata']['orderID'] ?? null;
        if (!$orderId) {
            return redirect()->route('payment-fail');
        }

        if (($paymentDetails['status'] ?? false) == true) {
            $this->payment::where(['attribute_id' => $orderId])->update([
                'payment_method' => 'paystack',
                'is_paid' => 1,
                'transaction_id' => $request['trxref'],
            ]);
            $data = $this->payment::where(['attribute_id' => $orderId])->first();
        ...
        $payment_data = $this->payment::where(['attribute_id' => $orderId])->first();
