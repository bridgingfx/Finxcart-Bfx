...
class SystemController extends Controller
{
    ...
    public function getChooseShippingAddressOther(Request $request): JsonResponse
    {
        ...
        $digitalDeliveryType = $request->input('digital_delivery_type');
        $digitalDeliveryValue = $request->input('digital_delivery_value');

        if ($request->filled('digital_delivery_type') && $request->filled('digital_delivery_value')) {
            session()->put('digital_delivery_type', $digitalDeliveryType);
            session()->put('digital_delivery_value', $digitalDeliveryValue);
        } else {
            session()->forget('digital_delivery_type');
            session()->forget('digital_delivery_value');

            // digital_delivery_type is only non-null when cart has digital products
            if ($request->filled('digital_delivery_type') && !$request->filled('digital_delivery_value')) {
                return response()->json(['errors' => translate('Please_enter_your_digital_delivery_address')], 403);
            }
        }
        ...
        if (!session('address_id') && !session('billing_address_id') && !($physicalProduct != 'yes' && session()->has('digital_delivery_type') && session()->has('digital_delivery_value'))) {
            return response()->json([
