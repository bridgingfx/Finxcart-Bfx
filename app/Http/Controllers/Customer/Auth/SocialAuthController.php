...
class SocialAuthController extends Controller
{
    ...
    public function handleProviderCallback(Request $request, $service)
    {
        ...
        // AUTH-01 FIX: If an account already exists for this email, NEVER overwrite its
        // password. A manual (email/password) customer who later signs in with a social
        // provider must keep their original password so normal login keeps working. We
        // only link the social identity + verify the email, then log them in.
        if ($user) {
            $this->customerRepo->updateWhere(params: ['email' => $user['email']], data: [
                'is_email_verified' => 1,
                'social_id' => $userSocialData->id,
                'login_medium' => $user['login_medium'] ?: $service,
                'temporary_token' => Str::random(40),
            ]);
            $user = $this->customerRepo->getFirstWhere(params: ['email' => $user['email']]);
            return self::actionCustomerLoginProcess($request, $user, $user['email']);
        }

        // No existing account for this email -> create a brand-new social account.
        ...
        if (!$phoneVerification) {
            $this->customerRepo->updateOrCreate(
                params: ['email' => $userSocialData->getEmail()],
                data: session('social_login_new_customer')
            );
            $user = $this->customerRepo->getFirstWhere(params: ['email' => $userSocialData->getEmail()]);
            return self::actionCustomerLoginProcess($request, $user, $user['email']);
        }
    ...
    public function socialLoginConfirmation(Request $request): View|RedirectResponse
    {
        ...
        if (!$phoneVerification && $socialLoginNewCustomer) {
            $this->customerRepo->updateOrCreate(
                params: ['email' => $socialLoginNewCustomer['email']],
                data: $socialLoginNewCustomer
            );
    ...
    public function updateSocialLoginConfirmation(Request $request): View|RedirectResponse
    {
        ...
        if ($phoneVerification && !$request['phone']) {
            Toastr::error(translate('Please_use_a_phone_number'));
            return back();
        }

        $socialLoginNewCustomer = session('social_login_new_customer');
        $phoneCheck = $phoneVerification && $request['phone']
            ? $this->customerRepo->getFirstWhere(params: ['phone' => $request['phone']])
            : null;
        if ($phoneCheck) {
        ...
        if ($socialLoginNewCustomer) {
            session()->put('social_login_new_customer', [
                ...
                'is_email_verified' => 1,
                'is_phone_verified' => $phoneVerification ? 0 : 1,
