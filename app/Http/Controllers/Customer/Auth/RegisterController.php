...
class RegisterController extends Controller
{
    ...
    public function getRegisterView(): View
    {
        ...
        $termsPage = BusinessPage::where('slug', 'terms-and-conditions')->first();
        return view('web-views.customer-views.auth.register', compact('termsPage'));
    }
    ...
    public function submitRegisterData(CustomerRegistrationRequest $request): JsonResponse|RedirectResponse
    {
        ...
        try {
            $name = trim(($user->f_name ?? '') . ' ' . ($user->l_name ?? '')) ?: $user->email;
            AdminNotification::create([
                'type' => 'customer_registration',
                'title' => 'New Customer Registered',
                'message' => "{$name} ({$user->email}) has created a new customer account.",
                'link' => route('admin.customer.view', ['user_id' => $user->id]),
                'reference_id' => $user->id,
            ]);
        } catch (\Throwable $e) {
            \Log::error('[CustomerRegister] Admin notification failed: ' . $e->getMessage());
        }
    ...
    public function verificationCheckView(Request $request): View
    {
        ...
        return view(VIEW_FILE_NAMES['customer_auth_verify'], [
            ...
            'localDevOtp' => app()->environment('local') ? ($token['token'] ?? null) : null,
        ]);
    }
        ...
        if ($timeDifferance > 0) {
            ...
            if ($request->ajax()) {
                return response()->json([
                    'status' => 0,
                    'message' => $message,
                    'new_time' => $timeDifferance,
                    'otp_expires_at' => now()->addSeconds($timeDifferance)->getTimestamp() * 1000,
                ], 429);
            }
            ...
            if ($identityType == 'phone') {
                ...
                if ($request->ajax()) {
                    return response()->json([
                        ...
                        'new_time' => $maxOTPHitTime,
                        'otp_expires_at' => now()->addSeconds($maxOTPHitTime)->getTimestamp() * 1000,
                    ]);
                }
                return redirect()->back();
