...
class CustomerAuthController extends Controller
{
                ...
                $request->validate([
                    'g-recaptcha-response' => [
                        function ($attribute, $value, $fail) {
                            ...
                            // blocked customer login for the default 60s+ socket timeout.
    ...
    public function resendOTPCode(Request $request): JsonResponse|RedirectResponse
    {
        ...
        if ($timeDifferance == 0) {
            ...
            if (request()->ajax()) {
                return response()->json(['status' => 1, 'new_time' => $otpResendTime, 'otp_expires_at' => now()->addSeconds($otpResendTime)->getTimestamp() * 1000]);
            } else {
            ...
            if (request()->ajax()) {
                return response()->json(['status' => 0, 'new_time' => $timeDifferance, 'otp_expires_at' => now()->addSeconds($timeDifferance)->getTimestamp() * 1000]);
            } else {
