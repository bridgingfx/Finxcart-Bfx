...
class PaymentController extends Controller
{
    ...
    {
        return response()->json(['message' => 'Payment cancelled'], 403);
    }
    ...
    public function web_payment_success(Request $request)
    {
        if($request->flag == 'success') {
            if (session()->has('payment_mode') && session('payment_mode') == 'app') {
                ...
                $order_ids = session('last_paid_order_ids', []);
                session()->forget('last_paid_order_ids');
                return view(VIEW_FILE_NAMES['order_complete'], compact('isNewCustomerInSession', 'order_ids'));
            }
        }else{
            if(session()->has('payment_mode') && session('payment_mode') == 'app'){
                return response()->json(['message' => 'Payment failed'], 403);
