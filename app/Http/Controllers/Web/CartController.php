...
class CartController extends Controller
{
    ...
    public function getVariantPrice(Request $request): array
    {
        ...
        if (!$product) {
            return [
                'price' => webCurrencyConverter(0),
                'discount' => webCurrencyConverter(0),
                'discount_type' => 'amount',
                'discount_amount' => 0,
                'tax' => webCurrencyConverter(0),
                'update_tax' => webCurrencyConverter(0),
                'quantity' => 0,
                'delivery_cost' => 0,
                'unit_price' => webCurrencyConverter(0),
                'total_unit_price' => webCurrencyConverter(0),
                'discounted_unit_price' => webCurrencyConverter(0),
                'color_name' => '',
                'stock_limit' => 0,
                'in_cart_status' => 0,
                'in_cart_quantity' => 0,
        ...
        // NEW: Define product types that act like 'digital'
        $digital_types = ['digital', 'recurring', 'subscription'];
        ...
        $requestQuantity = $productVariationCode != $string ? $minimumOrderQty : $request['quantity'];
        $inCartExistStatus = 0;
        $inCartExistKey = null;
        ...
        foreach ($getCartList as $cartItem) {
            ...
            // NEW: Check against the $digital_types array
            if (in_array($product['product_type'], $digital_types) && $request['variant_key'] && $cartItem['variant'] == $request['variant_key']) {
                $inCartExistStatus = 1;
                $inCartExistKey = $cartItem['id'];
        ...
        // NEW: Check against the $digital_types array
        if (in_array($product['product_type'], $digital_types) && $digitalVariation) {
            ...
            foreach ($getCartList as $cartItem) {
                // NEW: Check against the $digital_types array
                if (in_array($product['product_type'], $digital_types) && $cartItem['variant'] == $request['variant_key']) {
    ...
    public function addToCart(Request $request): JsonResponse|RedirectResponse
    {
        if (auth('seller')->check()) {
            return response()->json([
                'status' => 0,
                'vendor_blocked' => true,
                'message' => translate('Vendors_cannot_purchase_products._Please_register_or_login_as_a_customer_to_buy.'),
            ]);
        }
        ...
        $cart['cart_count'] = CartManager::cart_count();

        if ($cart['status'] == 1) {
            $product = \App\Models\Product::find($request->id);
            if ($product) {
                $cart['popup_product_name'] = $product->name;
                $cart['popup_product_type'] = $product->product_type;
                $cart['popup_product_thumbnail'] = getStorageImages(path: $product->thumbnail_full_url, type: 'product');
                $cart['popup_cart_url'] = route('shop-cart');
            }
        }
    ...
    public function updateNavCart(): JsonResponse
    {
        ...
        return response()->json([
            'data' => view(VIEW_FILE_NAMES['products_cart_partials'])->render(),
            'mobile_nav' => view(VIEW_FILE_NAMES['products_mobile_nav_partials'])->render(),
            'cart_count' => $cart?->count() ?? 0,
            'cart_total' => webCurrencyConverter(amount: CartManager::getCartListTotalAppliedDiscount($cart)),
        ]);
    }
    ...
    public function removeFromCart(Request $request): JsonResponse
    {
        ...
        $cart_count = CartManager::cart_count();
    ...
    public function updateQuantity(Request $request)
    {
        ...
        if ($response['status'] == 0) {
            $response['cart_count'] = CartManager::cart_count();
    ...
    function update_variation(Request $request)
    {
        ...
        // NEW: Define which types are "digital" in behavior
        $digital_types = ['digital', 'recurring', 'subscription'];

        // NEW: Check if the product type is in our array
        if (in_array($product['product_type'], $digital_types)) {
            return self::addToCartDigitalProduct($request, $product);
        } else {
            // This will now correctly handle 'physical' and any other unknown types
