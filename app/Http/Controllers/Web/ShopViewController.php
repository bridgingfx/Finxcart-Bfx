...
class ShopViewController extends Controller
{
    public function getShopInfoArray($shopId, $shopProducts, $sellerType, $sellerId): array
    {
        ...
        return [
            'id' => $shopId,
            'name' => $shopId == 0 ? getWebConfig(name: 'company_name') : ($shop?->name ?? translate('not_available')),
            ...
            'vacation_start_date' => $shopId == 0 ? $inhouseVacation['vacation_start_date'] : ($shop?->vacation_start_date ? date('Y-m-d', strtotime($shop->vacation_start_date)) : null),
            'vacation_end_date' => $shopId == 0 ? $inhouseVacation['vacation_end_date'] : ($shop?->vacation_end_date ? date('Y-m-d', strtotime($shop->vacation_end_date)) : null),
            'temporary_close' => $shopId == 0 ? $temporaryClose['status'] : ($shop?->temporary_close ?? false),
            'vacation_status' => $shopId == 0 ? $inhouseVacation['status'] : ($shop?->vacation_status ?? false),
    ...
    {
        $shopCheck = self::checkShopExistence($id);
        if ($shopCheck !== true) {
            return $shopCheck;
        }

        $productAddedBy = $id == 0 ? 'admin' : 'seller';
        $shop = $id == 0 ? null : Shop::where('id', $id)->first();
        $productUserID = $id == 0 ? $id : $shop->seller_id;
        ...
        if ($request->ajax()) {
            ...
            return response()->json([
                ...
                'html_products' => view(VIEW_FILE_NAMES['products__ajax_partials'], compact('products', 'categories', 'isInitialLoad'))->render()
            ], 200);
        }
    ...
    public function theme_aster($request, $id): View|JsonResponse|Redirector|RedirectResponse
    {
        ...
        })->when($id != 0, function ($query) use ($productUserID) {
            return $query->where(['added_by' => 'seller', 'user_id' => $productUserID]);
        });
        ...
        if ($id == 0) {
            ...
            $seller = Seller::find($productUserID);
            $totalOrder = $seller?->orders?->where('seller_is', 'seller')->where('order_type', 'default_type')->count() ?? 0;
            $products_for_review = Product::active()->where('added_by', 'seller')->where('user_id', $productUserID)->withCount('reviews')->count();
        }
    ...
    public function theme_fashion($request, $id): View|JsonResponse|Redirector|RedirectResponse
    {
        ...
        if ($id == 0) {
            ...
            $total_order = $seller?->orders?->where('seller_is', 'seller')->where('order_type', 'default_type')->count() ?? 0;
            $products_for_review = Product::active()->where('added_by', 'seller')->where('user_id', $id)->withCount('reviews')->count();
            $featuredProductsList = $featuredProductQuery->where(['added_by' => 'seller', 'user_id' => $id]);
        }
    ...
    public function filterProductsAjaxResponse(Request $request): JsonResponse
    {
        ...
        if ($request->has('shop_id')) {
            ...
            $shopCheck = self::checkShopExistence($shopID);
            if ($shopCheck !== true) {
                return response()->json([
                    'success' => 0,
                    'message' => translate('Shop_does_not_exist'),
                ], 404);
            }

            $productAddedBy = $shopID == 0 ? 'admin' : 'seller';
            $shop = $shopID == 0 ? null : Shop::where('id', $shopID)->first();
            $productUserID = $shopID == 0 ? $shopID : $shop->seller_id;
            $productListData = ProductManager::getProductListData($request, $productUserID, $productAddedBy);
        } else {
        ...
        return response()->json([
            'html_products' => view('web-views.products._ajax-products', [
                'products' => $products,
            ...
            'html_tags' => view('web-views.products._selected_filter_tags', [
