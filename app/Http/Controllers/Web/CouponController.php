...
class CouponController extends Controller
{
    public function apply(Request $request): JsonResponse|RedirectResponse
    {
        ...
        if ($coupon && (($coupon->coupon_type == 'first_order') || ($coupon->coupon_type == 'discount_on_purchase' && ($coupon->customer_id == '0' || $coupon->customer_id == auth('customer')->id())))) {
            ...
            foreach ($checkedCartList as $cart) {
            ...
            if ($total >= $coupon['min_purchase']) {
                ...
                return response()->json([
                    ...
                    'messages' => $this->getCouponSuccessMessages($coupon, $checkedCartList)
                ]);
            }
    ...
    private function getCouponSuccessMessages(Coupon $coupon, $checkedCartList = null): array
    {
        $messages = [translate('coupon_applied_successfully') . '!'];

        if ($coupon->seller_id) {
            $hasOtherVendorItems = ($checkedCartList ?? CartManager::getCartListQuery(type: 'checked'))
                ->contains(fn($cart) => !($cart->seller_is == 'seller' && $cart->seller_id == $coupon->seller_id));

            if ($hasOtherVendorItems) {
                $vendorName = $coupon->seller?->shop?->name ?? translate('this_seller');
                $messages[] = translate('this_coupon_only_applies_to_products_from') . ' ' . $vendorName . '. ' . translate('items_from_other_sellers_will_not_be_discounted');
            }
        }
