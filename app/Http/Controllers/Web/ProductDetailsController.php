...
class ProductDetailsController extends Controller
{
    ...
    public function getDefaultTheme(string $slug): View|RedirectResponse
    {
        $product = $this->productRepo->getWebFirstWhereActive(
            ...
            relations: ['seoInfo', 'digitalVariation', 'reviews' => 'reviews', 'seller.shop' => 'seller.shop',
                'digitalProductAuthors' => 'digitalProductAuthors', 'digitalProductPublishingHouse' => 'digitalProductPublishingHouse',
                'clearanceSale' => 'clearanceSale', 'subCategory' => 'subCategory']
        );
        ...
        if ($product) {
            ...
            $relatedProducts = $this->productRepo->getWebListWithScope(
                ...
                whereNotIn: ['id' => [$product['id']]],
                relations: ['reviews' => 'reviews', 'clearanceSale' => 'clearanceSale'],
                dataLimit: 12,
                offset: 1
            );
    ...
    public function getThemeAster(string $slug): View|RedirectResponse
    {
        ...
        if ($product ) {
            ...
            $relatedProducts = $this->productRepo->getWebListWithScope(
                ...
                whereNotIn: ['id' => [$product['id']]],
                relations: ['reviews' => 'reviews', 'flashDealProducts.flashDeal' => 'flashDealProducts.flashDeal', 'wishList' => 'wishList', 'compareList' => 'compareList', 'clearanceSale' => 'clearanceSale'],
                withCount: ['reviews' => 'reviews'],
                dataLimit: 12,
                offset: 1
            );
    ...
    public function getThemeFashion($slug): View|RedirectResponse
    {
        ...
        if ($product != null) {
            ...
            $moreProductFromSeller = $this->productRepo->getWebListWithScope(
                ...
                whereNotIn: ['id' => [$product['id']]],
                relations: ['wishList' => 'wishList', 'reviews' => 'reviews'],
                dataLimit: 5,
                offset: 1
            );
            if ($product['added_by'] == 'seller') {
            ...
            $ratingCount = $vendorReviewData->count();
            $avgRating = $vendorReviewData->avg('rating');
            $vendorRattingStatusPositive = (clone $vendorReviewData)->where('rating', '>=', 4)->count();
            ...
            $sellerList = $this->sellerRepo->getListWithScope(
                scope: 'active',
                filters: ['category_id' => $product['category_id']],
                relations: ['shop' => 'shop', 'product' => 'product'],
                withCount: ['product' => 'product'],
                dataLimit: 'all',
            );

            // Single grouped query (same seller->product scope: added_by='seller', user_id=seller id)
            // instead of loading every review for every product and summing in PHP per seller.
            $reviewStatsBySeller = Review::query()
                ->join('products', 'products.id', '=', 'reviews.product_id')
                ->where('products.added_by', 'seller')
                ->whereIn('products.user_id', $sellerList->pluck('id'))
                ->selectRaw('products.user_id as seller_user_id, AVG(reviews.rating) as avg_rating, COUNT(*) as rating_count')
                ->groupBy('products.user_id')
                ->get()
                ->keyBy('seller_user_id');

            $sellerList?->map(function ($seller) use ($reviewStatsBySeller) {
                $stats = $reviewStatsBySeller->get($seller->id);
                $seller['average_rating'] = $stats->avg_rating ?? 0;
                $seller['rating_count'] = (int) ($stats->rating_count ?? 0);
