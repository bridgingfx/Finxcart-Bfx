...
class HomeController extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        $wishlistedProductIds = $userId ? Wishlist::where('customer_id', $userId)
            ->whereIn('product_id', $latestProductsList->pluck('id'))
            ->pluck('product_id')
            ->toArray() : [];
        $latestProductsList->each(function ($product) use ($wishlistedProductIds) {
            $product->wish_list_count = in_array($product->id, $wishlistedProductIds) ? 1 : 0;
        });
        ...
        $dealOfTheDay = Cache::remember(CACHE_HOME_DEAL_OF_THE_DAY, CACHE_FOR_3_HOURS, function () {
		...
		/*Custom Section codes here*/
        $getCategorySectionData = function (?int $rootCategoryId, array $slugs = [], array $names = []): array {
            $rootCategory = CategoryManager::resolveRootCategory(
            ...
            $products = Product::whereHas('categories', fn ($query) => $query->whereIn('categories.id', $subcategoryIds))
                ->active()
                ->with(['categories', 'clearanceSale' => function ($query) {
                    return $query->active();
                }])
                ->get();

            return $subcategories->map(function ($subcat) use ($products) {
                return [
                    'subcategory' => $subcat,
        ...
        $getProductsByNames = function (array $names) {
            return Product::whereIn('name', $names)
                ...
                ->get()
                ->sortBy(fn ($product) => array_search($product->name, $names))
                ->values();
        };

        $webinarsCatProduct = Cache::remember(CACHE_HOME_CATEGORY_SECTION_PRODUCTS, CACHE_FOR_3_HOURS, function () use ($getCategorySectionData) {
            return $getCategorySectionData(261, ['webinars'], ['Webinars']);
        });
        $PreBuiltWebsites = $getProductsByNames(['Prop Firm Website Development', 'Broker Website Development', 'Website Templates']);
        $APIIntegrations = $getProductsByNames(['Global Payment API', 'Custom API Development', 'Client Onboarding Workflow']);
        $AddOns = $getProductsByNames(['Fintech Security Shield', 'AI Trading Tools', 'Trading Signals Hub']);
		$featuredProducts = Cache::remember(CACHE_HOME_TOP_FEATURED_PRODUCTS, CACHE_FOR_3_HOURS, function () {
            ...
            // NOTE: this section previously also built $productCatWithProducts here, but that
            // variable was never passed to the view (not in the compact() list below) — it was
            // dead code. The value actually rendered on the home page comes from
            // _home-top-slider.blade.php, which unconditionally overwrites it via
            // CategoryManager::getCategoryWithProducts() (now cached, see that method).
            $productsCategory = CategoryManager::resolveRootCategory(
                categoryId: 7,
                slugs: ['home'],
                names: ['Home']
            );

            $headCategoryIds = collect([
                $productsCategory?->id,
                CategoryManager::resolveRootCategory(categoryId: 8, slugs: ['marketplace'], names: ['Marketplace'])?->id,
            ])->filter()->values()->all();

            $fullCategoryStructure = Cache::remember(CACHE_HOME_FULL_CATEGORY_STRUCTURE, CACHE_FOR_3_HOURS, function () use ($headCategoryIds) {
                $heads = collect($headCategoryIds)
                    ->map(fn ($headId) => Category::find($headId))
                    ->filter()
                    ->values();

                // First pass: build the head -> category -> single-subcategory tree
                // with no product queries yet (subcategory count per cat/head is small,
                // so per-cat lookups here are cheap; the per-item product fetch below is not).
                $headEntries = $heads->map(function ($head) {
                    $categories = Category::where('parent_id', $head->id)
                        ->get()
                        ->map(function ($cat) use ($head) {
                            $subcategory = Category::where('parent_id', $cat->id)
                                ->whereNotNull('icon')
                                ->where('icon', '!=', '')
                                ->orderBy('id', 'desc')
                                ->limit(1)
                            ...
                            return ['head' => $head, 'cat' => $cat, 'subcategory' => $subcategory];
                        });

                    return ['head' => $head, 'categoryEntries' => $categories];
                });

                // Batch-fetch products for every selected subcategory in a single query.
                $subcategoryIds = $headEntries
                    ->flatMap(fn ($entry) => $entry['categoryEntries'])
                    ->pluck('subcategory')
                    ->filter()
                    ->pluck('id')
                    ->unique()
                ...
                $productsBySubcategory = Product::whereHas('categories', fn ($query) => $query->whereIn('categories.id', $subcategoryIds))
                    ->active()
                    ->with(['categories', 'clearanceSale' => function ($query) {
                        return $query->active();
                    }])
                ...
                return $headEntries->map(function ($headEntry) use ($productsBySubcategory) {
                    $head = $headEntry['head'];

                    $categories = $headEntry['categoryEntries']->map(function ($entry) use ($head, $productsBySubcategory) {
                        $cat = $entry['cat'];
                        $subcat = $entry['subcategory'];

                        if ($subcat) {
                            $products = $productsBySubcategory
                                ->filter(fn ($product) => $product->categories->contains('id', $subcat->id))
                                ->map(function ($product) use ($cat) {
                                    $product->head_category_name = $cat->name;
                                    return $product;
                                })
                                ->values();
                            ...
                            $subcategories = collect([[
                                'subcategory' => null,
                                'products'    => collect(),
                                'image'       => $cat->icon ?: $head->icon,
                                'cat_id'      => $cat->id ?: $head->id,
                            ]]);
                        }
                        ...
                        return [
                            ...
                            'image'         => $cat->icon ?: $head->icon,
                            'image_full_url'=> $cat->icon ? $cat->icon_full_url : $head->icon_full_url,
                    ...
                    return [
                        'head_category_name' => $head->name,
                        'image'              => $head->icon,
                        'image_full_url'     => $head->icon_full_url,
        ...
        $headSolutionCategory = CategoryManager::resolveRootCategory(slugs: ['solutions'], names: ['Solutions']);
        $headSolutionCategoryIds = $headSolutionCategory ? [$headSolutionCategory->id] : [];

        $solutioncategoryNames = Cache::remember(CACHE_HOME_SOLUTIONS_CATEGORY_NAMES, CACHE_FOR_3_HOURS, function () use ($headSolutionCategoryIds) {
            return collect($headSolutionCategoryIds)->map(function ($SolutionheadId) {
                $head = Category::find($SolutionheadId);
                if (!$head) return null;

                return Category::where('parent_id', $head->id)
                    ->get(['id', 'name'])
        ...
        // "Solutions" section tabs (Bestsellers / Featured / On Sale / 50% Offer).
        // Site-wide: not scoped to the Solutions category tree, so any active product
        // meeting a tab's own criteria (order count / featured / discount) qualifies.
        // clearanceSale is eager-loaded here because the same product can appear in
        // several of these tabs at once — without it, the product-card partial's
        // getProductPriceByType() call lazy-loads clearanceSale once per product per
        // tab (N+1 across up to 4 overlapping sections).
        $solutionsBaseQuery = function () {
            return Product::active()->with(['clearanceSale' => function ($query) {
                return $query->active();
            }]);
        };

        $solutionsSections = Cache::remember(CACHE_HOME_SOLUTIONS_SECTIONS, CACHE_FOR_3_HOURS, function () use ($solutionsBaseQuery) {
            return [
                'bestsellers' => $solutionsBaseQuery()
                    ->withCount('orderDetails')
                    ->orderByDesc('order_details_count')
                    ->take(8)
                    ->get(),
                'featured' => $solutionsBaseQuery()
                    ->where('featured', 1)
                    ->orderByDesc('id')
                    ->take(12)
                    ->get(),
                'onSale' => $solutionsBaseQuery()
                    ->where('discount', '>', 0)
                    ->orderByDesc('id')
                    ->take(12)
                    ->get(),
                // Compare the effective discount percentage regardless of how the discount was entered
                // (Flat vs Percent), since a flat discount is only "50% off" if it's >= half the unit price.
                'halfOff' => $solutionsBaseQuery()
                    ->where('discount', '>', 0)
                    ->where(function ($query) {
                        $query->where(function ($q) {
                            $q->where('discount_type', 'percent')->where('discount', '>=', 50);
                        })->orWhere(function ($q) {
                            $q->where('discount_type', 'flat')
                                ->where('unit_price', '>', 0)
                                ->whereRaw('(discount / unit_price) * 100 >= 50');
                        });
                    })
                    ->orderByDesc('id')
                    ->take(12)
        ...
        $solutionsBestsellers = $solutionsSections['bestsellers'];
        $solutionsFeatured = $solutionsSections['featured'];
        $solutionsOnSale = $solutionsSections['onSale'];
        $solutionsHalfOffProducts = $solutionsSections['halfOff'];

        $latestProductsListTotalCount = $this->cacheHomePageLatestProductCount();
        ...
        return view(VIEW_FILE_NAMES['home'],
            compact(
                ...
                'current_date', 'recommendedProduct', 'bannerTypeFooterBanner', 'newArrivalProducts', 'clearanceSaleProducts','webinarsCatProduct',
                'PreBuiltWebsites','APIIntegrations','AddOns','featuredProducts','fullCategoryStructure', 'solutioncategoryNames',
                'latestProductsListTotalCount', 'solutionsBestsellers', 'solutionsFeatured', 'solutionsOnSale', 'solutionsHalfOffProducts'
            )
        );
    }

    public function loadMoreLatestProducts(Request $request): JsonResponse
    {
        $offset = max((int) $request->get('offset', 12), 0);
        $perPage = 12;

        $ids = array_slice($this->cacheHomePageDistinctLatestProductIds(), $offset, $perPage);

        $products = Product::whereIn('id', $ids)
            ->with(['seller.shop', 'flashDealProducts.flashDeal', 'reviews', 'clearanceSale' => function ($query) {
                return $query->active();
            }])
            ->orderBy('id', 'desc')
            ->get();
        ...
        $wishlistedProductIds = $userId ? Wishlist::where('customer_id', $userId)
            ...
            ->pluck('product_id')
            ->toArray() : [];
        $products->each(function ($product) use ($wishlistedProductIds) {
            $product->wish_list_count = in_array($product->id, $wishlistedProductIds) ? 1 : 0;
        });

        $html = '';
        foreach ($products as $product) {
            $html .= '<div class="col-lg-3 col-md-3 col-sm-4 col-6 p-2">'
                . view('web-views.partials._filter-single-product', ['product' => $product])->render()
                . '</div>';
        }
    ...
    public function loadMoreTopRated(Request $request): JsonResponse
    {
        $offset = max((int) $request->get('offset', 6), 0);
        $perBatch = 6;

        $products = Product::active()->with(['reviews', 'seller.shop', 'clearanceSale' => function ($query) {
            return $query->active();
        }])
            ...
            ->skip($offset)->take($perBatch)->get();

        $html = '';
        foreach ($products as $product) {
            $html .= view('web-views.partials._top-rated-single-item', ['product' => $product])->render();
        }

        return response()->json([
            'html' => $html,
            'hasMore' => $products->count() === $perBatch,
        ]);
    }

    public function loadMoreBestSelling(Request $request): JsonResponse
    {
        $offset = max((int) $request->get('offset', 6), 0);
        $perBatch = 6;
        ...
        $html = '';
        foreach ($products as $bestSellItem) {
            $html .= view('web-views.partials._best-selling-single-item', ['bestSellItem' => $bestSellItem])->render();
        }
        ...
        if (auth('customer')->check()) {
            ...
            if ($orders) {
                ...
                $justForYouProducts = $this->product->active()
                    ->whereHas('categories', fn ($query) => $query->whereIn('categories.id', $ids))
