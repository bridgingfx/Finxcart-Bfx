...
class HomeController extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        foreach ($latestProductsList as $latestProduct)
        {
            $latestProduct->subCategory = $latestProduct->subCategory()->first(); // manually fetch if not already loaded
        }
        ...
        $getCategorySectionData = function (?int $rootCategoryId, array $slugs = [], array $names = []): array {
            ...
            return Category::where('parent_id', $rootCategory->id)
                ->where('home_status', 1)
                ->get()
                ->map(function ($subcat) {
                    $products = Product::whereHas('categories', fn ($query) => $query->where('categories.id', $subcat->id))
                        ->active()
                        ->get();
        ...
        $webinarsCatProduct = $getCategorySectionData(261, ['webinars'], ['Webinars']);
        $PreBuiltWebsites = $getCategorySectionData(275, ['websites', 'pre-built-websites'], ['Websites', 'Pre Built Websites', 'PreBuiltWebsites']);
        $APIIntegrations = $getCategorySectionData(290, ['api-integrations', 'api-integration'], ['API Integrations', 'API Integration']);
        $AddOns = $getCategorySectionData(308, ['addons', 'add-ons'], ['AddOns', 'Add Ons']);
		$featuredProducts = Product::active()
            ...
            if ($productsCategory) {
                $productCatWithProducts = Category::where('parent_id', $productsCategory->id)
                    ->where('home_status', 1)
                    ->get()
                    ->map(function ($subcat) {
                        $products = Product::whereJsonContains('category_ids', [['id' => (string) $subcat->id]])
                            ->active()
                            ->get();
            ...
            $headCategoryIds = collect([
                CategoryManager::resolveRootCategory(categoryId: 7, slugs: ['home'], names: ['Home'])?->id,
                CategoryManager::resolveRootCategory(categoryId: 8, slugs: ['marketplace'], names: ['Marketplace'])?->id,
            ])->filter()->values()->all();

            $fullCategoryStructure = collect($headCategoryIds)->map(function ($headId) {

                $head = Category::find($headId);                  
                if (!$head) return null;                         
        ...
        $headSolutionCategoryIds = [6];

        $solutioncategoryNames = collect($headSolutionCategoryIds)->map(function ($SolutionheadId) {
        ...
        // scoped to the Solutions category tree (root + sub + sub-sub categories).
        $solutionsSubCategoryIds = Category::where('parent_id', $headSolutionCategoryIds[0])->pluck('id');
        $solutionsSubSubCategoryIds = Category::whereIn('parent_id', $solutionsSubCategoryIds)->pluck('id');
        $solutionsCategoryIds = collect($headSolutionCategoryIds)
            ->merge($solutionsSubCategoryIds)
            ->merge($solutionsSubSubCategoryIds)
            ->unique()
            ->values()
            ->all();

        $solutionsBaseQuery = function () use ($solutionsCategoryIds) {
            return Product::active()->where(function ($query) use ($solutionsCategoryIds) {
                $query->whereHas('categories', function ($categoryQuery) use ($solutionsCategoryIds) {
                    $categoryQuery->whereIn('categories.id', $solutionsCategoryIds);
                })
                    ->orWhereIn('category_id', $solutionsCategoryIds)
                    ->orWhereIn('sub_category_id', $solutionsCategoryIds)
                    ->orWhereIn('sub_sub_category_id', $solutionsCategoryIds);
            });
        };

        $solutionsBestsellers = $solutionsBaseQuery()
            ->withCount('orderDetails')
            ->orderByDesc('order_details_count')
            ->take(12)
            ->get();

        $solutionsFeatured = $solutionsBaseQuery()
            ->where('featured_status', 1)
            ->orderByDesc('id')
            ->take(12)
            ->get();

        $solutionsOnSale = $solutionsBaseQuery()
            ->where('discount', '>', 0)
            ->orderByDesc('id')
            ->take(12)
            ->get();

        $solutionsHalfOffProducts = $solutionsBaseQuery()
    ...
    public function loadMoreLatestProducts(Request $request): JsonResponse
    {
        ...
        $products = Product::whereIn('id', $ids)
            ->with(['seller.shop', 'flashDealProducts.flashDeal', 'clearanceSale' => function ($query) {
                return $query->active();
            }])
            ->orderBy('id', 'desc')
            ->get();
