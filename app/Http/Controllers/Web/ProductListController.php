...
class ProductListController extends Controller
{
    ...
    public function default_theme($request): View|JsonResponse|Redirector|RedirectResponse
    {
        ...
        if ($redirectResponse = $this->getMissingCategoryRedirectResponse($request)) {
        ...
        if ($request['data_from'] == 'category' && $request['category_id']) {
            $data['brand_name'] = Category::find((int)$request['category_id'])?->name ?? translate('category_not_found');
        }
        ...
        if ($request->ajax()) {
            ...
            return response()->json([
                ...
                'html_products' => view('web-views.products._ajax-products', compact('products', 'isInitialLoad'))->render()
            ], 200);
        }
    ...
    private function getMissingCategoryRedirectResponse(Request $request): JsonResponse|RedirectResponse|null
    {
        if ($request['data_from'] !== 'category') {
            return null;
        }

        $requestedCategoryId = (int)($request['sub_sub_category_id'] ?: $request['sub_category_id'] ?: $request['category_id']);

        if (!$requestedCategoryId) {
            return null;
        }

        if (Category::find($requestedCategoryId)) {
            return null;
        }

        $message = translate('category_not_found');
    ...
    public function getFlashDealsProducts(Request $request): JsonResponse
    {
        ...
        $currentPage = $request->get('page') ?? Paginator::resolveCurrentPage('page');
