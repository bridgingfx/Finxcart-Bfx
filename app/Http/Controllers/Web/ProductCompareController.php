...
class ProductCompareController extends BaseController
{
     ...
     */
    public function getProductCompareListView(): View|RedirectResponse
    {
        // 'account_compare_list' has no dedicated view in the currently active theme
        // (only theme_aster ships one, and the default theme has no UI entry point
        // linking here at all) — redirect home rather than a fatal "View [] not
        // found" error for anyone who reaches this URL directly.
        if (empty(VIEW_FILE_NAMES['account_compare_list'])) {
        ...
        $compareLists = $this->productCompareRepo->getListWhere(
            ...
            relations: ['product', 'product.reviews', 'product.brand', 'product.category', 'product.digitalVariation'],
            dataLimit: 'all'
        );
        $attributes = [];
