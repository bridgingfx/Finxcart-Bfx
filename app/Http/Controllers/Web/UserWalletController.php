...
class UserWalletController extends Controller
{
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        if ($walletStatus == 1) {
            ...
            $totalCredit = WalletTransaction::where('user_id', $customerId)->sum('credit');
            $totalDebit  = WalletTransaction::where('user_id', $customerId)->sum('debit');
    ...
    {
        // 'wallet_account' has no view in either shipped theme (file_names.php maps
        // it to '' in both 'default' and 'theme_aster' — it was apparently only ever
        // built for a 'theme fashion' theme that isn't present in this codebase).
        // Redirect to the real wallet page instead of a fatal "View [] not found" error.
        if (empty(VIEW_FILE_NAMES['wallet_account'])) {
            return redirect()->route('wallet');
        }
