...
class   EmailTemplatesController extends BaseController
{
    ...
    {
        if (($request->hasFile('logo') && !$this->isWebpOrPng($request->file('logo')))
            || ($request->hasFile('icon') && !$this->isWebpOrPng($request->file('icon')))) {
            ToastMagic::error(translate('only_webp_and_png_images_are_allowed_for_the_header_and_footer_icon') . '!');
    ...
    private function isWebpOrPng($file): bool
    {
        $extension = strtolower($file?->getClientOriginalExtension() ?? '');
        return in_array($extension, ['webp', 'png'], true);
    }
