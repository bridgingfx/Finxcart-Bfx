...
class TransactionReportController extends Controller
{
    ...
    public function order_transaction_list(Request $request)
    {
        ...
        // The filter dropdown renders $seller->shop->name per row; eager-load to avoid
        // one `shops` query per seller (N+1).
        $sellers = Seller::approved()->with('shop')->get();
...
//        $totalStores = self::date_wise_common_filter($totalStoresQuery, $date_type, $from, $to)->count();
    ...
    public function getExpenseTransactionList(Request $request)
    {
        $search = $request['search'];
        $from = $request['from'];
        $to = $request['to'];
    ...
    public function commissionLedgerList(Request $request)
    {
        ...
        $service_charge_setting = (float) (getWebConfig(name: 'commission_service_charge_amount') ?? 0);

        $commission_ledgers = $filteredLedgerQuery->latest('updated_at')
            ->paginate(Helpers::pagination_limit())
            ->appends($query_param);

        return view('admin-views.transaction.commission-ledger', compact(
            'commission_ledgers',
            'sellers',
            'search',
            'from',
    ...
    public function commissionLedgerExportExcel(Request $request): BinaryFileResponse
    {
        ...
        $data = array_merge($summary, [
            ...
            'to' => $request['to'],
            'dateType' => $request['date_type'] ?? 'this_year',
            'seller' => $sellerId != 'all' ? Seller::with('shop')->find($sellerId) : 'all',
            'ledgers' => $ledgers,
        ]);
    ...
    public function commission_ledger_summary_pdf(Request $request)
    {
        ...
        $to = $request['to'];
        $sellerId = $request['seller_id'] ?? 'all';
        $sellerInfo = translate('all');

        if ($sellerId != 'all') {
            $seller = Seller::with('shop')->find($sellerId);
            $sellerInfo = $seller?->shop?->name ?? trim(($seller?->f_name ?? '') . ' ' . ($seller?->l_name ?? ''));
        }
        ...
        $data = [
            ...
            'company_email' => BusinessSetting::where('type', 'company_email')->first()?->value,
            'duration' => $duration,
            'seller_info' => $sellerInfo ?: translate('not_available'),
        ];

        $mpdf_view = View::make('admin-views.transaction.commission_ledger_summary_report_pdf', compact('data'));
        Helpers::gen_mpdf($mpdf_view, 'commission_ledger_summary_report_', $dateType);
    }

    public function updateCommissionServiceCharge(Request $request)
    {
        $request->validate([
            'commission_service_charge_amount' => 'required|numeric|min:0|max:999999|regex:/^\d+(\.\d+)?$/',
        ]);

        BusinessSetting::updateOrCreate(
            ['type' => 'commission_service_charge_amount'],
            ['value' => (string) $request->commission_service_charge_amount]
        );
    ...
    private function commissionLedgerFilteredQuery(Request $request)
    {
        ...
        $ledgerQuery = CommissionLedger::with(['seller.shop', 'order.orderTransaction'])
