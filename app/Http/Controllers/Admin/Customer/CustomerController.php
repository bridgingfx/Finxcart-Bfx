...
class CustomerController extends BaseController
{
    ...
    public function getListView(Request $request): View|RedirectResponse
    {
        $filters = [
            ...
            // The list view only shows an order count per customer, not order rows —
            // withCount() avoids hydrating every column of every order just to count.
            'withCount' => ['orders'],
        ];
        ...
        $customers = $this->customerRepo->getListWhereBetween(
            ...
            filters: $filters,
            // The list view renders $customer->image_full_url per row, which lazy-loads the
            // `storage` relation once per row without this eager load (N+1).
            relations: ['storage'],
    ...
    {
        $customer = $this->customerRepo->getFirstWhere(params: ['id' => $id], relations: ['addresses', 'refundOrders']);
        if (isset($customer)) {
            ...
            $orderStatusArray = [
                ...
                'ongoing' => $statusCounts->only(['pending', 'confirmed', 'processing', 'out_for_delivery'])->sum(),
    ...
    public function getUpdateView($id): View|RedirectResponse
    {
        ...
        if (!$customer || $customer['email'] === 'walking@customer.com') {
            ToastMagic::error(translate('customer_Not_Found'));
            return back();
        }
        return view(Customer::EDIT[VIEW], compact('customer'));
    }

    public function updateCustomer(CustomerUpdateRequest $request, $id, CustomerService $customerService): RedirectResponse
    {
        $customer = $this->customerRepo->getFirstWhere(params: ['id' => $id]);
        // The walking-customer record is a system placeholder used to attribute
        // POS/walk-in orders (matched by this exact email elsewhere in the app,
        // e.g. CustomerRepository/OrderRepository) — editing it here (especially
        // its email) would silently break that matching.
        if (!$customer || $customer['email'] === 'walking@customer.com') {
            ToastMagic::error(translate('customer_Not_Found'));
            return back();
        }
        $this->customerRepo->update(id: $id, data: $customerService->getUpdateData(request: $request, customer: $customer));
        ToastMagic::success(translate('customer_updated_successfully'));
        return redirect()->route('admin.customer.view', $id);
    }
    ...
    public function delete($id, CustomerService $customerService): RedirectResponse
    {
        ...
        try {
            $customerService->deleteWithRelatedData(customer: $customer);
            ToastMagic::success(translate('customer_deleted_successfully'));
        } catch (\Throwable $e) {
            \Log::error('[CustomerController] Customer delete failed: ' . $e->getMessage(), ['customer_id' => $id]);
