...
class LanguageController extends BaseController
{
    ...
    public function add(LanguageRequest $request, LanguageService $languageService): RedirectResponse
    {
        ...
        $this->syncPncLanguage(languageArray: $dataArray['languages']);
    ...
    public function updateStatus(Request $request, LanguageService $languageService): JsonResponse
    {
        ...
        $this->syncPncLanguage(languageArray: $languageArray);
        clearWebConfigCacheKeys();
        updateSetupGuideCacheKey(key: 'language_setup');
    ...
    public function deleteTranslateKey(Request $request, $lang): void
    {
        ...
        file_put_contents(
            base_path('resources/lang/' . $lang . '/messages.php'),
            LanguageService::buildLanguagePhpFile($fullData)
        );
    }
    ...
    public function updateTranslate(Request $request, $lang): JsonResponse
    {
        ...
        if (array_key_exists($textKey, $translatedMessagesArray)) {
            ...
            file_put_contents(
                base_path('resources/lang/' . $lang . '/messages.php'),
                LanguageService::buildLanguagePhpFile($dataFiltered)
            );
            ...
            $dataFiltered[base64_decode($request['key'])] = removeSpecialCharacters($request['value']);
            file_put_contents(
                base_path('resources/lang/' . $lang . '/new-messages.php'),
                LanguageService::buildLanguagePhpFile($dataFiltered)
            );
        }
    ...
    private function syncPncLanguage(array $languageArray): void
    {
        $languageCodes = collect($languageArray)
            ->filter(function ($language) {
                return ($language['status'] ?? 0) == 1 || ($language['code'] ?? null) === 'en';
            })
            ->pluck('code')
            ->filter()
            ->unique()
        ...
        if (($key = array_search('en', $languageCodes, true)) !== false) {
            unset($languageCodes[$key]);
        }

        array_unshift($languageCodes, 'en');
        $this->businessSettingRepo->updateOrInsert(type: 'pnc_language', value: json_encode(array_values($languageCodes)));
    }
