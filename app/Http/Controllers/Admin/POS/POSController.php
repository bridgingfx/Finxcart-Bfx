...
class POSController extends BaseController
{
    ...
    public function updateDiscount(Request $request): JsonResponse
    {
        ...
        if ($cart) {
            ...
            $cartProductIds = collect($cart)->filter(fn($item) => is_array($item))->pluck('id')->unique()->values()->all();
            $cartProducts = $this->productRepo->getListWhere(
                filters: ['productIds' => $cartProductIds],
                relations: ['clearanceSale' => function ($query) {
                    return $query->active();
                }],
                dataLimit: 'all'
            ...
            foreach ($cart as $item) {
                if (is_array($item)) {
                    $product = $cartProducts->get($item['id']);
    ...
    public function getCouponDiscount(Request $request): JsonResponse
    {
        ...
        if (($coupon['customer_id'] == '0' || $coupon['customer_id'] == $userId)) {
            if ($carts != null) {
                $couponCartProductIds = collect($carts)->filter(fn($cartItem) => is_array($cartItem))->pluck('id')->unique()->values()->all();
                $couponCartProducts = $this->productRepo->getListWhere(
                    filters: ['productIds' => $couponCartProductIds],
                    relations: ['clearanceSale' => function ($query) {
                        return $query->active();
                    }],
                    dataLimit: 'all'
                ...
                foreach ($carts as $cart) {
                    if (is_array($cart)) {
                        $product = $couponCartProducts->get($cart['id']);
    ...
    protected function calculateCartItemsData(string $cartName, array $customerCartData): array
    {
        ...
        if (session()->get($cartName)) {
            $sessionCart = session()->get($cartName);
            $cartProductIds = collect($sessionCart)->filter(fn($cartItem) => is_array($cartItem))->pluck('id')->unique()->values()->all();
            $cartProducts = $this->productRepo->getListWhere(
                filters: ['productIds' => $cartProductIds],
                relations: ['clearanceSale' => function ($query) {
                    return $query->active();
