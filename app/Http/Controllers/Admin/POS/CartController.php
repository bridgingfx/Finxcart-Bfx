...
use App\Enums\ViewPaths\Admin\Cart as CartViewPaths;
use App\Enums\ViewPaths\Admin\POS as POSViewPaths;
...
class CartController extends Controller
{
    ...
    /*
     |--------------------------------------------------------------------
     | POS session-cart methods (ported from Vendor\POS\CartController)
     |--------------------------------------------------------------------
     | These power the Admin POS "multiple held carts" UI (Hold Order,
     | New Cart, Switch Cart, Clear Cart, Remove item). They use the
     | session-based CartService/POSService architecture, not the
     | Cart Eloquent model used by the storefront-cart methods above.
     */
    ...
    public function removeCart(Request $request): JsonResponse
    {
        ...
        return response()->json(
            ['view' => view(CartViewPaths::CART[VIEW], compact('cartId', 'cartItems'))->render()]
        );
    }
    ...
    public function clearSessionCartIds(): RedirectResponse
    {
        ...
        return redirect()->route(POSViewPaths::INDEX[ROUTE]);
    }
    ...
    public function getCartIds(): JsonResponse
    {
        ...
        return response()->json([
            'view' => view(CartViewPaths::SUMMARY[VIEW], compact('summaryData', 'cartItems'))->render(),
        ]);
    }
