...
class DashboardController extends BaseController
{
    ...
    public function dashboard(): View
    {
        ...
        $topSellProduct = $this->productRepo->getTopSellList(dataLimit: DASHBOARD_TOP_SELL_DATA_LIMIT);
        $topCustomer = $this->orderRepo->getTopCustomerListLimited(relations: ['customer:id,f_name,l_name,image'], limit: DASHBOARD_DATA_LIMIT);
        ...
        $topVendorByOrderReceived = $this->orderRepo->getTopVendorListByOrderReceivedLimited(relations: ['seller.shop:id,seller_id,name,image,image_storage_type'], limit: DASHBOARD_DATA_LIMIT);

        $data = self::getOrderStatusData();
        $admin_wallet = $this->adminWalletRepo->getFirstWhere(params: ['admin_id' => (int) env('PRIMARY_ADMIN_ID', 1)]);
        ...
        $data += [
            ...
            'getTotalCustomerCount' => User::whereNot('email', 'walking@customer.com')->count(),
            'getTotalVendorCount' => Seller::count(),
            'getTotalDeliveryManCount' => DeliveryMan::where('seller_id', 0)->count(),
        ];
    ...
    {
        $today      = session('statistics_type') === 'today';
        $thisMonth  = session('statistics_type') === 'this_month';

        $applyDate = function ($query) use ($today, $thisMonth) {
            if ($today) {
        ...
        $statusCounts = $applyDate(Order::query())
            ->selectRaw('order_status, COUNT(*) as total')
            ->groupBy('order_status')
            ->pluck('total', 'order_status');

        return [
            'order'            => $applyDate(Order::query())->count(),
            'store'            => $applyDate(Seller::query())->count(),
            'product'          => $applyDate(Product::query())->count(),
            'customer'         => $applyDate(User::query())->whereNot('email', 'walking@customer.com')->count(),
            ...
            'out_for_delivery' => $statusCounts->get('out_for_delivery', 0),
        ];
    }
    ...
    {
        // Fires on every admin page load (base layout, not just the dashboard) —
        // a direct SQL count instead of fetching every unchecked order row.
        $newOrder = Order::where('checked', 0)->count();

        // Eager-load 'product' up front so the single-product branch below can
        // reuse this result instead of re-running the same filtered query.
        $restockProductList = $this->restockProductRepo->getListWhere(filters: ['added_by' => 'in_house'], relations: ['product'], dataLimit: 'all')->groupBy('product_id');
        $restockProduct = [];
        if (count($restockProductList) == 1) {
            $products = $restockProductList->first();
