...
class ShippingMethodController extends BaseController
{
    ...
    {
        return $this->getShippingMethodsView($request);
    }
     ...
     * @return View
     */
    public function getShippingMethodsView($request): View
    {
        ...
        // Backfill missing category_shipping_costs rows for any category added since the
        // last visit. Was previously an N-query loop (one `in_array` + one INSERT per
        // category on every page load); now a single diff + single bulk insert.
        $missingCategoryIds = array_diff($allCategoryIds, $allCategoryShippingCostArray);
        if (!empty($missingCategoryIds)) {
            $this->categoryShippingCostRepo->insert(
                data: array_map(
                    fn($id) => $this->categoryShippingCostService->getAddCategoryWiseShippingCostData(addedBy: 'admin', id: $id),
                    $missingCategoryIds
                )
            );
        }
    ...
    public function add(ShippingMethodRequest $request): RedirectResponse
    {
        ...
        updateSetupGuideCacheKey(key: 'shipping_method');
