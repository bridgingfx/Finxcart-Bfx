...
class OrderController extends BaseController
{
    ...
    public function getListView(object $request, string $status): View|JsonResponse
    {
        ...
        // The AJAX branch below serializes $orders wholesale as JSON with an
        // undocumented consumer contract, so it keeps the full row (no select) —
        // narrowing only applies to the normal view-rendering path.
        $orders = $this->orderRepo->getListWhere(
            orderBy: ['id' => 'desc'],
            searchValue: $request['searchValue'],
            filters: $filters,
            relations: ['customer', 'seller.shop', 'details'],
    ...
    /**
     * Deletes a single order and every row that references it (order_details,
     * transactions, shipments, refund requests, status histories, etc.) —
     * the same per-order cleanup used by customer/vendor cascade deletes,
     * scoped to just this one order_id instead of a whole customer/seller.
     */
    public function destroy(Request $request): JsonResponse
    {
        ...
        if (!$order) {
            return response()->json(['message' => translate('order_not_found_It_may_be_deleted')], 404);
        }

        DB::transaction(function () use ($order) {
            $orderId = $order->id;

            $orderDetailIds = DB::table('order_details')->where('order_id', $orderId)->pluck('id');
            DB::table('shipment_items')->whereIn('order_detail_id', $orderDetailIds)->delete();
            DB::table('shipments')->where('order_id', $orderId)->orWhere('source_order_id', $orderId)->delete();

            DB::table('order_status_histories')->where('order_id', $orderId)->delete();
            DB::table('order_delivery_verifications')->where('order_id', $orderId)->delete();
            DB::table('order_expected_delivery_histories')->where('order_id', $orderId)->delete();
            DB::table('offline_payments')->where('order_id', $orderId)->delete();
            DB::table('deliveryman_notifications')->where('order_id', $orderId)->delete();
            DB::table('admin_wallet_histories')->where('order_id', $orderId)->delete();
            DB::table('commission_ledger')->where('order_id', $orderId)->delete();
            DB::table('transactions')->where('order_id', $orderId)->delete();
            DB::table('refund_transactions')->where('order_id', $orderId)->delete();
            DB::table('order_transactions')->where('order_id', $orderId)->delete();
            DB::table('refund_requests')->where('order_id', $orderId)->delete();
            DB::table('order_details')->where('order_id', $orderId)->delete();

            $order->delete();
        });

        return response()->json(['message' => translate('order_deleted_successfully')]);
    }
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        $order = $this->orderRepo->getFirstWhere(params: ['id' => $id], relations: ['details.productAllStatus', 'details.shipmentItem', 'details.product', 'verificationImages', 'shipping', 'seller.shop', 'offlinePayments', 'deliveryMan', 'sourceShipment']);
        ...
        if ($order) {
            ...
            $orderStatusTransitions = [
                ...
                'processing' => $shippingMethod == 'sellerwise_shipping' ? ['out_for_delivery', 'canceled'] : ['delivered', 'canceled'],
            ...
            if ($order['order_type'] == 'default_type') {
                ...
                return view(Order::VIEW[VIEW], compact('order', 'linkedOrders',
                    ...
                    'currentOrderStatus', 'orderStatusLabels', 'availableOrderStatuses'));
            } else {
    ...
    ): JsonResponse
    {
        ...
        DB::transaction(function () use ($request, $order, $deliveryManTransactionService, $deliveryManWalletService, $orderStatusHistoryService, &$skipWalletUpdate) {
        ...
        app(OrderService::class)->syncPhysicalShipmentStatus(sourceOrder: $order, newStatus: $request['order_status']);
    ...
    public function uploadDigitalFileAfterSell(UploadDigitalFileAfterSellRequest $request): RedirectResponse
    {
        ...
        if ($this->orderDetailRepo->update(id: $orderDetails['id'], data: ['digital_file_after_sell' => $digitalFileAfterSell])) {
            app(OrderService::class)->markDigitalItemDelivered(orderDetailId: $orderDetails['id']);
