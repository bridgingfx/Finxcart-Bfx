...
class CouponController extends BaseController
{
    ...
    {
        $sellers = $this->vendorRepo->getListWhere(filters: ['status' => 'approved'], relations: ['shop:id,seller_id,name'], dataLimit: 'all', select: ['id', 'f_name', 'email']);
        $customers = $this->customerRepo->getListWhereNotIn([0]);
    ...
    public function getVendorList(Request $request): JsonResponse
    {
        ...
        foreach ($sellers as $seller) {
            $output .= '<option value="' . $seller->id . '">' . ($seller->shop->name ?? $seller->f_name) . '</option>';
        }
