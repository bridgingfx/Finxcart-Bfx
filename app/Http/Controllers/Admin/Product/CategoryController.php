...
class CategoryController extends BaseController
{
    ...
    public function updateStatus(Request $request, CategoryService $categoryService): JsonResponse
    {
        $category = $this->categoryRepo->getFirstWhere(params: ['id' => $request['id']], relations: ['childes.childes']);
        $homeStatus = (int)$request->get('home_status', 0);
        if ($category) {
            $categoryService->syncCategoryTreeHomeStatus(category: $category, homeStatus: $homeStatus);
        }
        updateSetupGuideCacheKey(key: 'category_setup');
    ...
    public function delete(Request $request, CategoryService $categoryService): JsonResponse
    {
        $category = $this->categoryRepo->getFirstWhere(params: ['id' => $request['id']], relations: ['childes.childes']);
        if ($category) {
            $categoryService->deleteCategoryTree(category: $category);
        }
    ...
    public function getDeleteInfo(Request $request): JsonResponse
{
    ...
    if (!$category) {
        return response()->json(['subCount' => 0, 'subSubCount' => 0, 'productCount' => 0, 'position' => 0]);
    }
    ...
    if ($position == 0) {
        $subCount = $category->childes->count();
        foreach ($category->childes as $sub) {
            $subSubCount += $sub->childes->count();
        }
    } elseif ($position == 1) {
        $subSubCount = $category->childes->count();
    }

    $productCount = \App\Models\Product::whereHas('categories', fn ($query) => $query->where('categories.id', $category->id))->count();
