...
use App\Exports\CommissionLedgerReportExport;
...
class TransactionReportController extends Controller
{
    ...
    public function pdf_order_wise_transaction(Request $request)
    {
        ...
        $transaction = OrderTransaction::with(['seller.shop', 'customer', 'order.coupon', 'orderDetails'])
    ...
    public function order_transaction_table_data_filter($request)
    {
        ...
        $transaction_query = OrderTransaction::with(['seller.shop', 'customer', 'order.deliveryMan', 'order.coupon'])
    ...
    public function expense_transaction_list(Request $request)
    {
        ...
        $expense_calculate_query = self::expense_transaction_common_query();
        ...
        if ($expense_calculate) {
            foreach ($expense_calculate as $calculate) {
                $order_commission = $calculate->admin_commission ?? 0;
                $total_expense += ($calculate->coupon_discount_bearer == 'seller' ? $calculate->discount_amount : 0) + ($calculate->free_delivery_bearer == 'seller' ? $calculate->extra_discount : 0) + $order_commission;
                $commission_expense += $order_commission;
        ...
        $expense_transaction_query = self::expense_transaction_common_query($search);
        ...
        return view('vendor-views.transaction.expense-list', compact('expense_transactions_table', 'expense_transaction_chart',
            'search', 'from', 'to', 'date_type', 'total_expense', 'free_delivery', 'coupon_discount', 'commission_expense'));
    }

    public function commission_ledger_list(Request $request)
    {
        $search = $request['search'];
        $from = $request['from'];
        $to = $request['to'];
        ...
        $filteredLedgerQuery = $this->commissionLedgerFilteredQuery($request);
        $summaryLedger = (clone $filteredLedgerQuery)->get();

        $total_gross = $summaryLedger->sum('gross_amount');
        $total_platform_commission = $summaryLedger->sum('company_share_amount');
        $total_service_charge = $summaryLedger->sum('service_charge_amount');
        $total_net_payout = $summaryLedger->sum('net_vendor_payout');

        $commission_ledgers = $filteredLedgerQuery->latest('updated_at')
            ->paginate(Helpers::pagination_limit())
            ->appends($query_param);

        return view('vendor-views.transaction.commission-ledger', compact(
            'commission_ledgers',
            'search',
            'from',
            'to',
    ...
    public function commissionLedgerExportExcel(Request $request): BinaryFileResponse
    {
        $vendorId = auth('seller')->id();
        $vendor = $this->vendorRepo->getFirstWhere(params: ['id' => $vendorId]);
        $ledgers = $this->commissionLedgerFilteredQuery($request)->latest('updated_at')->get();
        $summary = $this->commissionLedgerSummary($ledgers);
        ...
        return Excel::download(new CommissionLedgerReportExport($data), Report::COMMISSION_LEDGER_REPORT_LIST);
    }

    public function commission_ledger_summary_pdf(Request $request)
    {
        $dateType = $request['date_type'] ?? 'this_year';
        $from = $request['from'];
        $to = $request['to'];
        $duration = $dateType == 'custom_date' ? 'From ' . $from . ' To ' . $to : str_replace('_', ' ', $dateType);
        $shopName = Shop::where(['seller_id' => auth('seller')->id()])->first()?->name ?? translate('not_available');
        ...
        $data = [
            'summary' => [
                ['label' => translate('gross_Amount'), 'amount' => $summary['total_gross']],
                ['label' => translate('platform_commission'), 'amount' => $summary['total_platform_commission']],
                ['label' => translate('service_Charge'), 'amount' => $summary['total_service_charge']],
                ['label' => translate('net_Vendor_Payout'), 'amount' => $summary['total_net_payout']],
            ],
            'company_phone' => BusinessSetting::where('type', 'company_phone')->first()?->value,
            'company_name' => BusinessSetting::where('type', 'company_name')->first()?->value,
            'company_email' => BusinessSetting::where('type', 'company_email')->first()?->value,
        ...
        $mpdf_view = View::make('vendor-views.transaction.commission_ledger_summary_report_pdf', compact('data'));
        Helpers::gen_mpdf($mpdf_view, 'commission_ledger_summary_report_', $dateType);
    }

    private function commissionLedgerFilteredQuery(Request $request)
    {
        $search = $request['search'];
        $from = $request['from'];
        $to = $request['to'];
        ...
        $ledgerQuery = CommissionLedger::with(['order.orderTransaction'])
            ->where('seller_id', auth('seller')->id())
            ->when($search, function ($query) use ($search) {
                $query->where(function ($q) use ($search) {
                    $q->where('order_id', 'like', "%{$search}%")
                        ->orWhere('reference_id', 'like', "%{$search}%")
                        ->orWhereHas('order.orderTransaction', function ($transactionQuery) use ($search) {
                            $transactionQuery->where('transaction_id', 'like', "%{$search}%");
                        });
                });
            });

        return $this->date_wise_common_filter($ledgerQuery, $dateType, $from, $to);
    }

    private function commissionLedgerSummary($ledgers): array
    {
        return [
            'total_gross' => $ledgers->sum('gross_amount'),
            'total_platform_commission' => $ledgers->sum('company_share_amount'),
            'total_service_charge' => $ledgers->sum('service_charge_amount'),
            'total_net_payout' => $ledgers->sum('net_vendor_payout'),
        ];
    }

    public function expense_transaction_common_query($search = null, $vendorId = null)
    {
        return Order::with(['orderTransaction', 'coupon'])
            ->where([
                ...
                'seller_is' => 'seller',
                'seller_id' => $vendorId ?? auth('seller')->id(),
            ])
            ->where(function ($query) {
                $query->where('admin_commission', '>', 0)
    ...
    public function expense_transaction_same_year($request, $start_date, $end_date, $from_year, $number, $default_inc)
    {
        ...
        $orders = self::expense_chart_common_query($request)
            ->selectRaw('sum((CASE WHEN coupon_discount_bearer="seller" THEN discount_amount ELSE 0 END) + (CASE WHEN free_delivery_bearer="seller" THEN extra_discount ELSE 0 END) + COALESCE(admin_commission, 0)) as discount_amount, YEAR(updated_at) year, MONTH(updated_at) month')
            ->groupBy(DB::raw("DATE_FORMAT(updated_at, '%M')"))
            ->latest('updated_at')->get();
    ...
    public function expense_transaction_same_month($request, $start_date, $end_date, $month_date, $number, $default_inc)
    {
        ...
        $month = substr(date("F", strtotime("$year_month")), 0, 3);
        $orders = self::expense_chart_common_query($request)
            ->selectRaw('sum((CASE WHEN coupon_discount_bearer="seller" THEN discount_amount ELSE 0 END) + (CASE WHEN free_delivery_bearer="seller" THEN extra_discount ELSE 0 END) + COALESCE(admin_commission, 0)) as discount_amount, YEAR(updated_at) year, MONTH(updated_at) month, DAY(updated_at) day')
            ->groupBy(DB::raw("DATE_FORMAT(updated_at, '%D')"))
            ->latest('updated_at')->get();
    ...
    public function expense_transaction_this_week($request)
    {
        ...
        $orders = self::expense_chart_common_query($request)
            ->select(
                DB::raw('sum((CASE WHEN coupon_discount_bearer="seller" THEN discount_amount ELSE 0 END) + (CASE WHEN free_delivery_bearer="seller" THEN extra_discount ELSE 0 END) + COALESCE(admin_commission, 0)) as discount_amount'),
                DB::raw("(DATE_FORMAT(updated_at, '%W')) as day")
            )
            ->groupBy(DB::raw("DATE_FORMAT(updated_at, '%D')"))
            ->latest('updated_at')->get();
    ...
    {
        $orders = self::expense_chart_common_query($request)
            ->selectRaw('sum((CASE WHEN coupon_discount_bearer="seller" THEN discount_amount ELSE 0 END) + (CASE WHEN free_delivery_bearer="seller" THEN extra_discount ELSE 0 END) + COALESCE(admin_commission, 0)) as discount_amount, YEAR(updated_at) year')
            ->groupBy(DB::raw("DATE_FORMAT(updated_at, '%Y')"))
            ->latest('updated_at')->get();
    ...
    public function expense_chart_common_query($request)
    {
        ...
        $order_query = self::expense_transaction_common_query();
    ...
    public function expense_transaction_summary_pdf(Request $request)
    {
        ...
        if ($expense_transactions) {
            foreach ($expense_transactions as $transaction) {
                $order_commission = $transaction->admin_commission ?? 0;
                $total_expense += ($transaction->coupon_discount_bearer == 'seller' ? $transaction->discount_amount : 0) + ($transaction->free_delivery_bearer == 'seller' ? $transaction->extra_discount : 0) + $order_commission;
                $commission_expense += $order_commission;
    ...
    public function expenseTransactionExportExcel(Request $request): BinaryFileResponse
    {
        ...
        $vendorId = auth('seller')->id();
        $vendor = $this->vendorRepo->getFirstWhere(params: ['id' => $vendorId]);
        $expense_transaction_query = self::expense_transaction_common_query($search, $vendorId);
