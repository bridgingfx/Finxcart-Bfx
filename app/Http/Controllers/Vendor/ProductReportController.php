...
class ProductReportController extends Controller
{
    ...
    public function all_product(Request $request){
        ...
        $product_query = Product::with(['reviews', 'rating'])
        ...
        // The 3 status counts below share the same base filter (user_id/added_by
        // + date range) and differ only by request_status, so they're collapsed
        // into a single conditional-aggregation query instead of 3 round-trips.
        $product_status_query = Product::where(['user_id' => $seller_id, 'added_by' => 'seller']);
        // ->toBase() drops down to the plain query builder so the result is a stdClass,
        // not a hydrated Product model — otherwise Product's 'translate' global scope
        // (see Product::booted()) would silently attach ->with(['translations','reviews'])
        // and fire 2 extra queries just to build a model we never actually use.
        $status_counts = self::create_date_wise_common_filter($product_status_query, $date_type, $from, $to)
            ->toBase()
            ->selectRaw("
                SUM(CASE WHEN request_status = 2 THEN 1 ELSE 0 END) as reject_product_count,
                SUM(CASE WHEN request_status = 0 THEN 1 ELSE 0 END) as pending_product_count,
                SUM(CASE WHEN request_status = 1 THEN 1 ELSE 0 END) as active_product_count
            ")->first();

        $product_count = array(
            'reject_product_count'=> (int) $status_counts->reject_product_count,
            'active_product_count'=> (int) $status_counts->active_product_count,
            'pending_product_count'=> (int) $status_counts->pending_product_count
        );
