...
class ClearanceSaleController extends BaseController
{
    ...
    {
        $condition = ['status' => true, 'message' => 'product_added_successfully'];
        ...
        $existingProductIds = StockClearanceProduct::whereIn('product_id', $request['productIds'])
            ->pluck('product_id')
            ->all();

        // Validated in a single pass so a failing product rejects the whole
        // batch before any row is written — matches the prior two-pass
        // behavior, without re-querying/re-validating every product twice.
        foreach ($clearanceProductLists as $clearanceProductList) {
            if (in_array($clearanceProductList->id, $existingProductIds, true)) {
                continue;
            }
