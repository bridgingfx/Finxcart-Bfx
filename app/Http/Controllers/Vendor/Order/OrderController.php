...
class OrderController extends BaseController
{
    ...
    public function getListView(object $request): View
    {
        ...
        $from = $request['from'];
        $to = $request['to'];
        // Route param /{status} takes priority; fall back to query param then 'all'
        ...
        $relation = ['customer', 'shipping', 'shippingAddress', 'deliveryMan', 'billingAddress', 'details'];
        $filters = $this->buildVendorOrderFilters(request: $request, status: $status, vendorId: $vendorId);
        $orders = $this->orderRepo->getListWhere(
            ...
            select: [
                'id', 'customer_id', 'is_guest', 'seller_id', 'seller_is', 'payment_status',
                'order_status', 'payment_method', 'order_type', 'parent_order_id', 'child_number',
                'created_at', 'shipping_cost', 'extra_discount_type', 'extra_discount',
                'discount_amount', 'is_shipping_free', 'paid_amount',
                'shipping_method_id', 'shipping_address', 'delivery_man_id', 'billing_address',
            ],
        );
        $sellers = $this->vendorRepo->getByStatusExcept(status: 'pending', relations: ['shop:id,seller_id,name'], paginateBy: 'all', select: ['id', 'f_name', 'email']);
    ...
    public function exportList(Request $request, $status): BinaryFileResponse|RedirectResponse
    {
        ...
        $orders = $this->orderRepo->getListWhere(orderBy: ['id' => 'desc'], searchValue: $request['searchValue'], filters: $filters, relations: ['customer:id,f_name,l_name', 'seller:id', 'seller.shop:id,seller_id,name', 'orderDetails:id,order_id,qty,price,tax,discount,tax_model'], dataLimit: 'all');
    ...
    private function buildVendorOrderFilters(Request $request, string $status, int $vendorId): array
    {
        $filters = [
            'order_status' => $status,
            'date_type' => $request['date_type'],
        ...
        if (in_array($selectedFilter, ['default_type', 'POS'], true)) {
            $filters['order_type'] = $selectedFilter;
        } elseif (in_array($selectedFilter, ['admin', 'seller'], true)) {
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        $params = ['id' => $id, 'seller_id' => $vendorId, 'seller_is' => 'seller'];
        $relations = ['deliveryMan', 'verificationImages', 'details.shipmentItem', 'customer', 'shipping', 'offlinePayments', 'sourceShipment'];
        $order = $this->orderRepo->getFirstWhere(params: $params, relations: $relations);
    ...
    ): JsonResponse
    {
        $order = $this->orderRepo->getFirstWhere(
            params: ['id' => $request['id'], 'seller_id' => auth('seller')->id(), 'seller_is' => 'seller'],
            relations: ['customer', 'seller.shop', 'deliveryMan']
        );

        if (!$order) {
            return response()->json(['success' => 0, 'message' => translate('order_not_found')], 200);
        }

        $requestedStatus = $request['order_status'];
        $allowedStatuses = $this->getVendorAllowedOrderStatuses(
            currentStatus: $order['order_status'],
            sellerwiseShipping: getWebConfig(name: 'shipping_method') == 'sellerwise_shipping'
        );

        if (!in_array($requestedStatus, $allowedStatuses, true)) {
            return response()->json([
                'success' => 0,
                'message' => translate('invalid_order_status_transition'),
            ], 200);
        }
        ...
        if ($order['payment_method'] != 'cash_on_delivery' && $requestedStatus == 'delivered' && $order['payment_status'] != 'paid') {
        ...
        DB::transaction(function () use ($request, $requestedStatus, $order) {
            $this->orderRepo->updateStockOnOrderStatusChange($request['id'], $requestedStatus);
            $this->orderRepo->update(id: $request['id'], data: ['order_status' => $requestedStatus]);
            if ($requestedStatus == 'delivered') {
                $this->orderRepo->update(id: $request['id'], data: ['payment_status' => 'paid', 'is_pause' => 0]);
                $this->orderDetailRepo->updateWhere(params: ['order_id' => $order['id']], data: ['delivery_status' => $requestedStatus, 'payment_status' => 'paid']);
            }

            if ($requestedStatus == 'delivered' && $order['seller_id'] != null) {
        ...
        app(OrderService::class)->syncPhysicalShipmentStatus(sourceOrder: $order, newStatus: $requestedStatus);

        $updatedOrder = $this->orderRepo->getFirstWhere(params: ['id' => $request['id']], relations: ['customer', 'seller.shop', 'deliveryMan']);
        event(new OrderStatusEvent(key: $requestedStatus, type: 'customer', order: $updatedOrder));
        if ($requestedStatus == 'canceled') {
            event(new OrderStatusEvent(key: 'canceled', type: 'delivery_man', order: $updatedOrder));
        }

        $walletStatus = getWebConfig(name: 'wallet_status');
        $loyaltyPointStatus = getWebConfig(name: 'loyalty_point_status');

        if ($walletStatus == 1 && $loyaltyPointStatus == 1 && !$order['is_guest'] && $requestedStatus == 'delivered' && $order['seller_id'] != null) {
        ...
        if (!$order['is_guest'] && $refEarningStatus == 1 && $requestedStatus == 'delivered') {
        ...
        if ($order['delivery_man_id'] && $requestedStatus == 'delivered') {
            ...
            if ($order['deliveryman_charge'] && $requestedStatus == 'delivered') {
        ...
        $orderStatusHistoryData = $orderStatusHistoryService->getOrderHistoryData(orderId: $request['id'], userId: auth('seller')->id(), userType: 'seller', status: $requestedStatus);
        $this->orderStatusHistoryRepo->add($orderStatusHistoryData);

        return response()->json($requestedStatus);
    }

    private function getVendorAllowedOrderStatuses(string $currentStatus, bool $sellerwiseShipping): array
    {
        $transitions = [
    ...
    public function uploadDigitalFileAfterSell(UploadDigitalFileAfterSellRequest $request): RedirectResponse
    {
        ...
        if ($this->orderDetailRepo->update(id: $orderDetails['id'], data: ['digital_file_after_sell' => $digitalFileAfterSell])) {
            // Find the main order ID from the order detail
            $orderId = $orderDetails['order_id'];

            // Uploading the file is the vendor's digital-delivery action: mark just this
            // shipment item delivered. Physical status (order_status) is tracked
            // independently and must not be touched here.
            app(OrderService::class)->markDigitalItemDelivered(orderDetailId: $orderDetails['id'], vendorId: auth('seller')->id());

            $order = $this->orderRepo->getFirstWhere(params: ['id' => $orderId], relations: ['customer', 'seller.shop']);

            ToastMagic::success(translate('digital_file_upload_successfully'));

            if (($orderDetails['digital_delivery_type'] ?? null) === 'email' && !empty($orderDetails['digital_delivery_value'])) {
                try {
                    $customer    = $order->customer;
                    $customerName = trim(($customer?->f_name ?? '') . ' ' . ($customer?->l_name ?? ''));
                    app(\App\Listeners\OrderStatusListener::class)->sendDigitalReadyMail(
                        email:        $orderDetails['digital_delivery_value'],
                        customerName: $customerName,
                        orderId:      $orderId
                    );
                } catch (\Throwable $e) {
                    \Illuminate\Support\Facades\Log::error('Digital file upload email failed', [
                        'order_id' => $orderId,
                        'error'    => $e->getMessage(),
                    ]);
                }
    ...
    public function markDigitalDelivered(Request $request): RedirectResponse
    {
        $orderDetails = $this->orderDetailRepo->getFirstWhere(['id' => $request['order_detail_id']]);

        $marked = app(OrderService::class)->markDigitalItemDelivered(
            orderDetailId: $request['order_detail_id'],
            vendorId: auth('seller')->id(),
        );

        if ($marked) {
            ToastMagic::success(translate('digital_item_marked_as_delivered'));

            if ($orderDetails && ($orderDetails['digital_delivery_type'] ?? null) === 'email' && !empty($orderDetails['digital_delivery_value'])) {
                try {
                    $order = $this->orderRepo->getFirstWhere(params: ['id' => $orderDetails['order_id']], relations: ['customer', 'seller.shop']);
                    $customer = $order->customer;
                    $customerName = trim(($customer?->f_name ?? '') . ' ' . ($customer?->l_name ?? ''));
                    app(\App\Listeners\OrderStatusListener::class)->sendDigitalReadyMail(
                        email: $orderDetails['digital_delivery_value'],
                    ...
                    \Illuminate\Support\Facades\Log::error('Mark digital delivered email failed', [
