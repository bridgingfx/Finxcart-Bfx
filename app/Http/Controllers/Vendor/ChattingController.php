...
class ChattingController extends BaseController
{
    ...
    public function markTyping(Request $request): JsonResponse
    {
        ...
        if ($peerType) {
            $this->chattingService->markTyping('seller', $vendorId, $peerType, $peerId);
        }
    ...
    public function typingStatus(Request $request): JsonResponse
    {
        $vendorId = auth('seller')->id();
        [$peerType, $peerId] = $this->resolveChatPeer($request);
        $typing = $peerType && $this->chattingService->isTyping($peerType, $peerId, 'seller', $vendorId);
    ...
    private function resolveChatPeer(Request $request): array
    {
        ...
        if ($request->has(key: 'user_id')) {
            return ['customer', $request['user_id']];
        }
    ...
    public function addVendorMessage(ChattingRequest $request): JsonResponse
    {
        ...
        if ($request->has(key: 'delivery_man_id')) {
            ...
            event(new ChattingEvent(key: 'message_from_seller', type: 'customer', userData: $customer, messageForm: $vendor, messageText: $request['message'] ?? null));
