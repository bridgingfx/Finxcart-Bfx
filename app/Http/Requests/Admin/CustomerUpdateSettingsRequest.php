...
class CustomerUpdateSettingsRequest extends FormRequest
{
    ...
    {
        return [
            'add_fund_bonus'                    => 'nullable|numeric|max:100|min:0|regex:/^\d+(\.\d+)?$/',
            'loyalty_point_exchange_rate'       => 'nullable|numeric|min:1|regex:/^\d+(\.\d+)?$/',
            'ref_earning_exchange_rate'         => 'nullable|numeric|min:0|regex:/^\d+(\.\d+)?$/',
            'maximum_add_fund_amount'           => 'nullable|numeric|min:0|regex:/^\d+(\.\d+)?$/',
            'minimum_add_fund_amount'           => 'nullable|numeric|min:1|regex:/^\d+(\.\d+)?$/',
            'item_purchase_point'               => 'nullable|numeric|min:0|regex:/^\d+(\.\d+)?$/',
            'minimum_transfer_point'            => 'nullable|numeric|min:0|regex:/^\d+(\.\d+)?$/',
        ];
    }
