<?php

namespace App\Http\Requests\Admin;

use App\Traits\ResponseHandler;
use Illuminate\Contracts\Validation\ValidationRule;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Http\Exceptions\HttpResponseException;
use Illuminate\Validation\Rule;

class VendorAddRequest extends FormRequest
{
    use ResponseHandler;

    /**
     * Determine if the user is authorized to make this request.
     */
    public function authorize(): bool
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, ValidationRule|array|string>
     */
    public function rules(): array
    {
        return [
            'f_name' => 'required',
            'l_name' => 'required',
            // Scoped to this admin-created seller_type (company/individual) so the
            // same email/phone can independently exist as a Customer, a self-
            // registered Vendor, a Freelancer, or an admin-vendor of the other type —
            // matches the sellers_email_seller_type_unique composite DB index.
            'phone' => [
                'required', 'max:20', 'min:4',
                Rule::unique('sellers')->where(fn ($query) => $query->where('seller_type', $this->input('seller_type'))),
            ],
            'email' => [
                'required',
                Rule::unique('sellers')->where(fn ($query) => $query->where('seller_type', $this->input('seller_type'))),
            ],
            'image' => 'required|mimes:jpg,jpeg,png,webp,gif,bmp,tif,tiff',
            'password' => 'required|min:8|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*\W)(?!.*\s).{8,}$/|same:confirm_password',
            'seller_type' => 'required|in:company,individual',
            'shop_name' => 'required_if:seller_type,company|nullable',
            'shop_address' => 'required_if:seller_type,company|nullable',
            'logo'              => 'required|mimes: jpg,jpeg,png,webp,gif,bmp,tif,tiff',
            'banner'            => 'required|mimes: jpg,jpeg,png,webp,gif,bmp,tif,tiff',
            'bottom_banner'     => 'mimes: jpg,jpeg,png,webp,gif,bmp,tif,tiff',
            'company_website' => 'required_if:seller_type,company|nullable|string|max:255',
            'company_no' => 'required_if:seller_type,company|nullable|string|max:255',
            'company_license' => 'required_if:seller_type,company|nullable|file|mimes:jpg,jpeg,png,pdf,webp|max:10240',
            'company_registered_country' => 'required_if:seller_type,company|nullable|string|max:255',
            'personal_id_document' => 'required_if:seller_type,individual|nullable|file|mimes:jpg,jpeg,png,pdf,webp|max:10240',
        ];
    }

    public function messages(): array
    {
        return [
            'f_name.required' => translate('The_first_name_field_is_required'),
            'l_name.required' => translate('The_last_name_field_is_required'),
            'phone.required' => translate('The_phone_field_is_required'),
            'phone.unique' => translate('The_phone_has_already_been_taken'),
            'phone.max' => translate('please_ensure_your_phone_number_is_valid_and_does_not_exceed_20_characters'),
            'phone.min' => translate('phone_number_with_a_minimum_length_requirement_of_4_characters'),
            'email.required' => translate('The_email_field_is_required'),
            'email.unique' => translate('The_email_has_already_been_taken'),
            'image.required' => translate('The_image_field_is_required'),
            'image.mimes' => translate('The_image_type_must_be').'.jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff,.webp',
            'password.required' => translate('The_password_field_is_required'),
            'password.same' => translate('The_password_and_confirm_password_must_match'),
            'password.regex' => translate('The_password_must_be_at_least_8_characters_long_and_contain_at_least_one_uppercase_letter').','.translate('_one_lowercase_letter').','.translate('_one_digit_').','.translate('_one_special_character').','.translate('_and_no_spaces').'.',
            'seller_type.required' => translate('please_select_seller_type'),
            'seller_type.in' => translate('please_select_seller_type'),
            'shop_name.required_if' => translate('The_shop_name_field_is_required'),
            'shop_address.required_if' => translate('The_shop_address_field_is_required'),
            'logo.mimes' => translate('The_logo_type_must_be').'.jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff,.webp',
            'banner.mimes' => translate('The_banner_type_must_be').'.jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff,.webp',
            'bottom_banner.mimes' => translate('The_bottom_banner_type_must_be').'.jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff,.webp',
            'company_website.required_if' => translate('company_website_is_required'),
            'company_no.required_if' => translate('company_registration_no_is_required'),
            'company_license.required_if' => translate('company_license_is_required'),
            'company_registered_country.required_if' => translate('company_registered_country_is_required'),
            'personal_id_document.required_if' => translate('personal_id_document_is_required'),
        ];
    }

    protected function failedValidation(\Illuminate\Contracts\Validation\Validator $validator)
    {
        throw new HttpResponseException(response()->json(['errors' => $this->errorProcessor($validator)]));
    }
}
