...
trait CacheManagerTrait
{
    ...
    public function cacheTopRatedProductList()
    {
        return Cache::remember(CACHE_FOR_HOME_PAGE_TOP_RATED_PRODUCT_LIST, CACHE_FOR_3_HOURS, function () {
            return Product::active()->with(['reviews', 'seller.shop', 'clearanceSale' =>function ($query) {
                return $query->active();
            }])
     ...
     * categories. Dedupe those by name here, keeping the newest (highest id)
     * row per name, so the homepage doesn't show visual duplicates.
     */
    public function cacheHomePageDistinctLatestProductIds(): array
    {
        return Cache::remember(CACHE_FOR_HOME_PAGE_LATEST_PRODUCT_IDS, CACHE_FOR_3_HOURS, function () {
            return Product::active()
    ...
    public function cacheHomePageLatestProductList()
    {
        return Cache::remember(CACHE_FOR_HOME_PAGE_LATEST_PRODUCT_LIST, CACHE_FOR_3_HOURS, function () {
            $ids = array_slice($this->cacheHomePageDistinctLatestProductIds(), 0, 12);
            $latestProductsList = Product::whereIn('id', $ids)
                ->with(['seller.shop', 'flashDealProducts.flashDeal', 'subCategory', 'reviews', 'clearanceSale' => function ($query) {
                    return $query->active();
                }])->orderBy('id', 'desc')->get();
    ...
    public function cacheHomePageLatestProductCount()
    {
        return Cache::remember(CACHE_FOR_HOME_PAGE_LATEST_PRODUCT_COUNT, CACHE_FOR_3_HOURS, function () {
            return count($this->cacheHomePageDistinctLatestProductIds());
        });
    }
    ...
    {
        // This ran on every single request (called from AppServiceProvider::boot()).
        // The unconditional forget() right before the get() guaranteed a cache miss
        // every time, defeating the point of caching. InhouseShopController already
        // forgets this key when the setting actually changes, so it's not needed here.
