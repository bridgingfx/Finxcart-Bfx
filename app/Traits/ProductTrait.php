...
trait ProductTrait
{
    ...
    public function updateRestockRequestListAndNotify(object|array $product, object|array $updatedProduct): void
    {
        ...
        if (count($productUpdateVariation) > 0) {
            // Batched once instead of one RestockProduct query per updated variant —
            // was firing a separate SELECT per variation type on every product update.
            $variantTypes = array_column($productUpdateVariation, 'type');
            $restockRequestsByVariant = RestockProduct::with(['product'])
                ->where('product_id', $product['id'])
                ->whereIn('variant', $variantTypes)
                ->get()
                ->keyBy('variant');

            foreach ($productUpdateVariation as $productUpdateVariant) {
                $restockRequest = $restockRequestsByVariant->get($productUpdateVariant['type']);
