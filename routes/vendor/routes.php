...
use App\Http\Controllers\Vendor\SellerBankController;
...
use App\Http\Controllers\Vendor\Tier\TierController;
...
Route::group(['middleware' => ['maintenance_mode']], function () {
        ...
        Route::group(['prefix' => 'auth', 'as' => 'auth.'], function () {
            ...
            Route::group(['prefix' => 'forgot-password', 'as' => 'forgot-password.'], function () {
                Route::controller(ForgotPasswordController::class)->group(function () {
                    ...
                    Route::post(ForgotPassword::INDEX[URI], 'getPasswordResetRequest')->middleware('throttle:6,1');
                    Route::get(ForgotPassword::OTP_VERIFICATION[URI], 'getOTPVerificationView')->name('otp-verification');
                    Route::post(ForgotPassword::OTP_VERIFICATION[URI], 'submitOTPVerificationCode')->middleware('throttle:10,1');
                    Route::get(ForgotPassword::RESET_PASSWORD[URI], 'getPasswordResetView')->name('reset-password');
                    Route::post(ForgotPassword::RESET_PASSWORD[URI], 'resetPassword')->middleware('throttle:10,1');
                });
            });
            Route::group(['prefix' => 'registration', 'as' => 'registration.'], function () {
                Route::controller(RegisterController::class)->group(function () {
                    Route::get(Auth::VENDOR_REGISTRATION[URI], 'index')->name('index');
                    Route::post(Auth::VENDOR_REGISTRATION[URI], 'add')->middleware('throttle:6,1');
                });
            });
            Route::controller(RegisterController::class)->group(function () {
    ...
    Route::get('kyc/token', [SumsubController::class, 'getAccessToken'])->name('kyc.token');
    Route::get('payout-kyc', [SumsubController::class, 'index'])->name('payout-kyc.index');
    Route::post('payout-kyc/request-review', [SumsubController::class, 'requestManualReview'])->name('payout-kyc.request-review');

    // Tier Selection Routes (MUST be outside the 'auth.seller.plan' group)
     Route::group(['prefix' => 'tier', 'as' => 'tier.'], function () {
                Route::controller(TierController::class)->group(function () {
                    // Route to display all available tier plans
                    Route::get('/', 'index')->name('index');

                    // Route to handle the form submission for selecting a plan (and payment)
                    Route::post('select', 'select')->name('select');

                    Route::get('checkout', 'vendorCheckoutPayment')->name('vendor-checkout-payment');
                    Route::post('web-payment-request', 'vendorTierPaymentRequest')->name('web-payment-request');

                    Route::post('renew','renew')->name('renew');
                    Route::get('renew-link', 'renewSubscriptionLink')->name('renew-link');

                    Route::post('wallet-payment', 'payWithWallet')->name('wallet-payment');
                    Route::get('payment-success', 'vendorTierPaymentSuccess')->name('payment-success');
                    Route::get('payment-fail', 'vendorTierPaymentFail')->name('payment-fail');


                    Route::get('my-tier-plan', 'myTier')->name('my-tier-plan');
                    Route::post('cancel', [TierController::class, 'cancel'])->name('cancel');

                });
            });

    // Inner Group: Requires Active Plan (Authentication + Plan Check)
    Route::group(['middleware' => ['auth.seller.plan']], function () {


        Route::post('/notifications/mark-as-read', [VendorNotificationController::class, 'markAsRead'])->name('notifications.mark-as-read');
        Route::post('/notifications/mark-kyc-seen', [VendorNotificationController::class, 'markKycSeen'])->name('notifications.mark-kyc-seen');
        Route::get('/notifications/poll', [VendorNotificationController::class, 'poll'])->name('notifications.poll');
        ...
        Route::get('rcont/contract/{tierid}', [ContractController::class, 'show'])->name('cont.contract');
        // --- PRODUCT ---
        Route::group(['prefix' => 'products', 'as' => 'products.', 'middleware' => ['verification.complete']], function () {
            Route::controller(ProductController::class)->group(function () {
                ...
                Route::post('featured-status', 'featuredStatus')->name('featured-status');
        ...
        Route::group(['prefix' => 'orders', 'as' => 'orders.'], function () {
            Route::controller(OrderController::class)->group(function () {
                ...
                Route::post(Order::UPDATE_ADDRESS[URI], 'updateAddress')->name('address-update');
                ...
                Route::post(Order::MARK_DIGITAL_DELIVERED[URI], 'markDigitalDelivered')->name('mark-digital-delivered');
                Route::post(Order::UPDATE_STATUS[URI], 'updateStatus')->name('status');
            });
        });

        // --- CUSTOMER, REVIEWS, COUPON, etc. (All other modules) ---
        ...
        Route::group(['prefix' => 'profile', 'as' => 'profile.'], function () {
            Route::controller(ProfileController::class)->group(function () {
                ...
                Route::delete(Profile::BANK_INFO_UPDATE[URI] . '/{id}', 'removeBankInfo')->name('remove-bank-info');
            });
        });
        ...
        Route::group(['prefix' => 'business-settings', 'as' => 'business-settings.'], function () {
            ...
            Route::group(['prefix' => 'bank', 'as' => 'bank.'], function () {
                Route::controller(SellerBankController::class)->group(function () {
                    Route::get(Bank::INDEX[URI], 'index')->name('index');
        ...
        Route::group(['prefix' => 'transaction', 'as' => 'transaction.'], function () {
            Route::controller(TransactionReportController::class)->group(function () {
                ...
                Route::get('commission-ledger', 'commission_ledger_list')->name('commission-ledger');
    ...
    }); // End of 'auth.seller.plan' group

          Route::get('/products/{product_id}/contract',
    [ProductController::class, 'getProductContractView']
                )->name('contracts.contract');


    Route::controller(ContractController::class)->group(function () {
            // 1. Route to fetch the contract content via AJAX for the modal
            // Example access: /contract/content/Basic
            Route::get('/contract/content/{tier}', 'getContractHtml')->name('contract.content');
            Route::get('contract-test', 'showTestContractPage')->name('contract.test-page');
            // 2. Route for final form submission, PDF generation, and DB saving
            Route::post('/contract/submit', 'signAndSubmit')->name('contract.submit');
            Route::get('contract/download', 'downloadContract')->name('contract.download');
        });
