        ...
        <div class="d-flex justify-content-center">
            <div class="ext-center">
                ...
                <p>{{$vendorRegistrationHeader?->sub_title ?? translate('create_your_own_store').'.'.translate('already_have_store').'?'}} 
                    {{-- <a class="text-primary fw-bold" style="font-size: 16px; text-decoration: underline;" href="{{route('vendor.auth.login')}}">{{translate('login')}}</a> --}}
                    <a class="text-primary fw-bold" style="font-size: 16px; " href="{{route('vendor.auth.login')}}">{{translate('login')}}</a> 
                </p>
