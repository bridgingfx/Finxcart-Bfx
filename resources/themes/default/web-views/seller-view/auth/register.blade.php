...
<script>
    $(document).ready(function () {
        ...
        var registrationSubmitInFlight = false;
        $('#seller-registration').on('submit', function (e) {
            e.preventDefault();

            // Same double-submit guard as the OTP verify button below — a fast
            // double-click on the submit button (or Enter fired twice) can send
            // two concurrent registration requests before the disabled state
            // takes effect.
        ...
        $('#otp-verify-btn').on('click', function () {
            // A fast double-click/double-tap can fire this handler twice before
            // btn.prop('disabled', true) takes effect, sending two concurrent
            // verify requests. The first one logs the vendor in, which
            // regenerates the session's CSRF token — so the second, still
            // in-flight request then fails with a raw 419 CSRF mismatch
            // instead of a clean response. Guard with an explicit in-flight
            // flag so only one verify request is ever sent at a time.
        ...
        function cancelPendingOtpAndReload() {
            ...
            // (see RegisterController::getView()), so a bare reload here just
            // reopens the same modal with the same timer still running.
            $.ajax({
                url: '{{ route("vendor.auth.cancel-registration-otp") }}',
                method: 'POST',
                data: { _token: '{{ csrf_token() }}' },
                complete: function () {
                    location.reload();
