        ...
        <div class="card border-0 box-shadow-lg">
            <div class="card-body py-5">
                ...
                <div class="mx-auto mw-1000">
                    ...
                    <form action="{{route('track-order.result')}}" type="submit" method="post" class="p-3" novalidate>
                        @csrf
                        <div class="row g-3">
                ...
                <h6 class="font-weight-bold text-center m-0 pt-5 pb-2">
                    <span class="text-capitalize">{{ translate('your_order')}}</span> <span>:</span> <span
                            class="text-base">{{$orderDetails['id']}}</span>
                </h6>
                @if(isset($orderDetails->children) && $orderDetails->children->count() > 0)
                <div class="pb-3">
                    <small class="text-muted d-block text-center mb-3">{{ translate('this_order_has_multiple_shipments') }}</small>
                    @foreach($orderDetails->children as $child)
                    <div class="border rounded p-3 mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <a href="{{ route('track-order.order-wise-result-view', ['order_id' => $child->id]) }}" class="fs-13 font-bold text-primary">
                                {{ translate('Shipment ID') }}: {{ $orderDetails->id }}-{{ $child->child_number }}
                            </a>
                ...
                <ul class="nav nav-tabs media-tabs nav-justified order-track-info">
                    ...
                    @if ($orderDetails['order_status']!='returned' && $orderDetails['order_status']!='failed' && $orderDetails['order_status']!='canceled')
                            ...
                            <li class="nav-item ">
                                <div class="nav-link {{($orderDetails['order_status']=='confirmed') || ($orderDetails['order_status']=='processing') || ($orderDetails['order_status']=='processed') || ($orderDetails['order_status']=='out_for_delivery') || ($orderDetails['order_status']=='delivered')?'active-status' : ''}}">
                                    <div class="d-flex flex-sm-column gap-3 gap-sm-0">
                                        ...
                                        <div class="media-body">
                                            <div class="text-sm-center">
                                                <h6 class="media-tab-title text-nowrap mb-0 text-capitalize fs-14">{{ translate('packed')}}</h6>
                                            </div>
                            ...
                            <li class="nav-item">
                                <div class="nav-link {{($orderDetails['order_status']=='processing') || ($orderDetails['order_status']=='processed') || ($orderDetails['order_status']=='out_for_delivery') || ($orderDetails['order_status']=='delivered')?'active-status' : ''}}">
                                    <div class="d-flex flex-sm-column gap-3 gap-sm-0">
                                        ...
                                        <div class="media-body">
                                            <div class="text-sm-center">
                                                <h6 class="media-tab-title text-nowrap mb-0 text-capitalize fs-14">{{ translate('shipped')}}</h6>
                                            </div>
                        ...
                        @else
                            ...
                            {{-- Stage 3: License/Access Generated --}}
                            <li class="nav-item">
