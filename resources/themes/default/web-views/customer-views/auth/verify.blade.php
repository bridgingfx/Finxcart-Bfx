                        ...
                        <div class="card-body">
                            ...
                            <div class="resend_otp_custom text-center overflow-hidden {{ $get_time <= 0 ? 'd--none' : ''}}">
                                ...
                                <h6 class="text-primary m-0 pb-4 verifyTimer">
                                    <span class="verifyCounter" data-second="{{$get_time}}" data-expires-at="{{ now()->addSeconds($get_time)->getTimestamp() * 1000 }}"></span>s
                                </h6>
                            </div>
                            <form class="needs-validation_ otp-form" id="sign-up-form"
                                ...
                                <div class="d-flex gap-2 justify-content-between align-items-center mt-3">
                                    ...
                                    <button class="btn btn--primary w-100 resend-otp-button resendVerifyForm" {{ $get_time > 0 ? 'disabled' : '' }}
                                            type="button"
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <h5 class="font-black __text-20px text-center my-2">
                                        {{ translate('verification_done_successfully')}}
                                    </h5>
                                </div>
                            </div>
...
@push('script')
    <script>
        $(document).ready(function () {
            @if($localDevOtp ?? null)
                // Local dev only — the backend only ever sends this value when
                // app()->environment('local'), never in production.
                toastr.info('Local dev OTP: {{ $localDevOtp }}', '', {timeOut: 15000});
            @endif
