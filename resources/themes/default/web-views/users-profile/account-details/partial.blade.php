    ...
    <div>
        <div class="d-flex align-items-center gap-2 text-capitalize">
            @if($order->parent_order_id)
                <h4 class="text-capitalize mb-0 mobile-fs-14 fs-18 font-bold">
                    <span class="text-muted fs-14">{{translate('order')}} #{{$order->parent_order_id}}</span>
                    &nbsp;&mdash;&nbsp;{{translate('Shipment')}} #{{$order->parent_order_id}}-{{$order->child_number}}
                </h4>
            @else
            ...
            @else
                <span class="fs-12 font-semibold badge __badge badge-soft-primary rounded">
                    {{ $order['order_status']}}
                </span>
            @endif
        </div>
