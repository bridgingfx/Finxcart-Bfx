...
@section('content')
    <div class="container py-4 rtl __account-address text-align-direction">
        ...
        @php
            ...
            // $mapQuery = ($lat && $lng)
            //     ? trim($lat) . ',' . trim($lng)
            //     : ($address ? urlencode($address) : 'Dubai');
            ...
            <section class="col-lg-9 col-md-8">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('address-update') }}" method="post" novalidate>
                            @csrf
                            <div class="row pb-1">
                                <div class="col-md-6">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input class="form-control phone-input-with-country-picker" type="text"
                                        id="own_phone" value="+{{ $shippingAddress->phone }}" required="required">
                                    <input type="hidden" class="country-picker-phone-number w-50" name="phone"
                                        value="{{ $shippingAddress->phone }}" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                            ...
                            <div class="form-row">
                                <div class="col-md-12">
                                    <div class="form-group mb-1">
                                        ...
                                        <textarea class="form-control" id="address" name="address" required>{{ $shippingAddress->address ?? '' }}</textarea>
                                ...
                                <div class="col-md-12">
                                    @include('web-views.partials._address-map-search', ['mapQuery' => $mapQuery, 'locationValue' => $shippingAddress->address ?? ''])
                                </div>
                            </div>

                            <input type="hidden" id="latitude" name="latitude"
                                value="{{ $shippingAddress->latitude ?? '' }}">
                            <input type="hidden" id="longitude" name="longitude"
                                value="{{ $shippingAddress->longitude ?? '' }}">

                            <div class="modal-footer">
                                <a href="{{ route('account-address') }}"
                                    class="closeB btn btn-secondary fs-14 font-semi-bold py-2 px-4">{{ translate('close') }}</a>
                                <button type="submit"
                                    class="btn btn--primary fs-14 font-semi-bold py-2 px-4">{{ translate('update') }}
                                </button>
                            </div>
                        </form>
                    </div>
