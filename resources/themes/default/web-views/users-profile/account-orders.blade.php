                ...
                <div class="card __card d-none d-lg-flex web-direction customer-profile-orders h-100">
                    <div class="card-body">
                        ...
                        <div class="d-flex gap-2 mb-3">
                            <a href="{{ request()->fullUrlWithQuery(['show_order' => 'ongoing']) }}"
                               class="btn btn-sm {{ $show_order == 'ongoing' ? 'btn--primary' : 'btn-outline--primary' }}">
                                {{ translate('in_progress') }}
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['show_order' => 'delivered']) }}"
                               class="btn btn-sm {{ $show_order == 'delivered' ? 'btn--primary' : 'btn-outline--primary' }}">
                                {{ translate('purchased') }}
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['show_order' => 'canceled']) }}"
                               class="btn btn-sm {{ $show_order == 'canceled' ? 'btn--primary' : 'btn-outline--primary' }}">
                                {{ translate('cancelled') }}
                            </a>
                        </div>
                            ...
                            <table class="table __table __table-2 text-center">
                                ...
                                @foreach($orders as $order)
                                    <tr>
                                            ...
                                            <div class="media-order">
                                                ...
                                                <h6 class="font-weight-bold m-0 mb-1">
                                                    ...
                                                    <span class="fs-12 font-weight-medium">
                                                        {{ $order->children->count() > 0 ? $order->children->sum('order_details_sum_qty') : $order->order_details_sum_qty }} {{ translate('items') }}
                                                        @if($order->children->count() > 1)
                                                            &nbsp;<span class="badge badge-soft-primary fs-10">{{ $order->children->count() }} {{ translate('shipments') }}</span>
                                                        @endif
                                                    </span>
                                        ...
                                        <td class="bodytr">
                                            @php $isAllDigital = $order->details->isNotEmpty() && $order->details->every(fn($d) => $d->product_type === 'digital'); @endphp
                                            <div class="__btn-grp-sm flex-nowrap">
                                                ...
                                                @if($show_order == 'ongoing' && !$isAllDigital)
                                                <a href="{{ route('track-order.order-wise-result-view', ['order_id' => $order->id]) }}"
                                                   class="btn-outline--primary text-primary __action-btn btn-shadow rounded-full" title="{{ translate('track_order') }}">
