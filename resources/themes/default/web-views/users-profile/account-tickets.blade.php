        ...
        <div class="modal-dialog modal-lg  " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="mt-3" method="post" action="{{route('ticket-submit')}}" id="open-ticket-form" enctype="multipart/form-data" novalidate>
                        @csrf
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <label class="font-semibold" for="ticket-subject">{{translate('subject')}}</label>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <div class="">
                                    <label class="font-semibold" for="ticket-type">{{translate('type')}}</label>
                                    <select class="custom-select" id="ticket-type" name="ticket_type" required>
                                        <option value="Orders Related issue">{{translate('orders_related_issue')}}</option> {{-- NEW OPTION --}}
                            ...
                            <div class="form-group col-md-6">
                                <div class="">
                                    <label class="font-semibold" for="ticket-priority">{{translate('priority')}}</label>
                        ...
                        {{-- NEW ORDERS DROPDOWN FIELD - Initially hidden --}}
                        <div class="form-row mt-3" id="order-select-container" style="display: none;">
                            <div class="form-group col-md-12">
                                <label class="font-semibold" for="order-id">{{translate('Select_Related_Order')}}</label>
                                <select class="custom-select" id="order-id" name="order_id">
                                    <option value="">{{translate('select_an_order')}}</option>
                                    @foreach($customerOrders ?? [] as $order)
                                        <option value="{{ $order->id }}" data-details="{{ json_encode($order) }}">
                                            {{ translate('Order ID') }} #{{ $order->id }} - ({{ date('d/M/Y', strtotime($order->created_at)) }}) - {{ translate($order->order_status) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        {{-- END NEW ORDERS DROPDOWN FIELD --}}
                        ...
                        <div class="d-flex flex-wrap upload_images_area">
                            ...
                            <label class="py-0 d-flex align-items-center m-0 cursor-pointer">
                                ...
                                <input type="file" name="image[]" id="attachmentfiles" class="attachmentfiles h-100 position-absolute w-100 " hidden multiple accept="image/*">
                            </label>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn--container flex-nowrap justify-content-center">
                                                        ...
                                                        <button type="button" class="__action-btn btn-shadow rounded-full text-danger marl delete-ticket-by-modal" data-link="{{ route('support-ticket.delete',['id'=>$ticket->id])}}" title="{{ translate('delete') }}" aria-label="{{ translate('delete') }}">
        ...
        <div class="d-flex flex-column gap-3 d-md-none">
            ...
            <div class="bg-white rounded p-3 shadow-sm get-view-by-onclick" data-link="{{ route('support-ticket.index', $ticket['id']) }}">
                <div class="d-flex justify-content-between gap-2 align-items-end">
                    ...
                    <div>
                        <button type="button" class="__action-btn btn-shadow rounded-full text-danger stopPropagation marl delete-ticket-by-modal fs-15" data-link="{{ route('support-ticket.delete',['id'=>$ticket->id])}}" title="{{ translate('delete') }}" aria-label="{{ translate('delete') }}">
                            <i class="fa fa-trash"></i>
                        </button>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        const orderSelectContainer = $('#order-select-container');
        const orderSelect = $('#order-id');
        const ticketDescription = $('#ticket-description');
        
        // Function to handle showing/hiding the order dropdown
        function toggleOrderSelect() {
            if (ticketType.val() === 'Orders Related issue') {
                orderSelectContainer.show();
            } else {
                orderSelectContainer.hide();
                orderSelect.val('');
                if(ticketType.val() !== 'Orders Related issue'){
        ...
        // Check when the ticket type changes
        ...
        orderSelect.on('change', function() {
            ...
            if (selectedOption.val() !== "") {
                const orderDetailsJson = selectedOption.data('details');
                
                if (orderDetailsJson) {
                    const orderId = orderDetailsJson.id;
                    const orderDate = new Date(orderDetailsJson.created_at).toLocaleDateString();
                    const orderStatus = orderDetailsJson.order_status;
                    
                    // --- START: Extract Product Names ---
                    let productList = "";
                    if (orderDetailsJson.order_details && orderDetailsJson.order_details.length > 0) {
                        productList += "{{ translate('Involved Products') }}:\n";
                        orderDetailsJson.order_details.forEach((detail, index) => {
                            // Assuming product name is nested under 'product' or 'product_details'
                            let productName = 'Unknown Product';
                            if (detail.product_details && detail.product_details.name) {
                                productName = detail.product_details.name;
                            } else if (detail.product && detail.product.name) {
                                productName = detail.product.name;
                            } else if (detail.product_details && detail.product_details.translations && detail.product_details.translations.length > 0) {
                                // Fallback to translations if available and product is JSON decoded
                                const translation = detail.product_details.translations.find(t => t.locale === '{{ App::getLocale() }}');
                                productName = translation ? translation.value : detail.product_details.name;
                            }
                            
                            productList += `- ${productName} (Qty: ${detail.qty})\n`;
                        });
                        productList += "\n";
                    }
                    // --- END: Extract Product Names ---

                    // Construct the detailed problem description
                    let defaultDescription = `{{ translate('I am writing regarding Order ID') }} **#${orderId}**, placed on ${orderDate} which currently has the status: ${orderStatus}.\n\n`;
                    
                    if (productList) {
                        defaultDescription += productList;
                    }
                    
                    defaultDescription += `{{ translate('My specific issue is') }}: `;
                    
                    ticketDescription.val(defaultDescription);
                    ticketDescription.focus();
                    
                    // Move cursor to the end of the predefined text
                    const currentVal = ticketDescription.val();
                    if(ticketDescription[0].setSelectionRange){
                         ticketDescription[0].setSelectionRange(currentVal.length, currentVal.length);
                    }
                }
            } else {
                // Clear description if 'select an order' is chosen
                ticketDescription.val('');
            }
        });
    });
