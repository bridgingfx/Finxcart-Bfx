                    ...
                    <div class="p-lg-3 p-0">
                        ...
                        <div class="card border-sm">
                            <div class="p-lg-3">
                                    ...
                                    <table class="table table-borderless mb-0">
                                        ...
                                        <tr class="order_table_tr">
                                            ...
                                            @if( $order->order_type == 'default_type')
                                                ...
                                                @php $shippingAddressShow = 0; @endphp
                                                @foreach($order->details as $details)
                                                    @if(isset($details->product_details) && isset(json_decode($details->product_details)?->product_type) && json_decode($details->product_details)?->product_type == "physical")
                                                        @php $shippingAddressShow = 1; @endphp
                                                    @endif
                                                @endforeach

                                                @php $shipping=$order['shipping_address_data']; @endphp
                                ...
                                <div class="payment mb-3 p-3 border rounded">
                                    <h6 class="fs-13 font-bold text-capitalize mb-2">{{ translate('Shipments_in_this_order') }}</h6>
                                    <div class="d-flex flex-column gap-2">
                                        @foreach($order->children as $child)
                                        <div class="border rounded p-2">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <a href="{{ route('account-order-details', ['id' => $child->id]) }}" class="fs-13 font-bold text-primary d-block">
                                                        {{ translate('Shipment') }} #{{ $order->id }}-{{ $child->child_number }}
                                                        <span class="text-muted fs-12 font-regular">&middot; {{ $child->seller_is == 'admin' ? getWebConfig('company_name') : ($child->seller?->shop?->name ?? translate('vendor')) }}</span>
                                                    </a>
                                                    <span class="fs-12 text-muted">
                                                        {{ translate('Delivery') }}: {{ $child->is_shipping_free ? translate('free') : webCurrencyConverter($child->shipping_cost) }}
                                                    </span>
                                                </div>
                                                <span class="badge badge-soft-{{ $child->sourceShipment?->status == 'completed' ? 'success' : 'primary' }} fs-11 text-capitalize">
                                                    {{ translate($child->sourceShipment?->status ?? 'processing') }}
                                                </span>
                                            </div>
                                            @include('web-views.users-profile.partials.shipment-tracking', ['shipment' => $child->sourceShipment])
                                        </div>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div class="payment mb-3 p-3 border rounded">
                                    <div class="d-flex flex-wrap align-items-center gap-2">
                                        <span class="fs-13 font-bold">{{ translate('Shipment') }} #{{ $order->parent_order_id }}-{{ $order->child_number }}</span>
                                        <span class="text-muted fs-12">&middot; {{ $order->seller_is == 'admin' ? getWebConfig('company_name') : ($order->seller?->shop?->name ?? translate('vendor')) }}</span>
                                        <span class="text-muted fs-12">{{ translate('part_of') }}</span>
                                        <a href="{{ route('account-order-details', ['id' => $order->parent_order_id]) }}" class="badge badge-soft-primary fs-12 py-1 px-2">
                                            {{ translate('Order') }} #{{ $order->parent_order_id }}
                                        </a>
                                    </div>
                                    @include('web-views.users-profile.partials.shipment-tracking', ['shipment' => $order->sourceShipment])
                                </div>
                                @elseif($order->sourceShipment)
                                <div class="payment mb-3 p-3 border rounded">
                                    <h6 class="fs-13 font-bold text-capitalize mb-2">
                                        {{ translate('Shipment_tracking') }}
                                        <span class="text-muted fs-12 font-regular">&middot; {{ $order->seller_is == 'admin' ? getWebConfig('company_name') : ($order->seller?->shop?->name ?? translate('vendor')) }}</span>
                                    </h6>
                                    @include('web-views.users-profile.partials.shipment-tracking', ['shipment' => $order->sourceShipment])
                                </div>
                                @endif
                                ...
                                <div class="payment mb-3 table-responsive d-none d-lg-block">
                                    <table class="table table-borderless min-width-600px">
                                        ...
                                        <tbody>
                                        @foreach ($displayDetails as $key=>$detail)
                                            @php $product=json_decode($detail->product_details,true); @endphp
                                            @if($product)
                                                <tr>
                            ...
                            <div class="col-md-8 col-lg-5">
                                <div class="bg-white border-sm rounded">
                                    <div class="card-body">
                                        <table class="calculation-table table table-borderless mb-0">
                                            ...
                                            <tr class="border-top">
                                                ...
                                                <td>
                                                    <div class="text-end">
                                                        <span class="font-weight-bold amount">
                                                            {{ webCurrencyConverter(amount: $order->children->count() > 0 ? $order->order_amount : $orderTotalPriceSummary['totalAmount']) }}
                                                        </span>
                                                    </div>
                                                </td>
                                            </tr>
                                        ...
                                        @php
                                            // Only child orders (or standalone orders) can be cancelled individually
                                            $isParentOrder = $order->children->count() > 0;
                                            $isCancellable = !$isParentOrder
                                                && in_array($order['order_status'], ['pending', 'confirmed', 'processing'])
                                                && $order['payment_method'] !== 'offline_payment';
                                            $hasPhysical = $displayDetails->isNotEmpty()
                                                && $displayDetails->contains(fn($d) => ($d->product_type ?? 'physical') !== 'digital');
                                        @endphp
                                        @if($isCancellable && $hasPhysical)
