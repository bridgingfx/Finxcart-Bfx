...
<script>
{{-- Deferred to window 'load' since nothing else on the page depends on Firebase synchronously
     (it's only used for FCM push notifications and OTP recaptcha, not first-paint/interactivity). --}}
window.addEventListener('load', function () {
    try {
        ...
        @if($web_config['firebase_otp_verification'] && $web_config['firebase_otp_verification']['status'])
            {{-- Already running inside the outer window 'load' handler, so call directly
                 instead of assigning window.onload (which would never fire at this point). --}}
