    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body d-flex flex-column gap-3">
                <form action="{{route('review.store')}}" method="post" enctype="multipart/form-data" novalidate>
                    @csrf
                    ...
                    <div class="mb-5 mt-3">
                        <div class="star-rating-form">
                            <div class="star-wrap">
                                ...
                                <input class="star" {{!isset($rating) ? 'checked' : ''}} type="radio" value="-1" id="skip-star-{{$id}}" name="star-radio-{{$id}}"
                                       autocomplete="off"/>
                                ...
                                @endfor
                                <div id="result-{{$id}}" style="{{$style ?? ''}}">{{$ratingStatus ?? ''}}</div>
                            </div>
                        </div>
                    </div>
...
@push('script')
    <script>
        ...
        (function () {
            var modalId = '{{$id}}';
            var modal = $('#submitReviewModal' + modalId);
            modal.find("input[name='star-radio-" + modalId + "']").on("change", function () {
                var starVal = parseInt($(this).val());
                var result = $('#result-' + modalId);
                var getStatusText = $('#get-status-text');
                if (starVal < 1) return;
                modal.find('input[name="rating"]').val(starVal);
                if (starVal == 1) {
