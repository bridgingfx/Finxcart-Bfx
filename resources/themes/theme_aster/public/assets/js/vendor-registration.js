...
function submitRegistration(){
    ...
    Swal.fire({
        ...
        if (result.value) {
            const sellerType = ($("#seller_type").val() || "").trim();
            if (!sellerType) {
                $(".seller-type-error").removeClass("d-none");
                return;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.errors) {
                        ...
                        setTimeout(function () {
                            location.href = data.redirectRoute + '?registered=1';
                        }, 4000);
                    }
                },
                error: function (xhr) {
...
$(document).on("click", ".seller-type-card", function () {
    $(".seller-type-card").removeClass("selected");
    $(this).addClass("selected");
    $("#seller_type").val($(this).data("type"));
    $(".seller-type-error").addClass("d-none");
});
