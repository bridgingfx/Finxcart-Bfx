...
function getStockCheckOnVariantPrice(formSelector = ".add-to-cart-details-form") {
    ...
    const hasCartKey =
        $(formSelector).find('.product-exist-in-cart-list[name="key"]').val() !== "";
...
$(".quantity__plus").on("click", function () {
    ...
    const cartKey = $form.find('.product-exist-in-cart-list[name="key"]').val();
    ...
    if (cartKey !== "") {
        updateProductCartQuantity(parentForm, parseInt($qty.val()));
        return false;
    }
...
$(".quantity__minus").on("click", function () {
    ...
    if ($(this).data("prevent") !== true) {
        let currentVal = parseInt($qty.val());
        let minValue = parseInt($qty.attr("min"));
        if (cartKey !== "" && !isNaN(currentVal) && currentVal <= minValue) {
            removeProductFromCartState(parentForm);
            return false;
        }
...
$(".add-to-cart-details-form input").on("change", function () {
    ...
    let cartKey = $(formSelector)
        .find('.product-exist-in-cart-list[name="key"]')
        .val();
    if ($(this).hasClass("product_quantity__qty") && cartKey !== "") {
        updateProductCartQuantity(formSelector, parseInt($(this).val()));
        return false;
    }
    getVariantPrice(formSelector);
});

$(".add-to-cart-sticky-form input").on("change", function () {
    let formSelector = ".add-to-cart-sticky-form";
    let cartKey = $(formSelector)
        .find('.product-exist-in-cart-list[name="key"]')
        .val();
    if ($(this).hasClass("product_quantity__qty") && cartKey !== "") {
...
function syncProductCartState(formSelector, response = {}) {
    let forms = $(formSelector);

    forms.each(function () {
        let $form = $(this);
        let $cartKeyField = $form.find('.product-exist-in-cart-list[name="key"]');
        let $quantityWrapper = $form.find(".product-cart-quantity-wrapper");
        let $quantityInput = $form.find(".product_quantity__qty");
        let $addToCartButton = $form.find(".product-add-to-cart-button");
        let inCartStatus =
            typeof response?.in_cart_status !== "undefined"
                ? parseInt(response.in_cart_status)
                : $cartKeyField.val() !== ""
                  ? 1
                  : 0;
        let inCartKey = response?.in_cart_key ?? $cartKeyField.val() ?? "";
        let inCartQuantity =
            response?.in_cart_quantity ?? response?.qty ?? $quantityInput.val();

        if (inCartStatus === 1 && inCartKey !== "") {
            $cartKeyField.val(inCartKey);
            if ($quantityWrapper.length) {
                $quantityWrapper.removeClass("d-none");
            }
            if (inCartQuantity !== "" && inCartQuantity !== null) {
                $quantityInput.val(inCartQuantity);
            }
            $addToCartButton.html(
                $addToCartButton.data("go-to-cart") ??
                    $addToCartButton.data("update") ??
                    $addToCartButton.data("add")
            );
        } else {
            $cartKeyField.val("");
            if ($quantityWrapper.length) {
            ...
            $addToCartButton.html($addToCartButton.data("add"));
        }

        getStockCheckOnVariantPrice($form);
    });
}

function removeProductFromCartState(formSelector) {
    let $form = $(formSelector);
    let cartKey = $form.find('.product-exist-in-cart-list[name="key"]').val();
    if (cartKey === "") {
        return false;
    }

    $.post(
        $("#remove_from_cart_url").data("url"),
        {
        ...
        function (response) {
            ...
            syncProductCartState(".addToCartDynamicForm", { in_cart_status: 0 });
            getVariantPrice(formSelector);
...
function updateProductCartQuantity(formSelector, quantity) {
    let $form = $(formSelector);
    let cartKey = $form.find('.product-exist-in-cart-list[name="key"]').val();
    ...
    $.post(
        ...
        function (response) {
            ...
            syncProductCartState(formSelector, {
                in_cart_status: 1,
                in_cart_key: cartKey,
                in_cart_quantity: response.qty,
            });
            updateNavCart();
            getVariantPrice(formSelector);
        }
...
function addToCartOnclick() {
    $(".product-add-to-cart-button").on("click", function () {
        ...
        let cartKey = productCartForm.find('.product-exist-in-cart-list[name="key"]').val();
        if (cartKey !== "") {
            let cartUrl = $(this).data("route") ?? "/shop-cart";
        ...
        $.post({
            ...
            success: function (response) {
                ...
                if (response.status === 1) {
                    ...
                    syncProductCartState(formSelector, {
                        in_cart_status: 1,
                        in_cart_key: response.in_cart_key,
                        in_cart_quantity:
                            $(formSelector).find('input[name="quantity"]').val(),
                    });
...
function updateProductDetailsTopSection(formSelector, response) {
    ...
    syncProductCartState(".addToCartDynamicForm", response);
