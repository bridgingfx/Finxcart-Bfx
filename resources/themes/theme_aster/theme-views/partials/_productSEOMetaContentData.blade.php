@if(isset($productDetails) && isset($metaContentData))
    ...
    <link rel="canonical" href="{{ route('product', [$productDetails->slug]) }}"/>
    ...
    <script type="application/ld+json">
    {
        ...
        "description": "{{ addslashes(Str::limit(strip_tags($metaContentData?->description ?? $productDetails->details ?? $productDetails->name), 160)) }}",
        "image": "{{ $metaContentData?->image_full_url['path'] }}",
        "url": "{{ route('product', [$productDetails->slug]) }}",
        "brand": {
            "@type": "Brand",
            "name": "{{ addslashes($productDetails->brand?->name ?? $web_config['company_name']) }}"
        },
        "offers": {
            "@type": "Offer",
