@php
    ...
    // Batch-load per-seller/per-group lookups once instead of re-querying per group/item below.
    $cartSellerIdsForLookup = $cart
        ->map(fn($group) => $group->first()->seller_id ?? null)
        ->filter()
        ->push(0)
        ->unique()
        ->values();
    $cartGroupIdsForLookup = $cart->flatten(1)->pluck('cart_group_id')->filter()->unique()->values();

    $shippingTypesBySeller = ShippingType::whereIn('seller_id', $cartSellerIdsForLookup)->get()->keyBy('seller_id');
    $cartShippingsByGroup = CartShipping::whereIn('cart_group_id', $cartGroupIdsForLookup)->get()->keyBy('cart_group_id');
@endphp
<div class="container">
                    ...
                    <div class="card-body">
                        ...
                        @foreach($cart as $group_key=>$group)
                            @php
                                ...
                                // Every item in a group shares the same seller, so the shipping
                                // type only needs to be resolved once per group, not per item.
                                        ...
                                        <div class="bg-primary-light py-2 px-2 px-sm-3 mb-3 mb-sm-4">
                                            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                                                ...
                                                @if($physical_product && $shippingMethod=='sellerwise_shipping' && $shipping_type == 'order_wise')
                                                    @php
                                                        $choosen_shipping=$cartShippingsByGroup->get($cartItem['cart_group_id'])
                                                    @endphp
                        ...
                        @if($shippingMethod=='inhouse_shipping')
                            ...
                            @if ($shipping_type == 'order_wise' && $physical_product)
                                ...
                                @php($choosen_shipping=$cartShippingsByGroup->get($cartItem['cart_group_id']))
