...
@section('content')
    <div class="content container-fluid">
        ...
        <form id="freelancer-service-form" action="{{ route('freelancer.services.update', [$service->id]) }}" method="post" enctype="multipart/form-data" novalidate>
            @csrf
                            ...
                            <div class="input-group">
                                <select class="form-control" id="freelancer-category-select" name="freelancer_category_id" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" data-owner="{{ (int)$category->seller_id === (int)auth('freelancer')->id() ? '1' : '0' }}" {{ (int)old('freelancer_category_id', $service->freelancer_category_id) === (int)$category->id ? 'selected' : '' }}>
                            ...
                            <div class="input-group">
                                <select class="form-control" id="freelancer-specialization-select" name="freelancer_specialization_id" required>
                                    ...
                                    @foreach($specializations as $specialization)
                                        <option value="{{ $specialization->id }}" data-category="{{ $specialization->freelancer_category_id }}" data-owner="{{ (int)$specialization->seller_id === (int)auth('freelancer')->id() ? '1' : '0' }}" {{ (int)old('freelancer_specialization_id', $service->freelancer_specialization_id) === (int)$specialization->id ? 'selected' : '' }}>
