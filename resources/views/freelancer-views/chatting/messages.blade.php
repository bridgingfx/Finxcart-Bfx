...
@foreach($chattingMessages as $key => $message)
        ...
        $messageTimeText = $message->created_at->isToday() ? $message->created_at->format('h:i A') : $message->created_at->format('M d, h:i A');
        ?>
    @if ($message->sent_by_customer || $message->sent_by_admin)
        <div class="incoming_msg d-flex align-items-end gap-2 my-2">
            <div class="">
                @if($message->sent_by_admin)
                    <div class="avatar-img user-avatar-image border inbox-user-avatar-25 d-flex align-items-center justify-content-center bg-soft-secondary" style="width:40px;height:40px;border-radius:50%;">
            ...
            >
                @if($message->sent_by_admin)
                    <span class="badge badge-soft-info text-capitalize mb-1">{{ translate('support_team') }}</span>
                @endif
                <div class="received_withdraw_msg d-flex flex-column align-items-start">
                                            ...
                                            <div
                                                ...
                                                <a target="_blank"
                                                   ...
                                                   data-index="{{ $secondIndex }}">
                                                    ...
                                                    @else
                                                        <img class="img-fit aspect-1 w-60px" alt="" loading="lazy"
                    ...
                    @if($message->message)
                        <div class="message-text-section rounded mt-1 d-inline-block mt-2">
                            ...
                            <span class="small text-end d-block text-muted fl-visible-message-time">{{ $messageTimeText }}</span>
                        </div>
                    @endif
                    @if(empty($message->message))
                        <span class="small text-start d-block text-muted fl-visible-message-time mt-1">{{ $messageTimeText }}</span>
                    @endif
                </div>
            </div>
        </div>
            ...
            >
                ...
                @if(!empty($message->message))
                    <div class="message-text-section rounded mt-1 d-inline-block mt-2">
                        ...
                        <span class="small text-start d-block text-muted fl-visible-message-time">{{ $messageTimeText }}</span>
                    </div>
                @endif
                @if(empty($message->message))
                    <span class="small text-end d-block text-muted fl-visible-message-time mt-1">{{ $messageTimeText }}</span>
                @endif
            </div>
        </div>
    @endif
