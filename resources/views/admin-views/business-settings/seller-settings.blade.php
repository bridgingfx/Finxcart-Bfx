...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.business-settings.vendor-settings.update-vendor-settings') }}" method="post" id="update-vendor-settings-form" novalidate>
            @csrf
                            ...
                            <div class="col-xl-6 col-md-6">
                                <label
                                    ...
                                    <span class="d-flex flex-column">
                                        ...
                                        <span class="mb-0 fs-12">
                                            {{ translate('if_enabled_vendors_can_set_minimum_order_amount_for_their_vendors') }}
                                        </span>
                                    </span>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <div class="mb-3 mb-sm-20">
                    <h3 class="mb-1">{{ translate('Payout KYC Settings') }}</h3>
                    <p class="mb-0 fs-12">
                        {{ translate('Configure Sumsub keys and payout KYC rules. Company profile verification is handled separately in Vendor Verification.') }}
                    </p>
                </div>
                <div class="p-12 p-sm-20 bg-section rounded">
                    <form action="{{ route('admin.business-settings.kyc.update') }}" method="POST" novalidate>
                        @csrf
                        @php($kycMethod = getWebConfig('kyc_method') ?? 'manual')
                        <div class="row g-4">
                            <div class="col-md-6">
                                <label class="form-label">{{ translate('Payout KYC Method') }}</label>
                                <select name="kyc_method" id="kyc_method" class="form-control" onchange="toggleSumsubFields()">
                                    <option value="manual" {{ $kycMethod == 'manual' ? 'selected' : '' }}>{{ translate('Manual payout review') }}</option>
                                    <option value="sumsub" {{ $kycMethod == 'sumsub' ? 'selected' : '' }}>Sumsub ({{ translate('automated_KYC') }})</option>
                                    <option value="disabled" {{ $kycMethod == 'disabled' ? 'selected' : '' }}>{{ translate('Disabled') }} ({{ translate('no_KYC_required') }})</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ translate('Require KYC before first payout?') }}</label>
                                @php($kycRequiredForPayout = getWebConfig('kyc_required_for_payout') ?? '1')
                                <select name="kyc_required_for_payout" class="form-control">
                                    <option value="1" {{ (string) $kycRequiredForPayout === '1' ? 'selected' : '' }}>{{ translate('yes') }}</option>
                                    <option value="0" {{ (string) $kycRequiredForPayout === '0' ? 'selected' : '' }}>{{ translate('no') }}</option>
                                </select>
                            </div>
                        </div>

                        <div id="sumsub-fields" class="row g-4 mt-0" style="display:{{ $kycMethod == 'sumsub' ? 'flex' : 'none' }}">
                            <div class="col-md-6">
                                <label class="form-label">{{ translate('Sumsub App Token') }}</label>
                                <input type="text" name="sumsub_app_token" class="form-control" value="{{ getWebConfig('sumsub_app_token') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ translate('Sumsub Secret Key') }}</label>
                                <input type="password" name="sumsub_secret_key" class="form-control" value="{{ getWebConfig('sumsub_secret_key') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ translate('Sumsub Flow Name') }}</label>
                                <input type="text" name="sumsub_flow_name" class="form-control" value="{{ getWebConfig('sumsub_flow_name') ?? 'basic-kyc' }}">
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            <button type="submit" class="btn btn-primary px-3 px-sm-4">
                                <i class="fi fi-sr-disk"></i>
                                {{ translate('Save Payout KYC Settings') }}
                            </button>
                        </div>
                    </form>
                </div>
...
@push('script')
    <script>
        function toggleSumsubFields() {
            const method = document.getElementById('kyc_method');
            const fields = document.getElementById('sumsub-fields');

            if (method && fields) {
                fields.style.display = method.value === 'sumsub' ? 'flex' : 'none';
            }
        }
    </script>
@endpush
