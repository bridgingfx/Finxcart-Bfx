<div>
    ...
    <div class="main-table-inner mb-4">
        <div class="d-flex justify-content-center pt-3">
            <img width="76" class="mb-4" id="view-mail-logo" src="{{ $template->logo_full_url['path'] ?? $companyLogoUrl ?? getStorageImages(path: $companyLogo, type:'backend-logo')}}" alt="">
        </div>
        ...
        <div class="main-table-inner p-3 bg-white">
            <div class="d-flex mb-3 p-2">
                <div class="flex-1 w-49">
                    ...
                    <div class="mb-2">{{translate('Order').' # '.(isset($data['order']) ? ($data['order']->parent_order_id ? $data['order']->parent_order_id.'-'.$data['order']->child_number : ($data['order']->id ?? '432121')) : '432121')}} </div>
                    <div>{{date('d M, Y : h:i:A', strtotime(isset($data['order']) ? $data['order']->created_at : now()))}}</div>
                </div>
            </div>
