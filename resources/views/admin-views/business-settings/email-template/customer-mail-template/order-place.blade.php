<div>
    ...
    <div class="main-table-inner mb-4">
        <div class="d-flex justify-content-center pt-3">
            <img class="mb-4 w-100px h-auto" id="view-mail-logo" src="{{$template->logo_full_url['path'] ?? $companyLogoUrl ?? getStorageImages(path: $companyLogo, type:'backend-logo')}}" alt="">
        </div>
        ...
        <div class="main-table-inner p-3 bg-white">
            <div class="d-flex mb-3 p-2">
                <div class="flex-1 w-49">
                    ...
                    <div class="mb-2">{{translate('Order').' # '.($data['orderId'] ?? $data['order']->id ?? '432121')}} </div>
                    <div>{{date('d M, Y : h:i:A' ,strtotime($data['order']->created_at ?? now()))}}</div>
                </div>
                @if(isset($data['send-mail']))
                    @php
                        $showDeliveryAddress = false;
                            foreach (($data['orderDetails'] ?? $data['order']->details) as $key=>$detail){
