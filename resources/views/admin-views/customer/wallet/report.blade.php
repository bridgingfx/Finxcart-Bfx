            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('admin.customer.wallet.add-fund')}}" method="post" enctype="multipart/form-data" id="add-fund" novalidate>
                            @csrf
                            <div class="row g-4">
                                <div class="col-sm-6">
                                    <div class="form-group d-flex flex-column">
                                        <label class="mb-2 d-flex" for="customer">{{translate('customer')}}</label>
                                        <select id='form-customer' name="customer_id" data-placeholder="{{translate('select_customer')}}" class="get-customer-list-without-all-customer form-select" required>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        <label class="mb-2 d-flex align-items-center gap-1" for="reference">{{translate('reference')}} <small>({{translate('optional')}})</small></label>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12 pt-3">
                        <form action="{{route('admin.customer.wallet.report')}}" method="get" novalidate>
                            <div class="row">
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        <label class="mb-2 d-flex" for="start-date-time">{{translate('from_date')}}</label>
                                        <input type="date" name="from" id="start-date-time" value="{{request()->get('from')}}" class="form-control" title="{{ucfirst(translate('from_date'))}}">
                                    </div>
                                </div>
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        <label class="mb-2 d-flex" for="end-date-time">{{translate('to_date')}}</label>
                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        ...
                                        <div class="select-wrapper">
                                            <select name="transaction_type" class="form-select" title="{{translate('select_transaction_type')}}">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{route('admin.customer.wallet.report')}}" class="btn btn-secondary">{{translate('reset')}}</a>
                    ...
                    <table id="datatable" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table {{Session::get('direction') === "rtl" ? 'text-right' : 'text-left'}}">
                        ...
                        @foreach($transactions as $key => $transaction)
                            <tr>
                                ...
                                <td>
                                    <a href="{{route('admin.customer.view',['user_id'=>$transaction['user_id']])}}" class="text-dark text-hover-primary">{{Str::limit($transaction['user'] ? $transaction?->user->f_name.' '.$transaction?->user->l_name : translate('not_found'),20)}}</a>
                                </td>
                                <td>
...
@push('script')
    <script>
        ...
        // Re-initialize Select2 when modal is shown to ensure search works
        $('#add-fund-modal').on('shown.bs.modal', function () {
            var $select = $('#form-customer');
            ...
            $select.on('select2:open', function() {
                setTimeout(function() {
                    var $searchInput = $('#add-fund-modal .select2-search__field');
                    ...
                    $.post({
                        ...
                        success: function (data) {
                            if (data.errors) {
                                ...
                                toastMagic.success('{{translate("fund_added_successfully")}}');
                                setTimeout(() => {
                                    location.reload()
                                }, 1500);
                            }
