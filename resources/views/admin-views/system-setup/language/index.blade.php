    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                ...
                <form action="{{ route('admin.system-setup.language.add-new') }}" method="post" novalidate>
                    @csrf
                    <div class="mb-3 mb-sm-20">
            ...
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex gap-10 justify-content-sm-end">
                        <form action="{{ route('admin.system-setup.language.index') }}" method="get" novalidate>
                            <div class="form-group">
                                ...
                                <td>
                                    ...
                                    @else
                                        <form action="{{ route('admin.system-setup.language.update-status') }}"
                                              method="post" id="language-form-{{ $language['code'] }}" class="no-reload-form" novalidate>
                                            @csrf
                                            <input type="hidden" name="code" value="{{ $language['code'] }}">
                                            <label class="switcher mx-auto" for="language-status-{{ $language['code'] }}">
                                                <input
                                                    class="switcher_input custom-modal-plugin"
                                                    type="checkbox" value="1" name="status"
                                                    id="language-status-{{ $language['code'] }}"
                                                    {{ $language['status'] == 1 ? 'checked' : ' '}}
                                                    data-modal-type="input-change-form"
                                                    data-modal-form="#language-form-{{ $language['code'] }}"
                                            ...
                                            <div class="dropdown-menu">
                                                @if (!(array_key_exists('default', $language) && $language['default'] === true))
                                                    <button class="dropdown-item d-flex align-items-center gap-2"
                                                            ...
                                                            data-bs-target="#languageDefaultModal{{ $language['code'] }}">
                                                ...
                                                @if($language['code'] != 'en')
                                                    <a class="dropdown-item d-flex align-items-center gap-2 pointer"
                                                       ...
                                                       data-bs-target="#languageEditModal{{ $language['code'] }}">
                                                ...
                                                @else
                                                    <button class="dropdown-item d-flex align-items-center gap-2"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#languageDeleteModal{{ $language['code'] }}">
                                                        <i class="fi fi-rr-trash mt-1"></i>
                                                        {{ translate('delete') }}
                                                    </button>
                                                @endif
        ...
        @foreach($languageList as $key => $language)
            <form action="{{ route('admin.system-setup.language.update-default-status') }}" method="post" novalidate>
                @csrf
                <input type="hidden" name="code" value="{{ $language['code'] }}">
                <div class="modal fade" id="languageDefaultModal{{ $language['code'] }}" tabindex="-1"
                     aria-labelledby="languageDefaultModal{{ $language['code'] }}" aria-hidden="true">
            ...
            <form action="{{ route('admin.system-setup.language.delete', [$language['code']]) }}" method="get" novalidate>
                @csrf
                <input type="hidden" name="id" value="{{ $language['id'] }}">
                <div class="modal fade" id="languageDeleteModal{{ $language['code'] }}" tabindex="-1"
                     aria-labelledby="languageDeleteModal{{ $language['code'] }}" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header border-0 pb-0 d-flex justify-content-end">
                                <button type="button"
