...
<form action="{{ route('admin.system-setup.language.update') }}" method="post" novalidate>
    @csrf
    <div class="offcanvas offcanvas-end" tabindex="-1" id="languageEditModal{{ $language['code'] }}" aria-labelledby="languageEditModal{{ $language['code'] }}Label">
            ...
            <div class="mb-3 mb-sm-20">
                <div class="p-12 p-sm-20 bg-section rounded">
                    <div class="form-group">
                        <label class="form-label" for="language-{{ $language['code'] }}">
                            {{ translate('Language') }}
                        </label>
                        <input type="text" name="name" class="form-control" value="{{ $language['name'] }}"
                               placeholder="{{ translate('language_name') }}" id="language-{{ $language['code'] }}">
                        ...
                        <div
                            ...
                            <div class="form-check d-flex gap-2 align-items-center p-0">
                                <input class="form-check-input radio--input m-0" type="radio"
                                       name="direction" id="direction_left{{ $language['code'] }}"
                                       value="ltr" {{ !isset($language['direction']) || $language['direction'] == 'ltr' ? 'checked' : '' }}>
                                <label class="form-check-label" for="direction_left{{ $language['code'] }}">
                                    {{ translate('Left_to_Right') }}
                                </label>
                            </div>
                            <div class="form-check d-flex gap-2 align-items-center p-0">
                                <input class="form-check-input radio--input m-0" type="radio"
                                       name="direction" id="direction_right{{ $language['code'] }}"
                                       value="rtl" {{ isset($language['direction']) && $language['direction'] == 'rtl' ? 'checked' : '' }}>
                                <label class="form-check-label" for="direction_right{{ $language['code'] }}">
