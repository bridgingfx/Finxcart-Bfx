    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                ...
                <form action="{{route('admin.third-party.mail.update')}}" method="post" id="smtp-mail-config-form" novalidate>
                    @csrf
                    @if(isset($data_smtp))
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{route('admin.third-party.mail.update-sendgrid')}}" method="post" id="sendgrid-mail-config-form" novalidate>
                    @csrf
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{route('admin.third-party.mail.update-brevo')}}" method="post" id="brevo-mail-config-form" novalidate>
                    @csrf
                    @php($data_brevo=getWebConfig(name: 'mail_config_brevo'))
                    @if(isset($data_brevo))
                    ...
                    <div class="view-details-container">
                        <div class="d-flex justify-content-between align-items-center gap-3">
                            <div>
                                <h3>
                                    {{ translate('brevo_mail_configuration') }}
                                </h3>
                                <p class="mb-1 fs-12">
                                    {{ translate('send_emails_via_brevos_secure_cloud_email_service') }}.
                                </p>
                            </div>
                            <div class="d-flex gap-2">
                                <a href="javascript:" class="fs-12 fw-semibold d-flex align-items-end view-btn {{ $data_brevo['status'] == 1 ? 'active' : '' }}">{{ translate('View') }} <i class="fi fi-rr-arrow-small-down fs-16 trans3"></i></a>
                                <label class="switcher">
                                    <input
                                        class="switcher_input custom-modal-plugin"
                                        type="checkbox" value="1" name="status"
                                        {{ $data_brevo['status'] == 1 ? 'checked':'' }}
                                        data-modal-type="input-change-form"
                                        data-modal-form="#brevo-mail-config-form"
                                        data-on-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/maintenance_mode-on.png') }}"
                                        data-off-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/maintenance_mode-off.png') }}"
                                        data-on-title="{{translate('want_to_Turn_ON_the_brevo_mail_config_option').'?'}}"
                                        data-off-title="{{translate('want_to_Turn_OFF_the_brevo_mail_config_option').'?'}}"
                                        data-on-message="<p>{{translate('enabling_mail_configuration_services_will_allow_the_system_to_send_emails').'.'.translate('please_ensure_that_you_have_correctly_configured_the_brevo_settings_to_avoid_potential_issues_with_email_delivery')}}</p>"
                                        data-off-message="<p>{{translate('disabling_brevo_mail_configuration_services_stops_email_sending')}}</p>">
                                    <span class="switcher_control"></span>
                                </label>
                            </div>
                        </div>
                                    ...
                                    <div class="col-lg-6 col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text"
                                                           placeholder="{{translate('ex').':'}}{{translate('alex')}}"
                                                           class="form-control" name="name"
                                                           value="{{env('APP_MODE')=='demo' ? '' :$data_brevo['name']}}">
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="driver"
                                                           placeholder="{{translate('ex')}}:{{translate('brevo')}}"
                                                           value="{{env('APP_MODE')=='demo'?'':$data_brevo['driver']}}">

                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-sm-6">
                                        <div class="form-group">
                                            <label class="form-label" for="">{{translate('sender_ID')}}
                                                <span class="tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="top"
                                                    aria-label="{{translate('enter_your_sender_ID')}}"
                                                    data-bs-title="{{translate('enter_your_sender_ID')}}">
                                                    <i class="fi fi-sr-info"></i>
                                                </span>
                                            </label>
                                            <input type="text"
                                            placeholder="{{translate('ex').':'}}{{translate('example@example.com')}}"
                                            class="form-control" name="email"
                                            value="{{env('APP_MODE')=='demo'?'':$data_brevo['email_id']}}">
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-sm-6">
                                        <div class="form-group">
                                            <label class="form-label" for="brevoApiKey">{{translate('api_key')}}
                                                <span class="tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="top"
                                                    aria-label="{{translate('enter_your_brevo_api_key')}}"
                                                    data-bs-title="{{translate('enter_your_brevo_api_key')}}">
                                                    <i class="fi fi-sr-info"></i>
                                                </span>
                                            </label>
                                            <div class="input-group">
                                                <input type="password" class="form-control"
                                                               name="password" id="brevoApiKey"
                                                               placeholder="{{translate('ex')}}:xkeysib-xxxxxxxx"
                                                               value="{{env('APP_MODE')=='demo'?'':$data_brevo['password']}}">
