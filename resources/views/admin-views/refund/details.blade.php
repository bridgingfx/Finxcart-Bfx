...
@section('content')
                ...
                <div class="card h-100 refund-details-card">
                    <div class="card-body">
                        ...
                        <ul class="dm-info p-0 m-0">
                            ...
                            <li class="align-items-center">
                                <span class="left">{{translate('refund_status')}}</span> <span>:</span> <span class="right">
                                    @if ($refund['status'] == 'pending')
                                        <span class="badge badge-secondary text-bg-secondary"> {{translate($refund['status'])}}</span>
                                    @elseif($refund['status'] == 'approved')
                                        <span class="badge badge-primary text-bg-primary"> {{translate($refund['status'])}}</span>
                                    @elseif($refund['status'] == 'refunded')
                                        <span class="badge badge-success text-bg-success"> {{translate($refund['status'])}}</span>
                                    @elseif($refund['status'] == 'rejected')
                                        <span class="badge badge-danger text-bg-danger"> {{translate($refund['status'])}}</span>
                                    @endif
                                </span>
                            </li>
                            <li class="align-items-center">
                            ...
                            <li class="align-items-center">
                                <span class="left">{{translate('order_details')}} </span> <span>:</span> <span class="right"><a class="badge py-2 badge-primary text-bg-primary border border-primary px-2" href="{{route('admin.orders.details',['id'=>$order->id])}}">{{translate('view_details')}}</a></span>
                            </li>
                        </ul>
                    </div>
                </div>
                            ...
                            <div class="d-flex flex-wrap gap-3">
                                @if($refund['status'] != 'refunded')
                                    @if($refund['status'] != 'rejected')
                                        <button class="btn bg-danger bg-opacity-10 text-danger p-2 px-3" data-bs-toggle="modal" data-bs-target="#rejectModal">
                                            {{ translate('reject') }}
                                        </button>
                                    @endif
                                    @if($refund['status'] != 'approved')
                                        <button class="btn bg-primary bg-opacity-10 text-primary p-2 px-3" data-bs-toggle="modal" data-bs-target="#approveModal">
                                            {{ translate('approve') }}
                                        </button>
                                    @endif
                                    <button class="btn bg-success bg-opacity-10 text-success p-2 px-3" data-bs-toggle="modal" data-bs-target="#refundModal">
                                        {{ translate('refund') }}
                                    </button>
                                @endif
                            </div>
                        ...
                        <div class="refund-details">
                            <div class="img">
                                <div class="onerror-image border rounded">
                                    <img class="aspect-1" src="{{getStorageImages(path: ($refund->product ? $refund->product->thumbnail_full_url : ''),type: 'backend-product')}}" alt="">
                                </div>
                            </div>
                            <div class="--content flex-grow-1">
                                ...
                                @if($refund->orderDetails->digital_file_after_sell)
                                    ...
                                    <a href="{{file_exists( $downloadPath) ?  $downloadPath : 'javascript:' }}" class="btn btn-outline-primary btn-sm mt-3 {{file_exists( $downloadPath) ?  $downloadPath : 'download-path-not-found'}}" title="{{translate('download')}}">
                                        {{translate('download')}} <i class="fi fi-rr-download"></i>
                                    </a>
                                @endif
                            </div>
                            <ul class="dm-info p-0 m-0 w-l-115">
                                ...
                                <li>
                                    ...
                                    <span class="right">
                                        <strong>
                                            {{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $subtotal - $couponDiscount), currencyCode: getCurrencyCode())}}
                                        </strong>
                                    </span>
                                </li>
                            ...
                            @else
                                <div class="p-2 bg-light rounded">
                                    <div class="media m-1 gap-3">
                                        <img class="avatar rounded-circle" src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/delivery-man.png')}}" alt="{{translate('image')}}">
                            ...
                            <div class="table-responsive">
                                ...
                                @if(count($refund->refundStatus)==0)
                                    <div class="text-center p-4">
                                        <img class="mb-3 w-160" src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/empty-state-icon/default.png')}}"
    ...
    <div class="modal fade" id="rejectModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{route('admin.refund-section.refund.refund-status-update')}}" method="post" id="submit-rejected-form" novalidate>
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="d-flex flex-wrap justify-content-end gap-3 mt-3">
                            <button type="button" class="btn btn-secondary px-3" data-bs-dismiss="modal">{{ translate('close') }}</button>
    ...
    <div class="modal fade" id="approveModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{route('admin.refund-section.refund.refund-status-update')}}" method="post" id="submit-approve-form" novalidate>
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="id" value="{{$refund->id}}">
    ...
    <div class="modal fade" id="refundModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{route('admin.refund-section.refund.refund-status-update')}}" method="post" id="submit-refund-form" novalidate>
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="id" value="{{$refund->id}}">
