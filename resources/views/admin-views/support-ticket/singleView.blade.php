            ...
            <div class="card-header flex-wrap d-flex justify-content-between align-items-center gap-2">
                ...
                <div class="media d-flex gap-3">
                    ...
                    <div class="media-body">
                        <h5 class="font-size-md mb-1">{{ isset($ticket->customer)?$ticket->customer['f_name'].' '.$ticket->customer['l_name']:translate('not_found')}}</h5>
                        <div class="fs-12">{{ isset($ticket->customer)?$ticket->customer['phone']:''}}</div>
                    </div>
                </div>
                <div class="d-flex align-items-center flex-wrap gap-3">
                    <div class="type fw-bold badge text-bg-info badge-info">{{translate(str_replace('_',' ',$ticket['type']))}}</div>
                    <div class="priority d-flex flex-wrap align-items-center gap-3">
                        <span class="text-dark fs-12">{{translate('priority')}}:</span>
                        <span class="fw-bold badge text-bg-info badge-info">{{translate(str_replace('_',' ',$ticket['priority']))}}</span>
                    </div>
                </div>
                @endforeach
            </div>
                ...
                <div class="incoming_msg  w-75">
                    <div class="received_msg">
                        <div class="received_withdraw_msg">
                            ...
                            @endif
                            <span class="time_date fs-12 pt-2 d-flex justify-content-start"> {{$ticket->created_at->diffForHumans()}} </span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="type_msg">
                    <div class="input_msg_write">
                        @foreach($supportTicket as $reply)
                            <form class="needs-validation" href="{{route('admin.support-ticket.replay',$reply['id'])}}" method="post" enctype="multipart/form-data" novalidate>
                                @csrf
                                ...
                                <div class="position-relative d-flex align-items-center">
                                    @if(theme_root_path() == "default")
                                        <label class="py-0 px-3 d-flex align-items-center m-0 cursor-pointer position-absolute">
                                            ...
                                            <input type="file" id="select-media" class="h-100 position-absolute w-100 " hidden multiple  accept="image/*">
                                        </label>
                                    @endif
