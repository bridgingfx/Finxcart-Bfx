          ...
          <div class="col-md-6">
                <div class="card h-100">
                    <div class="card-header bg--primary--light">
                        {{-- UPDATED TITLE --}}
                        <h5 class="card-title text-capitalize">{{translate('product_video')}} & {{translate('preview_url')}}</h5>
                    </div>
                    <div class="card-body">
                        {{-- VIDEO SECTION --}}
                        <div>
                        ...
                        {{-- START: NEW PREVIEW URL SECTION --}}
                        <hr class="my-4">
                        <div>
                            <h6 class="mb-3 text-capitalize">
                                {{ translate('preview_url') }}
                            </h6>
                        </div>
                        @if($product['preview_url'])
                            <a href="{{ (str_contains($product->preview_url, "https://") || str_contains($product->preview_url, "http://")) ? $product['preview_url'] : "https://".$product['preview_url']}}" target="_blank"
                               class="text-primary">
                                {{ $product['preview_url'] }}
                            </a>
                        @else
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('admin.reviews.status') }}"
                                        method="POST" id="reviews-status{{ $review['id']}}-form" novalidate>
                                        @csrf
                 ...
                 aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form method="POST" action="{{ route('admin.reviews.add-review-reply') }}" novalidate>
                                @csrf
                                <div class="modal-body pt-0">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-group" action="{{ route('admin.products.deny', ['id'=>$product['id']]) }}"
                      method="post" id="product-status-denied" novalidate>
                    @csrf
