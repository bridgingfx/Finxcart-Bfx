                    ...
                    <div class="row g-2">
                                    ...
                                    <select name="seller_id" class="custom-select"
                                        ...
                                        @foreach ($sellers as $seller)
                                            <option
                                                ...
                                                {{ $seller->shop->name ?? $seller->f_name ?? $seller->email }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                ...
                                <select class="custom-select action-get-request-onchange"
                                        ...
                                        data-id="{{ request('sub_category_id') }}"
                                        data-url-prefix="{{ url('/admin/products/get-categories?parent_id=') }}"
                        ...
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                ...
                                <select class="custom-select" data-placeholder="Select from dropdown"
                                        ...
                                        data-id="{{ request('sub_sub_category_id') }}"
                        ...
                        <div class="col-12">
                            <div class="d-flex gap-3 justify-content-end mt-4">
                                <a href="{{ route('admin.products.list', [$type == 'seller' ? 'vendor' : 'in_house']) }}"
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="flex-grow-1 max-w-280">
                                    <form action="{{ url()->current() }}" method="get" novalidate>
                                        ...
                                        <input type="hidden" value="{{ request('brand_id') }}" name="brand_id">
                                        ...
                                        <input type="hidden" value="{{ request('sub_sub_category_id') }}" name="sub_sub_category_id">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                                <div class="dropdown">
                                    <a type="button" class="btn btn-outline-primary text-nowrap"
                                       href="{{ route('admin.products.export-excel', [$type == 'seller' ? 'vendor' : 'in_house']) }}?brand_id={{request('brand_id') }}&searchValue={{ request('searchValue') }}&category_id={{request('category_id') }}&sub_category_id={{request('sub_category_id') }}&sub_sub_category_id={{request('sub_sub_category_id') }}&seller_id={{request('seller_id') }}&status={{request('status') }}">
                                        <img width="14"
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            <tr>
                                ...
                                {{-- --- NEW COLUMN HEADERS (Only show for vendor products) --- --}}
                                @if($type == 'seller')
                                    <th>{{ translate('Vendor_Name') }}</th>
                                    <th>{{ translate('Tier_Plan') }}</th>
                                @endif
                            ...
                            @foreach($products as $key => $product)
                                <tr>
                                    ...
                                    {{-- --- NEW COLUMN DATA (Only show for vendor products) --- --}}
                                    @if($type == 'seller')
                                        <td>
                                            ...
                                            @else
                                                <span class="text-muted">{{ translate('Vendor_not_found') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-10">
                                            <a class="btn btn-outline-info icon-btn" title="{{ translate('barcode') }}"
    ...
    <script>
        $(document).ready(function () {
            setTimeout(function () {
                ...
                let subCategory = $("#sub-category-select").attr("data-id");
                let subSubCategory = $("#sub-sub-category-select").attr("data-id");

                if (category) {
                    getRequestFunctionality('{{ route('admin.products.get-categories') }}?parent_id=' + category + '&sub_category=' + (subCategory ?? ''), 'sub-category-select', 'select');
                }

                if (subCategory) {
                    getRequestFunctionality('{{ route('admin.products.get-categories') }}?parent_id=' + subCategory + '&sub_category=' + (subSubCategory ?? ''), 'sub-sub-category-select', 'select');
                }
            }, 100);
        });
    </script>
