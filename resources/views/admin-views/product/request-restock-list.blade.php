                        ...
                        <div class="col-sm-6 col-lg-6 col-xxl-3">
                            <div class="form-group">
                                ...
                                <div class="position-relative">
                                    ...
                                    <input type="text" class="js-daterangepicker previous-date-true placeholder-mode-true form-control" name="restock_date" placeholder="{{ translate('Select_Date') }}" value="{{request('restock_date')}}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-6 col-xxl-3">
            ...
            <div class="card">
                <div class="px-3 py-4 d-flex justify-content-between align-items-center gap-20 flex-wrap">
                    <h3 class="mb-0">
                        ...
                        <span class="badge text-dark bg-body-secondary fw-semibold rounded-50">{{ $restockProducts->total() }}</span>
                    </h3>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.products.view',['addedBy'=>($restockProduct->product['added_by'] == 'seller' ? 'vendor' : 'in-house'),'id'=>$restockProduct->product['id'] ?? 0]) }}"
                                        ...
                                        <img loading="lazy" src="{{ getStorageImages(path: $restockProduct?->product?->thumbnail_full_url, type: 'backend-product') }}"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a class="btn btn-outline-info icon-btn" title="View"
                                            ...
                                            <img src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/icons/restock_view.svg') }}" alt="">
                                        </a>
                                        <a class="btn btn-outline-primary icon-btn action-update-product-quantity"
                                            ...
                                            <img src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/icons/restock_update.svg') }}" alt="">
                                        </a>
                                        <span class="btn btn-outline-danger icon-btn delete-data"
                                                title="{{ translate('delete') }}"
                                                data-id="product-{{ $restockProduct->id}}">
                                            <img src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/icons/restock_delete.svg') }}" alt="">
                                        </span>
                                    </div>
                                    <form action="{{ route('admin.products.restock-delete',[$restockProduct->id]) }}"
                                            method="post" id="product-{{ $restockProduct->id}}" novalidate>
                                        @csrf @method('delete')
                                    </form>
                                </td>
                            </tr>
    ...
    <div class="modal fade update-stock-modal restock-stock-update" id="update-stock" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('admin.products.update-quantity') }}" method="post" class="odal-body p-20" novalidate>
                    @csrf
