...
@section('title', translate('freelancer_messages'))
...
@push('css_or_js')
    <style>
        ...
        .fl-chat-bubble { max-width: 70%; padding: 10px 14px; border-radius: 14px; font-size: 14px; line-height: 1.45; overflow-wrap: break-word; }
        ...
        .fl-chat-bubble-seller { background: #f1f3f6; color: #1f2937; border-bottom-left-radius: 4px; }
        ...
        .fl-chat-send-btn { background: #073b74; border-color: #073b74; color: #fff; min-width: 86px; font-weight: 700; }
        .fl-chat-send-btn:hover, .fl-chat-send-btn:focus { background: #052f5d; border-color: #052f5d; color: #fff; box-shadow: 0 0 0 3px rgba(7, 59, 116, 0.16); }
        .fl-chat-send-btn:active { background: #042648; border-color: #042648; color: #fff; }
        ...
        .fl-chat-unread-badge { margin-left: auto; min-width: 22px; height: 22px; border-radius: 999px; display: inline-flex; align-items: center; justify-content: center; background: #e12d55; color: #fff; font-size: 11px; font-weight: 700; padding: 0 7px; }
        .fl-chat-unread-badge.d-none { display: none !important; }
        ...
        .fl-chat-bubble { max-width: min(72%, 560px); padding: 11px 14px; border-radius: 8px; box-shadow: 0 8px 22px rgba(15, 23, 42, .07); overflow-wrap: break-word; }
...
@section('content')
    <div class="content container-fluid">
        <div class="mb-4">
            <h2 class="h1 mb-0 text-capitalize">{{ translate('freelancer_messages') }}</h2>
            <p class="text-muted mb-0">{{ translate('message_a_freelancer_directly') }}</p>
        </div>
                ...
                <div class="card h-100">
                    ...
                    <div class="card-body p-2 fl-chat-col">
                        @forelse($freelancers as $freelancer)
                            <a href="{{ route('admin.freelancer.messages.index', ['seller_id' => $freelancer->id]) }}"
                               class="fl-chat-list-item {{ $selectedFreelancer && $selectedFreelancer->id === $freelancer->id ? 'active' : '' }}">
                                ...
                                @php($unreadCount = $unreadCounts[$freelancer->id] ?? 0)
                                <span class="text-truncate flex-grow-1">{{ $freelancer->f_name }} {{ $freelancer->l_name }}</span>
                                <span class="fl-chat-unread-badge {{ $unreadCount > 0 ? '' : 'd-none' }}"
                                      data-freelancer-unread-badge="{{ $freelancer->id }}">{{ $unreadCount > 99 ? '99+' : $unreadCount }}</span>
                            </a>
                        @empty
                ...
                <div class="card h-100">
                    @if($selectedFreelancer)
                        <div class="card-header bg-white d-flex align-items-center gap-2">
                            <img class="fl-chat-avatar" src="{{ getStorageImages(path: $selectedFreelancer->image_full_url, type: 'backend-profile') }}" alt="">
                            <div>
                                <div class="fw-semibold">{{ $selectedFreelancer->f_name }} {{ $selectedFreelancer->l_name }}</div>
                                <small class="text-muted">{{ translate("last_login") }} {{ $selectedFreelancer->updated_at?->diffForHumans() }}</small>
                            </div>
                        </div>
                        <div class="card-body fl-chat-col" id="admin-freelancer-chat-messages">
                        ...
                        <div class="fl-chat-typing-indicator px-3 py-1 small text-muted d-none" id="fl-chat-typing-indicator">
                            {{ $selectedFreelancer->f_name }} {{ translate('is_typing') }}...
                        </div>

                        <div class="card-footer bg-white">
                            <form action="{{ route('admin.freelancer.messages.send') }}" method="POST" enctype="multipart/form-data" class="fl-chat-upload-form" id="fl-chat-form" novalidate>
                                @csrf
                                ...
                                <div class="d-flex gap-2 align-items-end">
                                    ...
                                    <textarea name="message" rows="1" class="form-control" id="fl-chat-input" placeholder="{{ translate('type_your_message') }}"></textarea>
                                    <button type="submit" class="btn fl-chat-send-btn flex-shrink-0" id="fl-chat-send-btn">{{ translate('send') }}</button>
                                </div>
...
@push('script')
    <script>
        (function () {
            const chatBox = document.getElementById('admin-freelancer-chat-messages');
            const form = document.getElementById('fl-chat-form');
            const textarea = document.getElementById('fl-chat-input');
            const sendBtn = document.getElementById('fl-chat-send-btn');
            const typingIndicator = document.getElementById('fl-chat-typing-indicator');
            ...
            const unreadCountsUrl = @json(route('admin.freelancer.messages.unread-counts'));
            const refreshUnreadBadges = () => {
                fetch(unreadCountsUrl, {
                    ...
                    .then((data) => {
                        ...
                        document.querySelectorAll('[data-freelancer-unread-badge]').forEach((badge) => {
                            const sellerId = badge.getAttribute('data-freelancer-unread-badge');
                            const unreadCount = parseInt(data.counts[sellerId] || 0, 10);
            ...
            // Live chat: while this freelancer's thread is open, re-fetch it roughly
            // every second so incoming messages show up without a reload. Only swap
            // the DOM (and re-anchor scroll) while the admin is already near the
            // bottom, so scrolling up into history isn't interrupted mid-read, and
            // skip while a send is in flight so the two responses can't race.
            const selectedFreelancerId = @json($selectedFreelancer?->id);
            if (selectedFreelancerId && chatBox) {
                const threadUrl = @json(route('admin.freelancer.messages.thread'));
                const pollThread = () => {
                    ...
                    fetch(threadUrl + '?seller_id=' + selectedFreelancerId, {
                ...
                // Typing indicator: ping the server (throttled) on every keystroke,
                // and poll (fast, cheap cache lookup) for whether the freelancer is
                // typing back.
                const typingUrl = @json(route('admin.freelancer.messages.typing'));
                const typingStatusUrl = @json(route('admin.freelancer.messages.typing-status'));
                if (textarea) {
                    let lastPing = 0;
                    textarea.addEventListener('input', function () {
                        ...
                        lastPing = now;
                        fetch(typingUrl + '?seller_id=' + selectedFreelancerId, {
                            method: 'POST',
                            headers: {
                ...
                if (typingIndicator) {
                    const pollTypingStatus = () => {
                        ...
                        fetch(typingStatusUrl + '?seller_id=' + selectedFreelancerId, {
                            headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
                        })
                            .then((response) => response.ok ? response.json() : null)
                            .then((data) => {
                ...
                form.addEventListener('submit', function (event) {
                    ...
                    fetch(form.action, {
                        ...
                        .then((data) => {
                            ...
                            textarea.value = '';
                            const preview = form.querySelector('.fl-chat-file-preview');
                            if (preview) preview.textContent = '';
                            document.querySelectorAll('.fl-chat-file-input').forEach((input) => { input.value = ''; });
                        })
                        .catch(() => {
