                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table
                                ...
                                @php $tax=0; @endphp
                                @php $row=0; @endphp
                                @php $sortedOrderDetails = $order->details->sortBy(fn($d) => ($d->product_type ?? 'physical') === 'digital' ? 1 : 0)->values(); @endphp
                                @php $currentItemSection = null; @endphp
                                @foreach($sortedOrderDetails as $key=>$detail)
                                    ...
                                    @if($productDetails)
                                        <tr>
                                            ...
                                            @php $subtotal=$detail['price']*$detail['qty']+$detail['tax']-$detail['discount']; @endphp
            ...
            <div class="col-lg-4 d-flex flex-column gap-3">
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div class="">
                            <label
                                class="form-label fw-semibold">{{ $physicalProduct ? translate('physical_delivery_status') : translate('change_order_status') }}</label>
                            <div class="select-wrapper">
                                <select name="order_status" id="order_status"
                                    class="status form-select" data-id="{{$order['id']}}" data-current-status="{{ $currentOrderStatus }}">
                        ...
                        @if($order->sourceShipment?->has_digital)
                            ...
                            @endphp
                            <div class="">
                                <label class="form-label fw-semibold">{{ translate('digital_delivery_status') }}</label>
                                <div class="d-flex justify-content-between align-items-center form-control h-auto">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="media d-flex gap-3 align-items-center">
                            ...
                            @elseif(empty($order->seller))
                                {{-- The vendor who placed this order no longer exists (deleted after
                                     the order was placed). Orders are intentionally kept when a vendor
                                     is deleted, so make that distinction explicit here rather than
                                     falling through to the generic "no shop found" empty state below. --}}
                                <div class="text-center p-4">
                                    <img class="w-25" src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/empty-state-icon/shop-not-found.png')}}"
                                         alt="{{translate('image_description')}}">
                                    <p class="mb-0 text-danger fw-medium">{{ translate('Vendor_No_Longer_Exists') }}</p>
                                </div>
                            @else
    ...
    <div class="modal fade" id="shippingAddressUpdateModal" tabindex="-1" aria-labelledby="shippingAddressUpdateModal"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body px-4 px-sm-5 pt-0">
                    <form action="{{route('admin.orders.address-update')}}" method="post" novalidate>
                        @csrf
                        <div class="d-flex flex-column align-items-center gap-2">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12">
                            <form action="{{route('admin.orders.update-deliver-info')}}" method="POST" novalidate>
                                @csrf
