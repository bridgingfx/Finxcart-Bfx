...
@section('content')
    <div class="content container-fluid">
        <div>
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.orders.list',['status'=>request('status')]) }}" id="form-data" method="GET" novalidate>
                        <div class="row g-3">
                            <div class="col-12">
                        ...
                        <table class="table table-hover table-borderless">
                            ...
                            @foreach($orders as $key=>$order)
                                ...
                                <tr class="status-{{$order['order_status'] }} class-all">
                                    ...
                                    <td >
                                        <a class="text-dark" href="{{route('admin.orders.details',['id'=>$order['id']])}}">{{ $order->parent_order_id ? $order->parent_order_id.'-'.$order->child_number : $order['id'] }} {!! $order->order_type == 'POS' ? '<span class="text--primary">(POS)</span>' : '' !!}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @if(isset($order->seller_id) && isset($order->seller_is))
                                            @if($order->seller_is == 'seller' && !$order->seller)
                                                {{-- The vendor was deleted after this order was placed; orders
                                                     are intentionally kept, so surface that plainly instead of
                                                     the generic "store not found" (shop-not-set-up) message. --}}
                                                <label class="badge badge-danger text-bg-danger">
                                                    {{ translate('Vendor_No_Longer_Exists') }}
                                                </label>
                                            @else
                                    ...
                                    <td>
                                        ...
                                        <form action="{{route('admin.orders.delete')}}"
                                            method="post" id="order-{{$order['id']}}" class="no-reload-form reload-true" novalidate>
                                            @csrf
