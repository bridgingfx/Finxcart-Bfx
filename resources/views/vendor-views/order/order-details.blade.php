...
@section('content')
    @php $shippingAddress = $order['shipping_address_data'] ?? null; @endphp
    <div class="content container-fluid">
        ...
        <div class="row gy-3" id="printableArea">
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-10 flex-md-nowrap justify-content-between mb-4">
                            <div class="d-flex flex-column gap-10">
                                <h4 class="text-capitalize">
                                    {{ $order->parent_order_id ? translate('Shipment ID') : translate('Order_ID') }} #{{ $order->parent_order_id ? $order->parent_order_id.'-'.$order->child_number : $order['id'] }}
                        ...
                        <div class="table-responsive datatable-custom">
                            <table
                                ...
                                <tbody>
                                @php
                                    $item_price=0; $total_price=0; $subtotal=0; $total=0;
                                    $shipping=0; $discount=0; $tax=0; $row=0;
                                    $sortedOrderDetails = ($order->details ?? collect())->sortBy(fn($d) => ($d->product_type ?? 'physical') === 'digital' ? 1 : 0)->values();
                                    $currentItemSection = null;
                                @endphp

                                @foreach($sortedOrderDetails as $key=>$detail)
                                    @php $itemSection = ($detail->product_type ?? 'physical') === 'digital' ? 'digital' : 'physical'; @endphp
                                    @if($itemSection !== $currentItemSection)
                                        @php $currentItemSection = $itemSection; @endphp
                                        <tr class="bg-light">
                                            <td colspan="6" class="fs-13 font-weight-bold text-capitalize py-2">
                                                <i class="tio-{{ $itemSection === 'digital' ? 'download' : 'box' }}"></i>
                                                {{ translate($itemSection === 'digital' ? 'digital_items' : 'physical_items') }}
                                            </td>
                                        </tr>
                                    @endif
                                    <?php
                                    ...
                                    @if($productDetails)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if(!empty($detail->digital_delivery_type))
                                                    <div class="mt-2 p-2 rounded border border-info text-wrap" style="font-size:12px">
                                                        <strong>{{ translate('delivery_preference') }}:</strong>
                                                        {{ translate($detail->digital_delivery_type) }}
                                                        @if(!empty($detail->digital_delivery_value))
                                                            — <span class="text-info">{{ $detail->digital_delivery_value }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                                @if(($detail->product_type ?? null) === 'digital')
                                                    <div class="mt-2 d-flex align-items-center gap-2">
                                                        @if($detail->shipmentItem?->delivery_mode === 'auto')
                                                            <span class="badge badge-soft-success fs-11 text-capitalize">{{ translate('delivered_automatically') }}</span>
                                                        @elseif($detail->shipmentItem?->status === 'delivered')
                                                            <span class="badge badge-soft-success fs-11 text-capitalize">{{ translate('delivered') }}</span>
                                                        @else
                                                            <span class="badge badge-soft-warning fs-11 text-capitalize">{{ translate('pending') }}</span>
                                                            <form action="{{ route('vendor.orders.mark-digital-delivered') }}" method="post" class="d-inline" novalidate>
                                                                @csrf
                                        ...
                                        <div class="modal fade" id="fileUploadModal-{{ $detail->id }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <form action="{{ route('vendor.orders.digital-file-upload-after-sell') }}"
                ...
                @php
                    try {
                        $commissionLedger = \App\Models\CommissionLedger::where('order_id', $order['id'])->first();
                    } catch (\Exception $e) {
                ...
                <div class="card mt-3">
                    <div class="card-body">
                        <h4 class="mb-3 d-flex align-items-center gap-2">
                            <img src="{{dynamicAsset(path: 'public/assets/back-end/img/order_report.png')}}" alt="" width="18">
                            {{translate('earnings_breakdown')}}
                        </h4>
                        @if($commissionLedger)
                            <dl class="row fs-14 mb-0">
                                <dt class="col-7 text-capitalize">{{translate('gross_sale')}}</dt>
                                <dd class="col-5 title-color text-right">
                                    <strong>{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $commissionLedger->gross_amount), currencyCode: getCurrencyCode())}}</strong>
                                </dd>
                                <dt class="col-7 text-capitalize">
                                    {{translate('platform_commission')}}
                                    <small class="text-muted">({{$commissionLedger->platform_rate}}%)</small>
                                </dt>
                                <dd class="col-5 title-color text-right">
                                    - <strong>{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $commissionLedger->company_share_amount), currencyCode: getCurrencyCode())}}</strong>
                                </dd>
                                @if($commissionLedger->service_charge_amount > 0)
                                    <dt class="col-7 text-capitalize">{{translate('service_charge')}}</dt>
                                    <dd class="col-5 title-color text-right">
                                        - <strong>{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $commissionLedger->service_charge_amount), currencyCode: getCurrencyCode())}}</strong>
                                    </dd>
                                @endif
                                <dt class="col-7 pt-2 border-top"><strong class="text-success">{{translate('your_earnings')}}</strong></dt>
                                <dd class="col-5 title-color text-right pt-2 border-top">
                                    <strong class="text-success">{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $commissionLedger->net_vendor_payout), currencyCode: getCurrencyCode())}}</strong>
                                </dd>
                            </dl>
                        @else
                            <p class="text-muted mb-0 fs-12">
                                {{ translate('commission_breakdown_available_after_delivery') }}
                            </p>
                        @endif
                    </div>
                </div>
            ...
            <div class="col-lg-4 d-flex flex-column gap-3">
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div>
                            <label
                                class="font-weight-bold title-color fz-14">{{ $physicalProduct ? translate('physical_delivery_status') : translate('change_order_status') }}</label>
                            @php
                                $shippingMethod = getWebConfig(name: 'shipping_method');
                                $sellerwiseShipping = $shippingMethod == 'sellerwise_shipping';
                                $currentOrderStatus = $order->order_status;
                                $orderStatusLabels = [
                                    'pending' => translate('pending'),
                                    'confirmed' => translate('confirmed'),
                                    'processing' => translate('preparing_shipment'),
                                ...
                                $orderStatusTransitions = [
                                    ...
                                    'processing' => $sellerwiseShipping ? ['out_for_delivery', 'canceled'] : ['delivered', 'canceled'],
                                ...
                                $availableOrderStatuses = array_values(array_unique(array_merge(
                                    [$currentOrderStatus],
                                    $orderStatusTransitions[$currentOrderStatus] ?? []
                                )));
                            @endphp
                            <select name="order_status" id="order_status" class="status form-control"
                                    data-id="{{$order['id']}}" data-current-status="{{ $currentOrderStatus }}">
                                @foreach($availableOrderStatuses as $status)
                                    <option value="{{ $status }}" {{ $currentOrderStatus == $status ? 'selected' : '' }}>
                                        {{ $orderStatusLabels[$status] ?? translate(str_replace('_', ' ', $status)) }}
                                    </option>
                                @endforeach
                            </select>
                            @if(count($availableOrderStatuses) > 1)
                                <small class="text-muted d-block mt-1">
                                    {{ translate('next') }}:
                                    {{ collect(array_slice($availableOrderStatuses, 1))->map(fn($status) => $orderStatusLabels[$status] ?? translate(str_replace('_', ' ', $status)))->join(', ') }}
                                </small>
                            @else
                                <small class="text-muted d-block mt-1">{{ translate('this_order_status_is_final') }}</small>
                            @endif
                        </div>
                        @if($order->sourceShipment?->has_digital)
                            @php
                                $digitalItems = ($order->details ?? collect())->filter(fn($d) => ($d->product_type ?? null) === 'digital');
                                $digitalDeliveredCount = $digitalItems->filter(fn($d) => $d->shipmentItem?->status === 'delivered')->count();
                                $digitalTotalCount = $digitalItems->count();
                                $digitalAllDelivered = $order->sourceShipment->digital_status === 'delivered';
                            @endphp
                            <div>
                                <label class="font-weight-bold title-color fz-14">{{ translate('digital_delivery_status') }}</label>
                                <div class="d-flex justify-content-between align-items-center form-control h-100">
                                    <span class="badge badge-soft-{{ $digitalAllDelivered ? 'success' : 'warning' }} fs-12 text-capitalize">
                                        {{ $digitalAllDelivered ? translate('delivered') : translate('pending') }}
                                    </span>
                                    <span class="text-muted fs-12">{{ $digitalDeliveredCount }}/{{ $digitalTotalCount }} {{ translate('items_delivered') }}</span>
                                </div>
                                <small class="text-muted d-block mt-1">
                                    {{ translate('digital_items_are_delivered_instantly_when_marked_-_no_status_stages_like_physical_shipping') }}
                                </small>
                            </div>
                        @endif
        ...
        <div class="modal fade" id="shippingAddressUpdateModal" tabindex="-1" aria-labelledby="shippingAddressUpdateModal"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body px-4 px-sm-5 pt-0">
                        <form action="{{route('vendor.orders.address-update')}}" method="post" novalidate>
                            @csrf
                            <div class="d-flex flex-column align-items-center gap-2">
                    ...
                    <div class="row">
                        <div class="col-md-12 rounded border p-3">
                            <div class="h3 text-cyan-blue text-center">
                                {{ $order->parent_order_id ? translate('Shipment ID') : translate('order') }}
                                #{{ $order->parent_order_id ? $order->parent_order_id.'-'.$order->child_number : $order->id }}
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12">
                            <form action="{{route('vendor.orders.update-deliver-info')}}" method="POST" novalidate>
                                @csrf
