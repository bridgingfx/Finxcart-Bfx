...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{route('vendor.delivery-man.update',[$deliveryMan['id']])}}" method="post" id="update-delivery-man-form" enctype="multipart/form-data" novalidate>
            @csrf
            <div class="card mb-3">
                <div class="card-body">
...
@push('script_2')
    ...
    <script>
        ...
        window.formValidators['update-delivery-man-form'] = function () {
            let isValid = true;
            if (!FormValidators.required('input[name="f_name"]', "{{ translate('first_name_field_is_required') }}")) isValid = false;
            if (!FormValidators.required('input[name="l_name"]', "{{ translate('last_name_field_is_required') }}")) isValid = false;
            if (!FormValidators.required('input[name="phone"]', "{{ translate('phone_field_is_required') }}")) isValid = false;
            else if (!FormValidators.phone('input[name="phone"]', "{{ translate('please_enter_a_valid_phone_number') }}")) isValid = false;
            if (!FormValidators.required('input[name="email"]', "{{ translate('email_field_is_required') }}")) isValid = false;
            else if (!FormValidators.email('input[name="email"]', "{{ translate('please_enter_a_valid_email_address') }}")) isValid = false;
            if (!FormValidators.required('#user_password', "{{ translate('password_field_is_required') }}")) isValid = false;
            else if (!FormValidators.minLength('#user_password', 8, "{{ translate('password_must_be_at_least_8_characters') }}")) isValid = false;
            if (!FormValidators.required('#confirm_password', "{{ translate('confirm_password_field_is_required') }}")) isValid = false;
            else if (!FormValidators.passwordsMatch('#user_password', '#confirm_password', "{{ translate('password_and_confirm_password_must_match') }}")) isValid = false;
