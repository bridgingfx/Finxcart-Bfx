            ...
            <div class="navbar-vertical-footer-offset pb-0">
                <div class="navbar-vertical-content">
                    ...
                    <ul class="navbar-nav navbar-nav-lg nav-tabs">
                        ...
                        {{-- $orderStatusCounts / $refundStatusCounts are computed once in the
                             layouts.vendor.app view composer (AppServiceProvider) and shared here. --}}
                        @if ($sellerPOS == 1 && $seller['pos_status'] == 1 && $seller?->seller_type !== 'freelancer')
                        ...
                        <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/messages*')?'active':''}}">
                            <a id="vendorSidebarChatToggle" class="nav-link"
                               href="{{route('vendor.messages.index', ['type' => 'customer'])}}" title="{{translate('inbox')}}">
                                <i class="tio-chat nav-icon"></i>
                                <span id="vendorSidebarChatToggleInner" class="navbar-vertical-aside-mini-mode-hidden-elements text-truncate d-flex align-items-center justify-content-between gap-2">
                                    <span>{{translate('inbox')}}</span>
                                    @php($unseenMessageCount = (int) ($chattingCounts->unseen_total ?? 0))
                                    @if($unseenMessageCount > 0)
                                        <span id="vendorSidebarChatUnseenBadge" class="vendor-sidebar-required-badge">{{ $unseenMessageCount }}</span>
                                    @endif
                                </span>
                            </a>
                        </li>
                        ...
                        <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/business-settings/withdraw*')?'active':''}}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                               href="{{route('vendor.business-settings.withdraw.index')}}" title="{{translate('wallet')}}">
                        ...
                        <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/business-settings/bank*')?'active':''}}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                               href="{{route('vendor.business-settings.bank.index')}}" title="{{translate('My_Banks')}}">
                        ...
                        @php($sidebarVendor = auth('seller')->user())
                        @php($sidebarVerification = $sidebarVendor?->vendorVerification)
                        @php($sidebarKycMethod = getWebConfig('kyc_method') ?? 'manual')
                        @php($sidebarCompanyIncomplete = ($sidebarVendor?->status ?? null) !== 'approved' && ($sidebarVerification?->status ?? null) !== 'approved')
                        @php($sidebarPayoutKycIncomplete = $sidebarKycMethod !== 'disabled' && ($sidebarVendor?->kyc_status ?? null) !== 'approved')
                        @if($sidebarCompanyIncomplete)
                            <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/verification*')?'active':''}}">
                                <a class="js-navbar-vertical-aside-menu-link nav-link vendor-verification-menu-link"
                                   href="{{ route('vendor.verification.form') }}" title="{{translate('Company_Verification')}}">
                                    <i class="tio-verified nav-icon"></i>
                                    <span class="navbar-vertical-aside-mini-mode-hidden-elements vendor-verification-menu-text">
                                        <span class="d-block text-truncate text-capitalize">{{translate('Company_Verification')}}</span>
                                        <span class="vendor-sidebar-required-badge">
                                            Required
                                        </span>
                                    </span>
                                </a>
                        ...
                        @endif
                        @if($sidebarPayoutKycIncomplete)
                            <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/payout-kyc*')?'active':''}}">
                                <a class="js-navbar-vertical-aside-menu-link nav-link vendor-verification-menu-link"
                                   href="{{ route('vendor.payout-kyc.index') }}" title="{{translate('Payout_KYC')}}">
                                    <i class="tio-security-on nav-icon"></i>
                                    <span class="navbar-vertical-aside-mini-mode-hidden-elements vendor-verification-menu-text">
                                        <span class="d-block text-truncate text-capitalize">{{translate('Payout_KYC')}}</span>
                                        <span class="vendor-sidebar-required-badge">
                                            Required
                                        </span>
                                    </span>
                        ...
                        <li class="nav-item">
                            <small class="nav-subtitle">{{translate('business_tier_management')}}</small>
                            <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                        </li>
                        <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/tier')?'active':''}}">
                            <a class="nav-link"
                               href="{{route('vendor.tier.index')}}" title="{{translate('tier')}}">
                                <i class="tio-users-switch nav-icon"></i>
                                <span
                                    class="navbar-vertical-aside-mini-mode-hidden-elements text-truncate">{{translate('tier')}}</span>
                            </a>
                        </li>
                         <li class="navbar-vertical-aside-has-menu {{Request::is('vendor/tier/my-tier-plan')?'active':''}}">
                            <a class="nav-link"
                               href="{{ route('vendor.tier.my-tier-plan') }}" title="{{translate('my_tier_plan')}}">
                                <i class="tio-notice nav-icon"></i>
                                <span
                                    class="navbar-vertical-aside-mini-mode-hidden-elements text-truncate">
                                    {{ translate('my_tier_plan') }}
                                </span>
                            </a>
                        </li>
                        @php( $shippingMethod = getWebConfig('shipping_method'))
