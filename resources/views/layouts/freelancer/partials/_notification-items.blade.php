...
@foreach ($notification_data as $item)
    <a href="{{ route('freelancer.dashboard.index') }}" class="dropdown-item position-relative notification-data-view" data-id="{{ $item->id }}" data-notification-type="system" data-redirect-url="{{ route('freelancer.dashboard.index') }}">
...
@if(isset($unreadNotifications))
    @foreach ($unreadNotifications as $item)
        @php($fallbackLink = $item->title === 'New Message from Admin' ? route('freelancer.messages.admin') : route('freelancer.dashboard.index'))
        ...
        @php($notificationLink = ($rawNotificationLink !== '' && !str_contains($rawNotificationLink, 'object Object') && (str_starts_with($rawNotificationLink, '/') || filter_var($rawNotificationLink, FILTER_VALIDATE_URL))) ? $rawNotificationLink : $fallbackLink)
