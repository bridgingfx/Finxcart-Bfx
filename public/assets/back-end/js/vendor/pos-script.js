...
function basicFunctionalityForCartSummary() {
    ...
    $(".action-form-submit").on("click", function () {
        // #order-place carries js-skip-auto-validate (this handler submits it via
        // AJAX, not a native submit, so the generic document-level submit handler
        // never runs on it) — call the generic engine explicitly so any required
        // field added to this form in the future is actually enforced instead of
        // silently doing nothing.
        if (typeof FormValidators !== "undefined" && !FormValidators.autoValidateForm("#order-place")) {
            return;
        }
