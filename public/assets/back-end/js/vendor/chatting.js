...
$(document).on("ready", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.userData) {
                    ...
                    $("#profile_last_active").html(response.userData.lastActive || "");
    ...
    // Live chat: while a conversation is open and the tab is visible, re-fetch
    // its thread roughly every second so incoming messages appear without a
    // page reload. #chatting-messages-section is flex-column-reverse (newest
    // message first in the DOM, scrollTop 0 shows it) — only swap in fresh
    // HTML while the user is at/near that position, so scrolling into history
    // isn't yanked back to the bottom mid-read. Shared by the Vendor and
    // Freelancer inboxes, which both load this same script. Skipped while a
    // send is in flight so the two responses can't race and clobber each other.
    (function () {
        function pollActiveChatThread() {
            let userId = $("#current-user-hidden-id").val();
            if (!userId || document.hidden || sendInFlight) {
                return;
            }
            let $container = $("#chatting-messages-section");
            if ($container.length === 0 || $container.scrollTop() > 40) {
                return;
            }
            let actionURL = $("#chatting-post-url").data("url") + userId;
            $.ajax({
                ...
                type: "GET",
                success: function (response) {
                    if (response.chattingMessages) {
                        $container.html(response.chattingMessages);
                    }
                },
            });
        }

        setInterval(pollActiveChatThread, 1000);
        document.addEventListener("visibilitychange", function () {
            if (document.visibilityState === "visible") {
                pollActiveChatThread();
            }
        });
    })();

    // Typing indicator: ping the server (throttled) on every keystroke in the
    // active thread, and poll (fast, cheap cache lookup) for whether the peer
    // is typing back. Shared by the Vendor and Freelancer inboxes.
    (function () {
        let lastPing = 0;

        $(document).on("input", "#msgInputValue", function () {
            let userId = $("#current-user-hidden-id").val();
            let $typingUrl = $("#chatting-typing-url");
            if (!userId || $typingUrl.length === 0) {
                return;
            }
            let now = Date.now();
            ...
            $.post($typingUrl.data("url") + userId);
        });

        function pollTypingStatus() {
            let userId = $("#current-user-hidden-id").val();
            let $indicator = $("#chat-typing-indicator");
            let $typingStatusUrl = $("#chatting-typing-status-url");
            if (!userId || $indicator.length === 0 || $typingStatusUrl.length === 0 || document.hidden) {
                return;
            }
            $.ajax({
                url: $typingStatusUrl.data("url") + userId,
                type: "GET",
                success: function (response) {
                    let isTyping = !!(response && response.typing);
                    if (isTyping) {
                        let partnerName = $("#profile_name").first().text().trim();
                        $indicator.text((partnerName ? partnerName + " " : "") + $indicator.data("suffix"));
                    }
                    $indicator.toggleClass("d-none", !isTyping);
                },
            });
        }
    ...
    function toggleVideo() {
        ...
        $("video").on("pause", function () {
            ...
            if (!$(videoElement).siblings(".modal_video-play-btn").length) {
                const iconSrc = $("#get-video-preview-icon").data("videoIcon");
