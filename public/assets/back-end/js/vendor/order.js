...
$("#order_status").on('change', function (e) {
    ...
    let previousValue = statusSelect.find('option[selected]').val() || statusSelect.data('current-status');
    let value = $(this).val();
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success == 0) {
                        toastMagic.warning(data.message ?? $("#message-order-status-delivered-text").data('text'));
                        location.reload();
                    } else {
