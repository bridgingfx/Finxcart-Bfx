// Last-resort safety net: if any AJAX request in the Vendor panel fails and
// nothing else has already shown a styled error (e.g. a page-specific handler
// like product-add-update.js's own error callback), surface it as a toast
// popup instead of letting the browser render the raw JSON response as the
// whole page. jQuery only fires the global ajaxError event for requests that
// didn't set `global: false`, and only after any per-request `error` callback
// has already run — so a request with its own error handling is unaffected,
// this only catches requests that had no handling at all.
$(document).ajaxError(function (event, jqXHR, ajaxSettings, thrownError) {
    if (jqXHR.status === 0 || ajaxSettings.suppressGlobalError) {
        return;
    }
    let message = 'Something went wrong. Please try again.';
    if (jqXHR.responseJSON) {
        if (jqXHR.responseJSON.errors && Array.isArray(jqXHR.responseJSON.errors) && jqXHR.responseJSON.errors[0]) {
            message = jqXHR.responseJSON.errors[0].message || message;
        } else if (jqXHR.responseJSON.errors && typeof jqXHR.responseJSON.errors === 'object') {
            let firstKey = Object.keys(jqXHR.responseJSON.errors)[0];
            let firstError = firstKey ? jqXHR.responseJSON.errors[firstKey] : null;
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Keep Laravel locale as the only language source.
...
function initCustomerListWithoutAllCustomer() {
    $(".get-customer-list-without-all-customer").each(function() {
        var $select = $(this);
        ...
        var isInsideModal = $select.closest(".modal").length > 0;
        ...
        $select.select2({
            ...
            allowClear: true,
            dropdownParent: isInsideModal ? $select.closest(".modal") : null,
            width: '100%',
            ajax: {
...
initCustomerListWithoutAllCustomer();

// Re-initialize when modal is shown
$(document).on('shown.bs.modal', '#add-fund-modal', function() {
    initCustomerListWithoutAllCustomer();
});
...
function resetDependentSelect($select, message) {
    if (!$select.length) {
        return;
    }
...
function getRequestFunctionality(getUrlPrefix, id, getElementType) {
    ...
    let nextElementId = $targetElement.data("element-id");
    resetDependentSelect($targetElement, message);
    resetDependentSelect($("#" + nextElementId), message);
    ...
    $.get({
        ...
        success: function (data) {
            if (getElementType === "select") {
                $targetElement.empty().append(data.select_tag);
                ...
                if (
                    ...
                    $nextElement.empty().append(data.sub_categories);
                    if ($nextElement.hasClass("select2-hidden-accessible")) {
                        $nextElement.trigger("change.select2");
                    }
                }
            }
        },
...
// the Vendor panel: enforces whatever the form's own markup already
...
// validation (e.g. the product add/edit forms) or is a search/filter form.
...
// Second, independent safety net for forms marked js-skip-auto-validate
// AND submitted via AJAX (product add/edit, POS cart, etc.): those forms
// have their own dedicated click-triggered AJAX submit path and rely on that
// handler alone to call e.preventDefault() on the form's native submit —
// if that handler is ever missing/fails to attach, pressing Enter in a text
// field still fires the browser's implicit native submit, which bypasses
// AJAX entirely and renders the server's raw JSON response as the whole
// page. Block every native submit on these forms here too, independent of
// whichever page-specific script is supposed to also be doing it — the two
// guards are redundant by design, not exclusive.
$(document).on("submit", "#product_form, #order-place", function (e) {
    e.preventDefault();
});
...
function productStockLimitStatus() {
    try {
        ...
        // This widget only exists in the Admin/Vendor product panels — the
        // #get-stock-limit-status element (and its data-action URL) isn't present
        // on other panels (e.g. Freelancer) that still load this shared script.
        // Without this guard, a falsy url makes jQuery default the request to the
        // current page's own URL, which then fails JSON parsing and trips the
        // global ajaxError handler, popping a spurious "Something went wrong"
        // toast on every page load there.
        if (!productStockAction) {
            return;
        }
...
$(document).on("click", ".action-input-no-index-event", function () {
    $(".input-no-index-sub-element").prop("checked", true);
});

$(document).on("click", 'input[name="meta_index"][value="index"]', function () {
    $(".input-no-index-sub-element").prop("checked", false);
});
