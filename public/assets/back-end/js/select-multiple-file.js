...
$(document).ready(() => {
    $("#select-file").on('change', function () {
        ...
        let fileSizeError = $('#message-media-error').data('text') || 'File size is too large. Please upload a smaller file';

        for (let index = 0; index < this.files.length; ++index) {
            if (this.files[index].size > maxFileSize) {
                toastMagic.warning(`"${this.files[index].name}" ${fileSizeError}`);
                continue;
            }
