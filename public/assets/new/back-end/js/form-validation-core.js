...
/**
 * Shared client-side form validation helpers for the Admin panel.
 * Uses the same invalid-feedback/is-invalid/text-danger convention already
 * used by @error()-driven server validation across admin views (Bootstrap
 * 5.3.3, stock red) so new validation looks identical to existing rendering.
 */
...
const FormValidators = {
    ...
    /**
     * Generic attribute-driven validator — see the Vendor-panel copy of
     * this file for the full rationale. Walks every visible input/select/
     * textarea in the form and enforces whatever the markup already
     * declares (required, type=email/tel/url/number min-max, minlength,
     * pattern), plus a password/confirm-password match and required
     * radio-group/checkbox checks.
     */
