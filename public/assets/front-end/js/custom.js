...
// Baseline client-side validation for every plain (non-AJAX) POST form on
// the customer-facing site: enforces whatever the form's own markup already
...
$(document).ready(function () {
    ...
    if (stickyElement.length !== 0 && offsetElement.length !== 0) {
        $(window).on("scroll", function () {
            ...
            // On short pages this offset can come out zero or negative, which
            // would hide the sticky button immediately on load/first scroll.
            // Only allow the hide behavior once the offset is meaningfully positive.
            if (elementOffset > 0 && scrollTop >= elementOffset) {
...
$(".change-language").on("click", function () {
    let langCode = $(this).data("language-code");
    let actionUrl = $(this).data("action");

    if (!langCode || !actionUrl) {
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Remove any old browser-translation cookie so Laravel locale stays in control.
...
function restartCustomerOtpTimer(duration, expiresAt) {
    let newCounter = $(".verifyCounter");
    let seconds = parseInt(duration, 10) || 60;
    let timerExpiresAt = parseInt(expiresAt, 10) || (Date.now() + seconds * 1000);
    newCounter.data("expires-at", timerExpiresAt);

    function newTick() {
        let remaining = Math.max(0, Math.ceil((timerExpiresAt - Date.now()) / 1000));
...
$("#customerOtpVerify").click(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 1) {
                restartCustomerOtpTimer(data.new_time || 60, data.otp_expires_at);
                toastr.success(data.message);
            } else {
        ...
        error: function (xhr) {
            ...
            if (data.new_time) {
                restartCustomerOtpTimer(data.new_time, data.otp_expires_at);
            } else {
                $(".resend-otp-button").removeAttr("disabled");
            }
...
$(".search-bar-input").keyup(function () {
    ...
    if (name.length > 0) {
        desktopSearchDebounceTimer = setTimeout(function () {
            $.get({
                url: $("#route-searched-products").data("url"),
                dataType: "json",
                data: {
        ...
        $.post({
            ...
            success: function (response) {
                if (typeof response.cart_count !== "undefined") {
                    $("#user_cart_count").text(response.cart_count);
                }
                ...
                if (response.status == 1) {
                    updateNavCart();
                    let productType = ($(formSelector).data("product-type") ?? "").toString();
                    let successMessage = response.message || 'Product added to cart successfully';
                    ...
                    if (response.popup_product_name) {
                        showCartAddedPopup(response);
                    }
                    ...
                    let $qtyControls = $(formSelector).find('.product-qty-controls');

                    if (productType === "digital") {
                        actionAddToCartBtn.show().html(actionAddToCartBtn.data("update") ?? actionAddToCartBtn.data("add"));
                    } else if ($qtyControls.length) {
                        $qtyControls.removeClass('d-none').addClass('d-flex');
                        actionAddToCartBtn.show().html(actionAddToCartBtn.data("update") ?? actionAddToCartBtn.data("add"));
                        let minusBtn = $(formSelector).find('.btn-number[data-type="minus"][data-field="quantity"]');
                        let qtyInput = $(formSelector).find('.input-number[name="quantity"]');
                        let minQtyVal = parseInt(qtyInput.attr('min') ?? 1);
                        let currentQtyVal = parseInt(qtyInput.val() ?? minQtyVal);
                        minusBtn.removeAttr('disabled').html(currentQtyVal <= minQtyVal ? '<i class="fi fi-rr-trash text-danger"></i>' : '-');
                    } else {
...
function commonFunctionalityForProductView() {
    ...
    $(".product-add-to-cart-button").on("click", function () {
        let btn = $(this);
        let cartUrl = btn.data("cart-url");
        let updateText = btn.data("update");
        let currentText = btn.text().trim();
        if (cartUrl && updateText && currentText === updateText) {
            localStorage.setItem("redirect_after_login", cartUrl);
            location.href = cartUrl;
            return;
        }
        let parentElement = btn.closest(".product-cart-option-container");
...
function cartQuantityInitialize() {
    $(document)
        .off("click.cartQuantity", ".btn-number")
        .on("click.cartQuantity", ".btn-number", function (e) {
        e.preventDefault();
        ...
        if (!isNaN(currentVal)) {
            if (type == "minus") {
                let cartKeyForRemove = productContainer.find('.product-exist-in-cart-list[name="key"]').val();
                let minQtyVal = parseInt(productContainer.find(".input-number").attr("min"));

                if (currentVal > minQtyVal) {
                    productContainer
                        .find(".input-number")
                        .val(currentVal - 1)
                        .change();
                } else if (cartKeyForRemove) {
                    let removeUrl = $("#route-cart-remove").data("url");
                    let token = $('meta[name="_token"]').attr("content");
                    cartItemRemoveFunction(removeUrl, token, cartKeyForRemove, '');
                    return;
                }

                let afterVal = parseInt(productContainer.find(".input-number").val());
                if (afterVal == minQtyVal && !cartKeyForRemove) {
    ...
    $(document)
        .off("focusin.cartQuantity", ".input-number")
        .on("focusin.cartQuantity", ".input-number", function () {
            $(this).data("oldValue", $(this).val());
        });

    $(document)
        .off("change.cartQuantity", ".input-number")
        .on("change.cartQuantity", ".input-number", function () {
        let productType = $(this).data("producttype");
        ...
        let syncCartKey = productContainer.find('.product-exist-in-cart-list[name="key"]').val();
        if (syncCartKey && valueCurrent > 0) {
            let syncProductId = productContainer.find('.product-generated-variation-code').data('product-id');
            clearTimeout($(this).data('cartSyncTimer'));
            let self = this;
            let minusButton = productContainer.find(".btn-number[data-type='minus'][data-field='quantity']");
            minusButton.html(valueCurrent <= minValue ? '<i class="fi fi-rr-trash text-danger"></i>' : '-');
            $(this).data('cartSyncTimer', setTimeout(function () {
                detailsPageCartSync(syncCartKey, syncProductId, valueCurrent);
            }, 400));
        }
    });

    $(document)
        .off("keydown.cartQuantity", ".input-number")
        .on("keydown.cartQuantity", ".input-number", function (e) {
            if (
...
function detailsPageCartSync(cartKey, productId, quantity) {
    let updateUrl = $("#route-cart-updateQuantity-guest").data("url");
    let token = $('meta[name="_token"]').attr("content");
    if (!updateUrl || !cartKey) return;
    $.post(updateUrl, {
        _token: token,
        key: cartKey,
...
function updateQuantity(key, element) {
    $.post(
        ...
        function (response) {
            updateNavCart();
            $("#cart-summary").empty().html(response.htmlView ?? response.data ?? response);
            couponCode();
        }
    );
}
...
function updateCartQuantity(cart_id, product_id, action, event) {
    ...
    if (cart_quantity_of.val() > cart_quantity_of.data("current-stock")) {
        cartItemRemoveFunction(remove_url, token, cart_id, "");
        return false;
    }
...
function getUpdateProductAddUpdateCartBtn(response) {
    try {
        ...
        if (!productVariantExist) {
            ...
            let $qtyControlsRestore = $(".product-qty-controls");
            let productType = ($(".add-to-cart-details-form").data("product-type") ?? "").toString();
            if (productType === "digital") {
                actionAddToCartBtn.show();
                actionAddToCartBtn.html(actionAddToCartBtn.data("add"));
            } else if ($qtyControlsRestore.length) {
                $qtyControlsRestore.addClass('d-none').removeClass('d-flex');
                actionAddToCartBtn.show().html(actionAddToCartBtn.data("add"));
                $(".btn-number[data-type='minus']").attr('disabled', true).html('-');
            } else {
                actionAddToCartBtn.show();
                actionAddToCartBtn.html(actionAddToCartBtn.data("add"));
            }
...
function checkValidityForVariantPrice(formSelector) {
    var qtyInput = $(formSelector).find("input[name=quantity]");
...
function updateProductDetailsTopSection(formSelector, response) {
    ...
    let currentQty = parseInt(response?.in_cart_quantity ?? $(formSelector).find(".product-details-cart-qty").val() ?? 1);
    let minQty = parseInt($(formSelector).find(".product-details-cart-qty").attr("min") ?? 1);
    ...
    let actionAddToCartBtn = $(formSelector).find(".product-add-to-cart-button");
    let $qtyControlsVariant = $(formSelector).find('.product-qty-controls');
    let productType = ($(formSelector).data("product-type") ?? response?.product_type ?? "").toString();
    ...
    if (response?.in_cart_status === 1) {
        if (productType === "digital") {
            actionAddToCartBtn
                .show()
                .html(actionAddToCartBtn.data("update") ?? actionAddToCartBtn.data("add"));
        } else {
            let $addToCartBtn = $(formSelector).find(".product-add-to-cart-button");
            $addToCartBtn.show().html($addToCartBtn.data("update") ?? $addToCartBtn.data("add"));
            if ($qtyControlsVariant.length) {
                $qtyControlsVariant.removeClass('d-none').addClass('d-flex');
                $(formSelector).find('.btn-number[data-type="minus"]').removeAttr('disabled');
            }
        }
    } else {
        actionAddToCartBtn.show();
        actionAddToCartBtn.html(actionAddToCartBtn.data("add"));
        if (productType === "physical" && $qtyControlsVariant.length) {
            $qtyControlsVariant.addClass('d-none').removeClass('d-flex');
        }
    }

    let minusButton = $(formSelector).find('.btn-number[data-type="minus"][data-field="quantity"]');
    if (response?.in_cart_status === 1 && currentQty <= minQty) {
        minusButton.html('<i class="fi fi-rr-trash text-danger"></i>');
    } else {
        minusButton.html('-');
    }
}
...
function responseManager(response) {
    if (response.status === "success") {
        ...
        if (response?.redirectRoute) {
            ...
            let redirectAfterLogin = localStorage.getItem("redirect_after_login");
...
var cartPopupTimer = null;

function showCartAddedPopup(response) {
    var popup = $('#cart-added-popup');
    $('#cart-popup-name').text(response.popup_product_name || '');
    $('#cart-popup-img').attr('src', response.popup_product_thumbnail || '');
    var badge = $('#cart-popup-type-badge');
    if (response.popup_product_type === 'digital') {
        badge.text('Digital').removeClass('physical').addClass('digital');
    } else {
        badge.text('Physical').removeClass('digital').addClass('physical');
    }
    popup.addClass('open');
    if (cartPopupTimer) clearTimeout(cartPopupTimer);
    cartPopupTimer = setTimeout(closeCartAddedPopup, 5000);
}

function closeCartAddedPopup() {
    if (cartPopupTimer) { clearTimeout(cartPopupTimer); cartPopupTimer = null; }
    $('#cart-added-popup').removeClass('open');
}
