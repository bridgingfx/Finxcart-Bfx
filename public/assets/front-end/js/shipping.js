...
function shipping_method_select(get_value){
    ...
    $('#shipping_phone_view').val(shipping_value.phone);
    ...
    if (shipping_value.address && shipping_value.address.trim() !== "") {
    let mapSrc = `https://www.google.com/maps?q=${encodeURIComponent(shipping_value.address)}&output=embed`;
    $('#map').attr('src', mapSrc);
    $('#location').val(shipping_value.address);
    } else if (shipping_value.latitude && shipping_value.longitude) {
        let mapSrc = `https://www.google.com/maps?q=${shipping_value.latitude},${shipping_value.longitude}&output=embed`;
        $('#map').attr('src', mapSrc);
    } else {
        $('#map').attr('src', 'https://www.google.com/maps?q=Dubai&output=embed');
    }
...
function billing_method_select(get_billing_value){
    ...
    $('input[name="billing_phone"]').val(billing_value.phone);
...
function checkoutFromShipping() {
    ...
    $('#shipping_phone_view').val($('#phone').val() || $('#shipping_phone_view').val());
    $('input[name="billing_phone"]').val($('#billing_phone').val() || $('input[name="billing_phone"]').val());
    ...
    if(physical_product === 'yes') {
        ...
        billing_address_same_shipping = sameAsShippingCheckbox && sameAsShippingCheckbox.length ? sameAsShippingCheckbox.is(":checked") : true;

        if (typeof FormValidators !== 'undefined') {
            allAreFilled = FormValidators.autoValidateForm('#address-form');
        } else {
        ...
        if (billing_address_same_shipping != true && billingAddressForm.length > 0) {
            if (typeof FormValidators !== 'undefined') {
                allAreFilled_shipping = FormValidators.autoValidateForm('#billing-address-form');
            } else {
        ...
        billing_address_same_shipping = true;
    }

    if (!allAreFilled || !allAreFilled_shipping) {
        if (typeof toastr !== 'undefined') {
            toastr.error($('.please_fill_out_this_field').data('text') || 'Please fill out all required fields.');
        }
        return;
    }
    ...
    if (isCheckCreateAccount && isCheckCreateAccount.length && isCheckCreateAccount.prop('checked')) {
        if (typeof FormValidators !== 'undefined') {
            let passwordOk = FormValidators.required(customerPassword, 'Password is required')
                & FormValidators.minLength(customerPassword, 8, 'Password must be at least 8 characters')
                & FormValidators.required(customerConfirmPassword, 'Please confirm your password')
                & FormValidators.passwordsMatch(customerPassword, customerConfirmPassword, 'Passwords do not match');
            if (!passwordOk) {
    ...
    $.post({
        ...
        data: {
            ...
            digital_delivery_type: $('#digital_delivery_type').val() || null,
            digital_delivery_value: $('#digital_delivery_value').val() || null,
        },
        ...
        error: function (data) {
            if (data.errors) {
                ...
                // data.responseJSON is undefined on a true network failure/timeout
                // (no server response at all) — fall back instead of throwing and
                // silently swallowing the error toast.
                let error_msg = data.responseJSON && data.responseJSON.errors
