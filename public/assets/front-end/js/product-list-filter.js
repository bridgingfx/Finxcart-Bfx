...
let productListPageData = {
    ...
    sort_by: productListPageBackup.data('sort') || 'latest',
...
function getProductListFilterRender() {
    ...
    $.get({
        ...
        success: function (response) {
            $('#ajax-products-view').html(response?.html_products || response?.view || '');
            $(".view-page-item-count").html(response.total_product || 0);
...
function listPageProductTypeCheck() {
    const productType = (productListPageData?.product_type || '').toString();
